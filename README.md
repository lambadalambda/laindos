...
- Mounts ISO-9660 CD-ROM media as read-only `D:` for file open/read/`EXEC`/overlay load through subdirectories, root and current-directory `FindFirst`/`FindNext`, and minimal MSCDEX detection coverage. QEMU uses BIOS EDD reads; 86Box ATAPI profiles fall back to direct IDE packet reads.
...
- Provides vendor-gated `make test-sammax-cd-files`, `make test-sammax-cd-start`, `make test-sammax-cd-setmuse`, `make test-sammax-cd-setmuse-save`, `make test-sammax-cd-install`, and `make test-sammax-cd-install-select` smokes for the Sam & Max Hit the Road CD data track from its cue/bin archive.
