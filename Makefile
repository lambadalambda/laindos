...
.PHONY: all clean run site check-docs-sync test test-serial bench-disk-write bench-io-hot-paths bench-fat16-alloc bench-metadata bench-cd-cache installer test-installer monkey-demo nightly-package run-monkey-demo test-monkey-demo test-attached-hd-shell test-shell-batch-builtins extras-hd run-extras-hd test-cd-bios test-cd-file test-cd-subdir test-cd-find test-cd-mscdex test-cd-audio test-cd-chunks test-cd-share test-boot-mem test-cd-exec test-cd-media-swap test-cd-86box test-sammax-cd-files test-sammax-cd-start test-sammax-cd-setmuse test-sammax-cd-setmuse-save test-sammax-cd-install test-sammax-cd-install-select test-sammax-cd-dig test-normality-install test-monkey-full test-mi2-save test-wolf3d-smoke test-ascendancy-smoke test-norton-commander-smoke test-norton-commander-launch test-norton-commander-copy test-norton-commander-rename-delete test-norton-commander-mkdir-rmdir test-norton-commander test-shortline-smoke test-stunt-island-smoke test-civ-smoke test-civ-86box test-simon-smoke test-mm2-smoke test-wc-smoke test-settlers2-smoke test-game-smokes
...
bench-disk-write:
	$(PYTHON) scripts/bench_disk_write.py
...
bench-metadata:
	$(PYTHON) scripts/bench_metadata.py

bench-cd-cache:
	$(PYTHON) scripts/bench_cd_cache.py

installer:
	$(PYTHON) scripts/build_installer.py

test-installer:
	$(RUN_TEST) $(PYTHON) scripts/test_installer.py
...
test-cd-audio:
	$(RUN_TEST) $(PYTHON) scripts/test_cd_audio.py

test-cd-chunks:
	$(RUN_TEST) $(PYTHON) scripts/test_cd_chunks.py

test-boot-mem:
	$(RUN_TEST) $(PYTHON) scripts/test_boot_mem.py

test-cd-share:
	$(RUN_TEST) $(PYTHON) scripts/test_cd_share.py
...
test-sammax-cd-start: vendor/Bestseller\ Games\ Gold\ 3\ -\ Sam\ &\ Max\ Hit\ the\ Road.zip
	$(RUN_TEST) $(PYTHON) scripts/test_sammax_cd_start.py

test-sammax-cd-setmuse: vendor/Bestseller\ Games\ Gold\ 3\ -\ Sam\ &\ Max\ Hit\ the\ Road.zip
	$(RUN_TEST) $(PYTHON) scripts/test_sammax_cd_setmuse.py

test-sammax-cd-setmuse-save: vendor/Bestseller\ Games\ Gold\ 3\ -\ Sam\ &\ Max\ Hit\ the\ Road.zip
	$(RUN_TEST) $(PYTHON) scripts/test_sammax_cd_setmuse_save.py

test-sammax-cd-install: vendor/Bestseller\ Games\ Gold\ 3\ -\ Sam\ &\ Max\ Hit\ the\ Road.zip
	$(RUN_TEST) $(PYTHON) scripts/test_sammax_cd_install.py

test-sammax-cd-install-select: vendor/Bestseller\ Games\ Gold\ 3\ -\ Sam\ &\ Max\ Hit\ the\ Road.zip
	$(RUN_TEST) $(PYTHON) scripts/test_sammax_cd_install_select.py

test-sammax-cd-dig: vendor/Bestseller\ Games\ Gold\ 3\ -\ Sam\ &\ Max\ Hit\ the\ Road.zip
	$(RUN_TEST) $(PYTHON) scripts/test_sammax_cd_dig.py

test-normality-install: vendor/Bestseller\ Games\ Gold\ 3\ -\ Sam\ &\ Max\ Hit\ the\ Road.zip
	$(RUN_TEST) $(PYTHON) scripts/test_normality_install.py
...
test-stunt-island-smoke: vendor/002514_stunt_island.7z
	$(RUN_TEST) $(PYTHON) scripts/test_stunt_island_smoke.py

test-civ-smoke: vendor/sid-meiers-civilization-au.zip
	$(RUN_TEST) $(PYTHON) scripts/test_civ_smoke.py

test-civ-86box: vendor/sid-meiers-civilization-au.zip
	$(RUN_TEST) $(PYTHON) scripts/test_civ_86box.py

test-simon-smoke: vendor/simon1demo.zip
	$(RUN_TEST) $(PYTHON) scripts/test_simon_smoke.py
