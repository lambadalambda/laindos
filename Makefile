...
# Re-check HEAD every build; the helper preserves mtime unless bytes change.
$(SHELLCOM): $(PROGRAMDIR)/shell.asm scripts/build_shell_com.py FORCE
	$(PYTHON) scripts/build_shell_com.py $@
...
test-site: site
	sh scripts/run_site_playwright_container.sh
...
bench-read-paths:
	$(PYTHON) scripts/bench_read_paths.py
