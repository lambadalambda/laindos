...
- Added per-test build directory support and `scripts/run_tests.py` so default QEMU tests can run in parallel without racing on shared `build/*.bin` and `build/*.com` artifacts.
- Added shared headless smoke helpers in `scripts/testlib.py` for QEMU monitor sockets, monitor key/text input, `screendump`, framebuffer activity checks, command execution, and serial marker checks. `scripts/test_monkey_full.py`, `scripts/test_wolf3d_smoke.py`, and `scripts/test_ascendancy_smoke.py` use these helpers; broader image-building consolidation remains open.
- Acceptance criteria are satisfied; remaining broader consolidation is tracked by `continue-qemu-test-harness-consolidation.md`.
