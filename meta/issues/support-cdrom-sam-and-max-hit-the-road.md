...
- Focused generated-ISO smokes now cover BIOS CD reads, read-only `D:` file open/read, drive selection, root `FindFirst`/`FindNext`, and MSCDEX install/drive/version probes; the issue remains open for vendor cue/bin setup/startup coverage and any deeper MSCDEX device-driver calls the target requires.
