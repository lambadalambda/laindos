...
- Focused generated-ISO smokes cover BIOS CD reads, read-only `D:` file open/read/`EXEC`/overlay load through subdirectories, drive selection, root/subdirectory/current-directory `FindFirst`/`FindNext`, and MSCDEX install/drive/version probes. Vendor-gated smokes normalize the Sam & Max MODE1/2352 cue/bin data track, read `D:\SAMNMAX` files, start root `D:\INSTALL.EXE` under QEMU `-icount shift=6`, start `D:\SAMNMAX\SAMNMAX.EXE`, verify the SETMUSE sound-card list, save SETMUSE configuration to `C:\SAMNMAX.CD\SETMUSE.INI`, pass the sound-driver prompt, and verify an active framebuffer.
