...
- Focused generated-ISO smokes now cover BIOS CD reads, read-only `D:` file open/read through subdirectories, drive selection, root `FindFirst`/`FindNext`, and MSCDEX install/drive/version probes. A vendor-gated smoke now normalizes the Sam & Max MODE1/2352 cue/bin data track and reads `D:\SAMNMAX` files; the issue remains open for installer/startup coverage and any deeper MSCDEX device-driver calls the target requires.
