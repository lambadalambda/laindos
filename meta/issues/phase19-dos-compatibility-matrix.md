...
| `AH=60h` truename | implemented/partial | medium | `COMPATAPI` covers drive/current-directory canonicalization, slash normalization, dot components, and dot-dot components for 8.3 paths. |
| `AX=5D06h` DOS swappable data area pointer | implemented/stub | medium | `COMPATAPI` covers clear-carry success, unsupported-subfunction failure, nonzero swap sizes, zero critical/InDOS flags, and current PSP in the minimal SDA header. Full internal SDA layout remains deferred. |
...
| `AH=71h` Windows long-filename family | implemented/stub | medium | Returns carry set with `AX=7100h` for unsupported LFN APIs so DJGPP/SMMU-style callers fall back to 8.3 FindFirst/FindNext paths; full LFN semantics remain deferred. |
