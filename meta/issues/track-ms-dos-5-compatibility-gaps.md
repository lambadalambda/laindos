...
| --- | --- | --- | --- | --- |
| Full `COMMAND.COM` behavior | Small target-driven shell | Missing `SET`, prompt expansion, aliases, pipes, general redirection, `CALL`, `FOR`, and environment-variable expansion. | Split only for a failing installer or real batch script; otherwise churn. | `docs/site/page_shell.jsx:42-47` |
...
| Extended error and low-level disk structs | Unknown calls return function error; no `AH=59h`, no broad DPB/media-ID surface. | Missing richer error and drive-internal query behavior. | Possible targeted utility/runtime fix if observed. | `src/kernel/int21.inc:516-542` |
...
| Extended open/create | No `AH=6Ch`; unsupported functions fall through to the unhandled-call path. | Missing DOS 4+/5 extended open/create. | Potentially useful small API if a setup/runtime probes it. | `src/kernel/int21.inc:510-524` |
