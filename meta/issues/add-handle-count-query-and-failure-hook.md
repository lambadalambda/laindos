...
- Completed on 2026-06-07 with test-build `INT 21h AH=F0h AL=00h` returning the current `H_USED` handle count in `AX`, a one-shot `TEST_FLUSH_DIR_SLOT_FAIL` hook in `flush_dir_slot`, and `scripts/test_handleleak.py` / `tests/programs/handleleak.asm` covering the post-`alloc_handle` failed-create path plus a subsequent successful create/open.
