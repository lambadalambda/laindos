...
- Loads `.COM` and MZ `.EXE` programs with PSP setup, relocation, terminate vectors, per-child EXEC resume state, environment blocks, and MCB allocation; the arena ends at the BIOS INT 12h conventional-memory line (the EBDA stays with the BIOS) and the default allocation strategy is plain DOS first fit. A `.COM` owns the largest free block with SP entering at 0xFFFE and the zero word at [SP], exactly as real DOS loads them — `.COM` images carry no BSS, and era programs freely use the room past their file image. `LOADFIX.COM` stays bundled for EXEPACK-era binaries such as Civilization that need to start above segment 1000h on lean DOS layouts.
- Provides a small shell with `AUTOEXEC.BAT`, current directory support, environment/PATH/BLASTER handling, and parent/child `EXEC` coverage including inherited child PSP handle tables; large COPY/TYPE/MORE buffers are transient so shell-launched games still clear a 580 KiB conventional-memory gate.
...
- Minimal XMS detection, query, single-block allocation, handle-release behavior, XMS block moves, and backed EMS with multiple handles.
...
- Full XMS multi-handle/reallocation/HMA behavior, full named-handle/alternate-map EMS behavior, UMB/HMA, load-high behavior, SHARE, redirectors, printing, or networking.
