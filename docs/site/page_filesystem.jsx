...
const FS_SECTIONS = [
  ...
  },
  {
    ...
    body: [
      "Reads convert the current file position to a cluster index and sector-in-cluster, reuse the handle's last-cluster cache when possible, and direct-read aligned full-sector requests of at least two sectors. Smaller reads still cache the sector in `READ_CACHE_BUF` until another read needs a different LBA. If the read targets the dirty write-back sector, LainDOS flushes it and fills `READ_CACHE_BUF` from `WRITE_CACHE_BUF` rather than rereading stale media data.",
    ...
    code: [
      [1710, "    mov ax, si"],
      [3514, "    call cluster_lba"],
      [3142, "    call read_sectors"],
      [3161, "    cmp byte [cs:rf_cache_valid], 1"],
      [3560, "    call wf_mark_sector_cache"],
      [3181, "    mov dx, READ_CACHE_BUF"],
      [3533, "    call read_sector"],
      [3187, "    mov byte [cs:rf_cache_valid], 1"],
      [3213, "    mov dx, READ_CACHE_BUF"],
      [3407, ".wf_file:"],
      [3426, "    call activate_drive_for_handle"],
      [2312, "    call fat_alloc_cluster"],
      [3509, "    call wf_get_cluster"],
      [3517, "    call wf_prepare_sector_cache"],
      [3551, "    mov ax, WRITE_CACHE_BUF"],
      [3533, "    call read_sector"],
      [3560, "    call wf_mark_sector_cache"],
      [3495, "    call fat_alloc_cluster"],
      [3509, "    call wf_get_cluster"],
      [3514, "    call cluster_lba"],
      [3522, "    mov dx, WRITE_CACHE_BUF"],
      [3533, "    call read_sector"],
      [3551, "    mov ax, WRITE_CACHE_BUF"],
      [3563, "    call wf_flush_sector_cache"],
      [3560, "    call wf_mark_sector_cache"],
      [3661, "    mov ax, [cs:wf_written]"],
    ],
    hi: [3142, 3161, 3187, 3213, 3509, 3517, 3560],
  ...
  },
  {
    ...
    code: [
      ...
      [3727, ".delete_file:"],
      [3740, "    call parse_root_path"],
      [3744, "    call find_in_dir"],
      [2421, "    mov byte [es:di], 0xE5"],
      [4132, "    call flush_dir_slot"],
      [3760, "    call fat_free_chain"],
      [3761, "    call flush_fat"],
      [4957, ".rename_file:"],
      [4973, "    call parse_root_path"],
      [4979, "    call find_in_dir"],
      [4985, "    call entry_has_open_handle"],
      [5001, "    cmp ax, [cs:rn_dir_cluster]"],
      [5015, "    mov si, name_buf"],
      [5020, "    call flush_dir_slot"],
      [5087, ".commit_file:"],
      [5117, "    call wf_flush_handle_dir_entry"],
      [5119, "    call flush_fat"],
    ],
    hi: [2450, 2546, 3727, 3761, 4957, 5087],
