...
const FS_SECTIONS = [
  ...
  },
  {
    ...
    code: [
      ...
      [9, "    mov cl, [cs:kspc]"],
      [13, "    add ax, [cs:kdsta]"],
      [25, "read_sector: PERF_INC perf_sector_reads"],
      [26, "    call wf_flush_sector_cache"],
      [34, "write_sector: PERF_INC perf_sector_writes"],
      [35, "    mov byte [cs:rf_cache_valid], 0"],
      [93, "setup_sector_io:"],
      [98, "    mov ax, [cs:kpart_lba]"],
      [106, "setup_bios_chs:"],
      [113, "    cmp dx, [cs:kbio_spt]"],
      [115, "    div word [cs:kbio_spt]"],
      [120, "    cmp ax, 1024"],
      [139, "finish_sector_io:"],
      [142, "    mov ax, es"],
      [143, "    add ax, 0x1000"],
    ],
    hi: [1, 5, 13, 35, 98, 120, 143],
  ...
  },
  {
    ...
    code: [
      ...
      [75, "fat16_next:"],
      [87, "    call fat16_window_load          ; ax = FAT-relative sector"],
      [110, "fat16_window_load:"],
      [153, "fat16_window_flush:"],
      [156, "    cmp byte [cs:fat16_cache_dirty], 1"],
      [180, "    call write_sector"],
      [228, "fat_set:"],
      [280, "fat16_set:"],
      [293, "    call fat16_window_load          ; ax = FAT-relative sector"],
      [300, "    mov [es:bx], ax"],
      [301, "    mov byte [cs:fat16_cache_dirty], 1"],
      [313, "fat_alloc_cluster: PERF_INC perf_fat_allocs"],
      [362, "fat_free_chain:"],
      [392, "flush_fat: PERF_INC perf_fat_flushes"],
      [395, "    cmp byte [cs:kfat_bits], 16"],
      [443, "    call write_sector"],
    ],
    hi: [75, 87, 110, 153, 280, 301, 392],
  ...
  },
  {
    ...
    body: [
      "Reads convert the current file position to a cluster index and sector-in-cluster, reuse the handle's last-cluster cache when possible, then cache the sector in `READ_CACHE_BUF` until another read needs a different LBA. If the read targets the dirty write-back sector, LainDOS flushes it and fills `READ_CACHE_BUF` from `WRITE_CACHE_BUF` rather than rereading stale media data.",
    ...
    code: [
      [1710, "    mov ax, si"],
      [3439, "    call cluster_lba"],
      [3086, "    cmp byte [cs:rf_cache_valid], 1"],
      [3485, "    call wf_mark_sector_cache"],
      [3138, "    mov dx, READ_CACHE_BUF"],
      [3458, "    call read_sector"],
      [3112, "    mov byte [cs:rf_cache_valid], 1"],
      [3138, "    mov dx, READ_CACHE_BUF"],
      [3332, ".wf_file:"],
      [3351, "    call activate_drive_for_handle"],
      [2312, "    call fat_alloc_cluster"],
      [3434, "    call wf_get_cluster"],
      [3442, "    call wf_prepare_sector_cache"],
      [3476, "    mov ax, WRITE_CACHE_BUF"],
      [3458, "    call read_sector"],
      [3485, "    call wf_mark_sector_cache"],
      [3420, "    call fat_alloc_cluster"],
      [3434, "    call wf_get_cluster"],
      [3439, "    call cluster_lba"],
      [3447, "    mov dx, WRITE_CACHE_BUF"],
      [3458, "    call read_sector"],
      [3476, "    mov ax, WRITE_CACHE_BUF"],
      [3488, "    call wf_flush_sector_cache"],
      [3485, "    call wf_mark_sector_cache"],
      [3586, "    mov ax, [cs:wf_written]"],
    ],
    hi: [3086, 3112, 3138, 3434, 3442, 3485],
  ...
  },
  {
    ...
    code: [
      ...
      [3652, ".delete_file:"],
      [3665, "    call parse_root_path"],
      [3669, "    call find_in_dir"],
      [2421, "    mov byte [es:di], 0xE5"],
      [4057, "    call flush_dir_slot"],
      [3685, "    call fat_free_chain"],
      [3686, "    call flush_fat"],
      [4882, ".rename_file:"],
      [4921, "    call parse_root_path"],
      [4930, "    call find_in_dir"],
      [4910, "    call entry_has_open_handle"],
      [4926, "    cmp ax, [cs:rn_dir_cluster]"],
      [4940, "    mov si, name_buf"],
      [4945, "    call flush_dir_slot"],
      [5012, ".commit_file:"],
      [5042, "    call wf_flush_handle_dir_entry"],
      [5044, "    call flush_fat"],
    ],
    hi: [2450, 2546, 3652, 3686, 4882, 5012],
