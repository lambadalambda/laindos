...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3497, "%if LOAD_SEG <= RELOC_SEG"],
      [3498, "%error \"LOAD_SEG must be above RELOC_SEG\""],
      [3500, "%if (kernel_end - kernel_entry) > ((LOAD_SEG - RELOC_SEG) * 16)"],
      [3501, "%error \"kernel exceeds boot relocation gap\""],
      [3503, "%if (kernel_end - kernel_entry) > ((SEC_BUF - RELOC_SEG) * 16)"],
      [3504, "%error \"kernel overlaps SEC_BUF\""],
      [3506, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3507, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3509, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3510, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3512, "%if (ROOT_SEG + ROOT_BUF_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3513, "%error \"ROOT buffer overlaps kernel stack\""],
      [3515, "%if (ROOT_SEG + ROOT_BUF_PARAS + STACK_ROOT_GUARD_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3516, "%error \"ROOT buffer leaves too little kernel stack guard\""],
      [3524, "%if (RELOC_SEG + (KERNEL_STACK_TOP / 16)) > MCB_START"],
      [3525, "%error \"kernel stack overlaps MCB arena\""],
      [3527, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3528, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3530, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3531, "%error \"EMS frame must be inside conventional arena\""],
      [3542, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3543, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
  ...
  },
  {
    ...
    code: [
      ...
      [1593, "    mov si, es"],
      [1594, "    dec si"],
      [1460, "    cmp byte [ds:0], MCB_SIG_M"],
      [1477, "    cmp byte [ds:0], MCB_SIG_Z"],
      ...
      [1501, "    mov byte [ds:0], MCB_SIG_M"],
      [1381, "    mov word [es:3], cx"],
      [1663, "    mov word [ds:3], bx"],
      [1648, "    mov ax, [ds:3]"],
      [1658, "    mov byte [es:0], dl"],
