...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3744, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3745, "%error \"kernel leaves too little HMA stack guard\""],
      [3747, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3748, "%error \"kernel exceeds boot load area\""],
      [3750, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3751, "%error \"FAT buffer overlaps CD_BUF\""],
      [3753, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3754, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3756, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3757, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3759, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3760, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3762, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3763, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3765, "%if MCB_START >= MEM_TOP"],
      [3766, "%error \"MCB arena is empty\""],
      [3768, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3769, "%error \"EMS frame must be inside conventional arena\""],
      [3780, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
