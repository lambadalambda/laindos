...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    code: [
      ...
      [192, "    mov ax, MCB_START"],
      [155, "    mov es, ax"],
      [194, "    mov byte [es:0], MCB_SIG_Z"],
      [195, "    mov word [es:1], 0"],
      [196, "    mov ax, MEM_TOP - MCB_START - 1"],
      [197, "    mov word [es:3], ax"],
      [198, "    mov word [mcb_first], MCB_START"],
      [199, "    mov word [cur_psp], 0"]],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3637, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3638, "%error \"kernel leaves too little HMA stack guard\""],
      [3640, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3641, "%error \"kernel exceeds boot load area\""],
      [3643, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3644, "%error \"FAT buffer overlaps CD_BUF\""],
      [3646, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3647, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3649, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3650, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3652, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3653, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3655, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3656, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3658, "%if MCB_START >= MEM_TOP"],
      [3659, "%error \"MCB arena is empty\""],
      [3661, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3662, "%error \"EMS frame must be inside conventional arena\""],
      [3673, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3674, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
  ...
  },
  {
    ...
    code: [
      [2356, "do_terminate:"],
      [2365, "%if ENABLE_EMS"],
      [2252, "    mov word [cs:ems_alloc_pages], 0"],
      [2369, "    mov word [cs:xms_alloc_kb], 0"],
      [2370, "    call release_inherited_handles"],
      [2371, "    call close_owned_handles"],
      [2373, "    mov si, [cs:mcb_first]"],
      [2375, "    mov ds, si"],
      [2382, "    mov ax, [cs:cur_psp]"],
      [2383, "    cmp word [ds:1], ax"],
      [2385, "    mov word [ds:1], 0"],
      [2389, "    call mcb_walk_next"],
      [2393, "    call mcb_coalesce_all_free"],
      [2398, "    mov ax, [0x16]"],
      [2399, "    mov [cs:cur_psp], ax"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1736, "init_xms_size:"],
      [1738, "    mov word [cs:xms_total_kb], 0"],
      [1739, "    mov ah, 0x88"],
      [1740, "    int 0x15"],
      [1745, "    cmp ax, XMS_MAX_KB - 64"],
      [1747, "    mov ax, XMS_MAX_KB - 64"],
      [1749, "    mov [cs:xms_total_kb], ax"],
      [1771, "int2f_handler:"],
      [1781, "    cmp ax, 0x4300"],
      [1783, "    cmp ax, 0x4310"],
      [1788, "    mov al, 0x80"],
      [1791, "    mov bx, xms_entry"],
      [1801, "xms_entry:"],
      [1818, "    cmp ah, 0x08"],
      [1820, "    cmp ah, 0x09"],
      [1822, "    cmp ah, 0x0A"],
      [1824, "    cmp ah, 0x0B"],
      [1861, "    mov ax, [cs:xms_total_kb]"],
      [1885, "    cmp word [cs:xms_alloc_kb], 0"],
      [1871, "    cmp dx, [cs:xms_total_kb]"],
      [1873, "    mov [cs:xms_alloc_kb], dx"],
      [1888, "    mov ax, 1"],
      [1875, "    mov dx, 1"],
      [1887, "    mov word [cs:xms_alloc_kb], 0"],
      [1891, ".move:"],
      [1900, "    test ax, 1"],
      [1908, "    call xms_prepare_endpoint"],
      [1908, "    call xms_prepare_endpoint"],
      [1962, "    mov ax, 0x8700"],
      [1967, "    int 0x15"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1602, "%if ENABLE_EMS"],
      [1603, "    mov [es:0x67*4], word int67_handler"],
      [1605, "%else"],
      [1606, "    mov [es:0x67*4], word int67_absent_handler"],
      [2118, "%if !ENABLE_EMS"],
      [2119, "int67_absent_handler:"],
      [2120, "    mov ah, 0x80"],
      [2124, "%if ENABLE_EMS"],
      [2125, "int67_handler:"],
      [2149, ".frame:"],
      [2151, "    mov bx, EMS_FRAME_SEG"],
      [2153, ".pages:"],
      [2154, "    mov bx, EMS_TOTAL_PAGES"],
      [2159, ".alloc:"],
      [2164, "    cmp bx, EMS_TOTAL_PAGES"],
      [2166, "    mov [cs:ems_alloc_pages], bx"],
      [2174, ".map:"],
      [2179, "    cmp al, 3"],
      [2181, "    cmp bx, [cs:ems_alloc_pages]"],
      [2206, "    call ems_copy_16k"],
      [2206, "    call ems_copy_16k"],
      [2224, "    mov [cs:si+ems_map_pages], bx"],
      [2252, "    mov word [cs:ems_alloc_pages], 0"],
      [2276, "ems_clear_map:"],
      [2277, "    mov word [cs:ems_map_pages], 0xFFFF"]],
