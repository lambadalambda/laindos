...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3406, "%if LOAD_SEG <= RELOC_SEG"],
      [3407, "%error \"LOAD_SEG must be above RELOC_SEG\""],
      [3409, "%if (kernel_end - kernel_entry) > ((LOAD_SEG - RELOC_SEG) * 16)"],
      [3410, "%error \"kernel exceeds boot relocation gap\""],
      [3412, "%if (kernel_end - kernel_entry) > ((SEC_BUF - RELOC_SEG) * 16)"],
      [3413, "%error \"kernel overlaps SEC_BUF\""],
      [3415, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3416, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3418, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3419, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3421, "%if (ROOT_SEG + ROOT_BUF_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3422, "%error \"ROOT buffer overlaps kernel stack\""],
      [3424, "%if (ROOT_SEG + ROOT_BUF_PARAS + STACK_ROOT_GUARD_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3425, "%error \"ROOT buffer leaves too little kernel stack guard\""],
      [3433, "%if (RELOC_SEG + (KERNEL_STACK_TOP / 16)) > MCB_START"],
      [3434, "%error \"kernel stack overlaps MCB arena\""],
      [3436, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3437, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3439, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3440, "%error \"EMS frame must be inside conventional arena\""],
      [3451, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3452, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
  ...
  },
  {
    ...
    code: [
      [2209, "do_terminate:"],
      [2218, "%if ENABLE_EMS"],
      [2219, "    mov word [cs:ems_alloc_pages], 0"],
      [2222, "    mov word [cs:xms_alloc_kb], 0"],
      [2223, "    call release_inherited_handles"],
      [2224, "    call close_owned_handles"],
      [2226, "    mov si, [cs:mcb_first]"],
      [2228, "    mov ds, si"],
      [2235, "    mov ax, [cs:cur_psp]"],
      [2236, "    cmp word [ds:1], ax"],
      [2238, "    mov word [ds:1], 0"],
      [2242, "    call mcb_walk_next"],
      [2246, "    call mcb_coalesce_all_free"],
      [2251, "    mov ax, [0x16]"],
      [2252, "    mov [cs:cur_psp], ax"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1648, "init_xms_size:"],
      [1650, "    mov word [cs:xms_total_kb], 0"],
      [1651, "    mov ah, 0x88"],
      [1652, "    int 0x15"],
      [1656, "    cmp ax, XMS_MAX_KB"],
      [1658, "    mov ax, XMS_MAX_KB"],
      [1660, "    mov [cs:xms_total_kb], ax"],
      [1666, "int2f_handler:"],
      [1668, "    cmp ax, 0x4300"],
      [1670, "    cmp ax, 0x4310"],
      [1675, "    mov al, 0x80"],
      [1678, "    mov bx, xms_entry"],
      [1688, "xms_entry:"],
      [1691, "    cmp ah, 0x08"],
      [1693, "    cmp ah, 0x09"],
      [1695, "    cmp ah, 0x0A"],
      [1697, "    cmp ah, 0x0B"],
      [1714, "    mov ax, [cs:xms_total_kb]"],
      [1720, "    cmp word [cs:xms_alloc_kb], 0"],
      [1724, "    cmp dx, [cs:xms_total_kb]"],
      [1726, "    mov [cs:xms_alloc_kb], dx"],
      [1727, "    mov ax, 1"],
      [1728, "    mov dx, 1"],
      [1740, "    mov word [cs:xms_alloc_kb], 0"],
      [1744, ".move:"],
      [1753, "    test ax, 1"],
      [1761, "    call xms_prepare_endpoint"],
      [1761, "    call xms_prepare_endpoint"],
      [1815, "    mov ax, 0x8700"],
      [1820, "    int 0x15"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1514, "%if ENABLE_EMS"],
      [1515, "    mov [es:0x67*4], word int67_handler"],
      [1517, "%else"],
      [1518, "    mov [es:0x67*4], word int67_absent_handler"],
      [1971, "%if !ENABLE_EMS"],
      [1972, "int67_absent_handler:"],
      [1973, "    mov ah, 0x80"],
      [1977, "%if ENABLE_EMS"],
      [1978, "int67_handler:"],
      [2002, ".frame:"],
      [2004, "    mov bx, EMS_FRAME_SEG"],
      [2006, ".pages:"],
      [2007, "    mov bx, EMS_TOTAL_PAGES"],
      [2012, ".alloc:"],
      [2017, "    cmp bx, EMS_TOTAL_PAGES"],
      [2019, "    mov [cs:ems_alloc_pages], bx"],
      [2027, ".map:"],
      [2032, "    cmp al, 3"],
      [2034, "    cmp bx, [cs:ems_alloc_pages]"],
      [2059, "    call ems_copy_16k"],
      [2059, "    call ems_copy_16k"],
      [2077, "    mov [cs:si+ems_map_pages], bx"],
      [2105, "    mov word [cs:ems_alloc_pages], 0"],
      [2129, "ems_clear_map:"],
      [2130, "    mov word [cs:ems_map_pages], 0xFFFF"]],
