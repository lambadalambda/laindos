const MEM_FLOW = [
  ["Layout", "The kernel relocates to 0280h, keeps scratch buffers below 1000h, and reserves A000h for VGA."],
...
const MEM_REGIONS = [
  ...
  ["0280:0000", "The relocated kernel and stack live here; compile-time guards keep them below buffers and MCB_START."],
...
const MEM_SECTIONS = [
  {
    ...
    body: [
      "`src/memory.inc` is the first file to read before moving buffers. These equates decide where the kernel relocates, where CD and FAT scratch sectors live, where the DOS arena begins, and where conventional memory ends.",
    ...
    code: [
      ...
      [4, "RELOC_SEG equ 0x0280"],
      [5, "SECTOR_BUF_PARAS equ 0x20"],
      [6, "CD_BUF_PARAS equ 0x80"],
      [7, "CD_BUF equ 0x0200"],
      [8, "SEC_BUF equ 0x0B00"],
      [9, "READ_CACHE_BUF equ (SEC_BUF + SECTOR_BUF_PARAS)"],
      [10, "MCB_START equ 0x1000"],
      [11, "MEM_TOP equ 0xA000"],
      [12, "ENV_PARAS equ 16"],
      [13, "ENV_SIZE_BYTES equ (ENV_PARAS * 16)"],
      [14, "ENV_OWNER_TEMP equ 0xFFFF"],
      [15, "%if (MEM_TOP & 0xFF) != 0"],
      [16, "%error \"MEM_TOP must be 256-byte aligned\""],
      [18, "MCB_SIG_M equ 'M'"],
      [19, "MCB_SIG_Z equ 'Z'"]],
  ...
  },
  {
    ...
    code: [
      [118, "kernel_entry:"],
      [134, "    mov ax, RELOC_SEG"],
      [141, "    jmp RELOC_SEG:.relocated"],
      [142, ".relocated:"],
      [146, "    mov ss, ax"],
      [147, "    mov sp, KERNEL_STACK_TOP"],
      [158, "    int 0x12"],
      [159, "    mov [mem_kib], ax"],
      [167, "%if ENABLE_XMS"],
      [168, "    call init_xms_size"],
      [182, "    mov ax, MCB_START"],
      [145, "    mov es, ax"],
      [184, "    mov byte [es:0], MCB_SIG_Z"],
      [185, "    mov word [es:1], 0"],
      [186, "    mov ax, MEM_TOP - MCB_START - 1"],
      [187, "    mov word [es:3], ax"],
      [188, "    mov word [mcb_first], MCB_START"],
      [189, "    mov word [cur_psp], 0"]],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3482, "%if LOAD_SEG <= RELOC_SEG"],
      [3483, "%error \"LOAD_SEG must be above RELOC_SEG\""],
      [3485, "%if (kernel_end - kernel_entry) > ((LOAD_SEG - RELOC_SEG) * 16)"],
      [3486, "%error \"kernel exceeds boot relocation gap\""],
      [3488, "%if (kernel_end - kernel_entry) > ((SEC_BUF - RELOC_SEG) * 16)"],
      [3489, "%error \"kernel overlaps SEC_BUF\""],
      [3491, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3492, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3494, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3495, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3497, "%if (ROOT_SEG + ROOT_BUF_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3498, "%error \"ROOT buffer overlaps kernel stack\""],
      [3500, "%if (ROOT_SEG + ROOT_BUF_PARAS + STACK_ROOT_GUARD_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3501, "%error \"ROOT buffer leaves too little kernel stack guard\""],
      [3509, "%if (RELOC_SEG + (KERNEL_STACK_TOP / 16)) > MCB_START"],
      [3510, "%error \"kernel stack overlaps MCB arena\""],
      [3512, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3513, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3515, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3516, "%error \"EMS frame must be inside conventional arena\""],
      [3527, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3528, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
  ...
  },
  {
    ...
    code: [
      ...
      [1270, "    je .as_set"],
      [622, "    xor ax, ax"],
      [1274, "    mov al, [cs:alloc_strat]"],
      [1277, "    cmp bl, 2"],
      [1279, "    mov [cs:alloc_strat], bl"],
      [1316, ".alloc_mem:"],
      [1325, "    mov al, [cs:alloc_strat]"],
      [1004, "    cmp al, 2"],
      [1329, "    jmp near .am_find_last"],
      [1331, "    cmp al, 1"],
      [1333, "    jmp near .am_find_best"],
      [1335, "    cmp word [cs:am_req], 1"],
      [1337, "    cmp word [cs:am_req], SMALL_ALLOC_HIGH_MAX"],
      [1329, "    jmp near .am_find_last"],
      [1341, "    mov si, [cs:mcb_first]"],
      [1354, "    cmp ax, [cs:am_req]"],
      [1356, ".am_use:"],
      [979, "    mov ax, [cs:cur_psp]"],
      [1422, ".am_find_last:"],
      [1446, ".am_find_best:"],
      [1503, ".am_nomem:"],
      [1506, ".am_scan_largest:"],
      [1527, "    mov ax, 8"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1559, "    mov si, es"],
      [1560, "    dec si"],
      [1344, "    cmp byte [ds:0], MCB_SIG_M"],
      [1346, "    cmp byte [ds:0], MCB_SIG_Z"],
      [1569, "    mov word [ds:1], 0"],
      [1570, "    call mcb_merge_free_forward"],
      [1577, ".resize_mem:"],
      [1584, "    mov [cs:rm_req], bx"],
      [1585, "    mov si, es"],
      [1353, "    mov ax, [ds:3]"],
      [1597, "    jae .rm_shrink"],
      [1605, "    cmp byte [es:0], MCB_SIG_M"],
      [1610, "    cmp word [es:1], 0"],
      [1614, "    add ax, cx"],
      [1517, "    cmp ax, bx"],
      [1629, "    mov byte [es:0], dl"],
      [1374, "    mov byte [ds:0], MCB_SIG_M"],
      [1373, "    mov word [es:3], cx"],
      [1636, "    mov word [ds:3], bx"],
      [1620, "    mov ax, [ds:3]"],
      [1650, "    mov byte [es:0], dl"],
      [1369, "    mov word [es:1], 0"],
      [1636, "    mov word [ds:3], bx"],
      [1698, "    mov [es:0x02], ax"],
      [1741, ".rm_cant_grow:"],
      [1519, "    mov bx, ax"],
  ...
  },
  {
    ...
    code: [
      ...
      [457, "    mov word [cs:exec_env_seg], 0"],
      [459, "    mov bx, ENV_PARAS"],
      [460, "    call alloc_mem_direct"],
      [462, "    mov [cs:exec_env_seg], ax"],
      [465, "    dec ax"],
      [466, "    mov ds, ax"],
      ...
      [508, "assign_exec_environment_owner:"],
      [512, "    mov bx, [cs:exec_env_seg]"],
      [517, "    dec bx"],
      [522, "    mov ax, [cs:cur_psp]"],
      [523, "    mov [ds:1], ax"]],
  ...
  },
  {
    ...
    code: [
      [2215, "do_terminate:"],
      [2224, "%if ENABLE_EMS"],
      [2225, "    mov word [cs:ems_alloc_pages], 0"],
      [2228, "    mov word [cs:xms_alloc_kb], 0"],
      [2229, "    call release_inherited_handles"],
      [2230, "    call close_owned_handles"],
      [2232, "    mov si, [cs:mcb_first]"],
      [2234, "    mov ds, si"],
      [2241, "    mov ax, [cs:cur_psp]"],
      [2242, "    cmp word [ds:1], ax"],
      [2244, "    mov word [ds:1], 0"],
      [2248, "    call mcb_walk_next"],
      [2252, "    call mcb_coalesce_all_free"],
      [2257, "    mov ax, [0x16]"],
      [2258, "    mov [cs:cur_psp], ax"]],
  ...
  },
  {
    ...
    code: [
      ...
      [110, "%ifndef ENABLE_XMS"],
      [111, "%define ENABLE_XMS 1"],
      [1646, "init_xms_size:"],
      [1648, "    mov word [cs:xms_total_kb], 0"],
      [1649, "    mov ah, 0x88"],
      [1650, "    int 0x15"],
      [1654, "    cmp ax, XMS_MAX_KB"],
      [1656, "    mov ax, XMS_MAX_KB"],
      [1658, "    mov [cs:xms_total_kb], ax"],
      [1664, "int2f_handler:"],
      [1674, "    cmp ax, 0x4300"],
      [1676, "    cmp ax, 0x4310"],
      [1681, "    mov al, 0x80"],
      [1684, "    mov bx, xms_entry"],
      [1694, "xms_entry:"],
      [1697, "    cmp ah, 0x08"],
      [1699, "    cmp ah, 0x09"],
      [1701, "    cmp ah, 0x0A"],
      [1703, "    cmp ah, 0x0B"],
      [1720, "    mov ax, [cs:xms_total_kb]"],
      [1744, "    cmp word [cs:xms_alloc_kb], 0"],
      [1730, "    cmp dx, [cs:xms_total_kb]"],
      [1732, "    mov [cs:xms_alloc_kb], dx"],
      [1747, "    mov ax, 1"],
      [1734, "    mov dx, 1"],
      [1746, "    mov word [cs:xms_alloc_kb], 0"],
      [1750, ".move:"],
      [1759, "    test ax, 1"],
      [1767, "    call xms_prepare_endpoint"],
      [1767, "    call xms_prepare_endpoint"],
      [1821, "    mov ax, 0x8700"],
      [1826, "    int 0x15"]],
  ...
  },
  {
    ...
    code: [
      ...
      [114, "%ifndef ENABLE_EMS"],
      [115, "%define ENABLE_EMS 0"],
      [1512, "%if ENABLE_EMS"],
      [1513, "    mov [es:0x67*4], word int67_handler"],
      [1515, "%else"],
      [1516, "    mov [es:0x67*4], word int67_absent_handler"],
      [1977, "%if !ENABLE_EMS"],
      [1978, "int67_absent_handler:"],
      [1979, "    mov ah, 0x80"],
      [1983, "%if ENABLE_EMS"],
      [1984, "int67_handler:"],
      [2008, ".frame:"],
      [2010, "    mov bx, EMS_FRAME_SEG"],
      [2012, ".pages:"],
      [2013, "    mov bx, EMS_TOTAL_PAGES"],
      [2018, ".alloc:"],
      [2023, "    cmp bx, EMS_TOTAL_PAGES"],
      [2025, "    mov [cs:ems_alloc_pages], bx"],
      [2033, ".map:"],
      [2038, "    cmp al, 3"],
      [2040, "    cmp bx, [cs:ems_alloc_pages]"],
      [2065, "    call ems_copy_16k"],
      [2065, "    call ems_copy_16k"],
      [2083, "    mov [cs:si+ems_map_pages], bx"],
      [2111, "    mov word [cs:ems_alloc_pages], 0"],
      [2135, "ems_clear_map:"],
      [2136, "    mov word [cs:ems_map_pages], 0xFFFF"]],
