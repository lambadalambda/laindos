...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3950, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3951, "%error \"kernel leaves too little HMA stack guard\""],
      [3953, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3954, "%error \"kernel exceeds boot load area\""],
      [3956, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3957, "%error \"FAT buffer overlaps CD_BUF\""],
      [3959, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3960, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3962, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3963, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3965, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3966, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3968, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3969, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3971, "%if MCB_START >= MEM_TOP"],
      [3972, "%error \"MCB arena is empty\""],
      [3974, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3975, "%error \"EMS frame must be inside conventional arena\""],
      [3986, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [3950, 3953, 3956, 3950, 3950, 3950, 3953, 3956, 3959, 3986],
