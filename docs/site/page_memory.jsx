...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    code: [
      [120, "kernel_entry:"],
      [136, "    mov ax, RELOC_SEG"],
      [143, "    jmp RELOC_SEG:.relocated"],
      [144, ".relocated:"],
      [148, "    mov ss, ax"],
      [149, "    mov sp, KERNEL_STACK_TOP"],
      [160, "    int 0x12"],
      [161, "    mov [mem_kib], ax"],
      [169, "%if ENABLE_XMS"],
      [170, "    call init_xms_size"],
      [184, "    mov ax, MCB_START"],
      [147, "    mov es, ax"],
      [186, "    mov byte [es:0], MCB_SIG_Z"],
      [187, "    mov word [es:1], 0"],
      [188, "    mov ax, MEM_TOP - MCB_START - 1"],
      [189, "    mov word [es:3], ax"],
      [190, "    mov word [mcb_first], MCB_START"],
      [191, "    mov word [cur_psp], 0"]],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3496, "%if LOAD_SEG <= RELOC_SEG"],
      [3497, "%error \"LOAD_SEG must be above RELOC_SEG\""],
      [3499, "%if (kernel_end - kernel_entry) > ((LOAD_SEG - RELOC_SEG) * 16)"],
      [3500, "%error \"kernel exceeds boot relocation gap\""],
      [3502, "%if (kernel_end - kernel_entry) > ((SEC_BUF - RELOC_SEG) * 16)"],
      [3503, "%error \"kernel overlaps SEC_BUF\""],
      [3505, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3506, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3508, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3509, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3511, "%if (ROOT_SEG + ROOT_BUF_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3512, "%error \"ROOT buffer overlaps kernel stack\""],
      [3514, "%if (ROOT_SEG + ROOT_BUF_PARAS + STACK_ROOT_GUARD_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3515, "%error \"ROOT buffer leaves too little kernel stack guard\""],
      [3523, "%if (RELOC_SEG + (KERNEL_STACK_TOP / 16)) > MCB_START"],
      [3524, "%error \"kernel stack overlaps MCB arena\""],
      [3526, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3527, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3529, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3530, "%error \"EMS frame must be inside conventional arena\""],
      [3541, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3542, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
  ...
  },
  {
    ...
    code: [
      [2217, "do_terminate:"],
      [2226, "%if ENABLE_EMS"],
      [2227, "    mov word [cs:ems_alloc_pages], 0"],
      [2230, "    mov word [cs:xms_alloc_kb], 0"],
      [2231, "    call release_inherited_handles"],
      [2232, "    call close_owned_handles"],
      [2234, "    mov si, [cs:mcb_first]"],
      [2236, "    mov ds, si"],
      [2243, "    mov ax, [cs:cur_psp]"],
      [2244, "    cmp word [ds:1], ax"],
      [2246, "    mov word [ds:1], 0"],
      [2250, "    call mcb_walk_next"],
      [2254, "    call mcb_coalesce_all_free"],
      [2259, "    mov ax, [0x16]"],
      [2260, "    mov [cs:cur_psp], ax"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1678, "    cmp ax, 0x4310"],
      [1683, "    mov al, 0x80"],
      [1686, "    mov bx, xms_entry"],
      [1696, "xms_entry:"],
      [1699, "    cmp ah, 0x08"],
      [1701, "    cmp ah, 0x09"],
      [1703, "    cmp ah, 0x0A"],
      [1705, "    cmp ah, 0x0B"],
      [1722, "    mov ax, [cs:xms_total_kb]"],
      [1746, "    cmp word [cs:xms_alloc_kb], 0"],
      [1732, "    cmp dx, [cs:xms_total_kb]"],
      [1734, "    mov [cs:xms_alloc_kb], dx"],
      [1749, "    mov ax, 1"],
      [1736, "    mov dx, 1"],
      [1748, "    mov word [cs:xms_alloc_kb], 0"],
      [1752, ".move:"],
      [1761, "    test ax, 1"],
      [1769, "    call xms_prepare_endpoint"],
      [1769, "    call xms_prepare_endpoint"],
      [1823, "    mov ax, 0x8700"],
      [1828, "    int 0x15"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1979, "%if !ENABLE_EMS"],
      [1980, "int67_absent_handler:"],
      [1981, "    mov ah, 0x80"],
      [1985, "%if ENABLE_EMS"],
      [1986, "int67_handler:"],
      [2010, ".frame:"],
      [2012, "    mov bx, EMS_FRAME_SEG"],
      [2014, ".pages:"],
      [2015, "    mov bx, EMS_TOTAL_PAGES"],
      [2020, ".alloc:"],
      [2025, "    cmp bx, EMS_TOTAL_PAGES"],
      [2027, "    mov [cs:ems_alloc_pages], bx"],
      [2035, ".map:"],
      [2040, "    cmp al, 3"],
      [2042, "    cmp bx, [cs:ems_alloc_pages]"],
      [2067, "    call ems_copy_16k"],
      [2067, "    call ems_copy_16k"],
      [2085, "    mov [cs:si+ems_map_pages], bx"],
      [2113, "    mov word [cs:ems_alloc_pages], 0"],
      [2137, "ems_clear_map:"],
      [2138, "    mov word [cs:ems_map_pages], 0xFFFF"]],
