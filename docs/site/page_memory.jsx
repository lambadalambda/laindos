...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4303, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4304, "%error \"kernel leaves too little HMA stack guard\""],
      [4306, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4307, "%error \"kernel exceeds boot load area\""],
      [4309, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4310, "%error \"FAT buffer overlaps CD_BUF\""],
      [4312, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4313, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4315, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4316, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4318, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [4319, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4321, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4322, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4324, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4325, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4327, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4328, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4330, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4331, "%error \"CD cache overlaps MCB arena\""],
      [4333, "%if MCB_START >= MEM_TOP"],
      [4334, "%error \"MCB arena is empty\""],
      [4336, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4337, "%error \"EMS frame must be inside conventional arena\""],
      [4348, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4303, 4306, 4309, 4324, 4327, 4330, 4333, 4336, 4348],
  ...
  },
  {
    ...
    code: [
      [2693, "do_terminate:"],
      [2703, "%if ENABLE_EMS"],
      [2704, "    mov word [cs:ems_alloc_pages], 0"],
      [2707, "    mov word [cs:xms_alloc_kb], 0"],
      [2708, "    call release_inherited_handles"],
      [2709, "    call close_owned_handles"],
      [2711, "    mov si, [cs:mcb_first]"],
      [2770, "    mov ds, si"],
      [2669, "    mov ax, [cs:cur_psp]"],
      [2715, "    cmp word [ds:1], ax"],
      [2717, "    mov word [ds:1], 0"],
      [2712, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
      [2720, "    call mcb_coalesce_all_free"],
      [2725, "    mov ax, [0x16]"],
      [{a: "do_terminate_restore_parent_psp"}, "    mov [cs:cur_psp], ax"]],
    hi: [2693, 2703, 2703, 2703, 2693, 2708, {a: "do_terminate_restore_parent_psp"}],
  ...
  },
  {
    ...
    code: [
      ...
      [2037, "init_xms_size:"],
      [2039, "    mov word [cs:xms_total_kb], 0"],
      [2040, "    mov ah, 0x88"],
      [2041, "    int 0x15"],
      [2046, "    cmp ax, XMS_MAX_KB - 64"],
      [2048, "    mov ax, XMS_MAX_KB - 64"],
      [2050, "    mov [cs:xms_total_kb], ax"],
      [2072, "int2f_handler:"],
      [2084, "    cmp ax, 0x4300"],
      [2086, "    cmp ax, 0x4310"],
      [2092, "    mov al, 0x80"],
      [2095, "    mov bx, xms_entry"],
      [2108, "xms_entry:"],
      [2125, "    cmp ah, 0x08"],
      [2127, "    cmp ah, 0x09"],
      [2129, "    cmp ah, 0x0A"],
      [2131, "    cmp ah, 0x0B"],
      [2168, "    mov ax, [cs:xms_total_kb]"],
      [2174, "    cmp word [cs:xms_alloc_kb], 0"],
      [2178, "    cmp dx, [cs:xms_total_kb]"],
      [2180, "    mov [cs:xms_alloc_kb], dx"],
      [2156, "    mov ax, 1"],
      [2145, "    mov dx, 1"],
      [2194, "    mov word [cs:xms_alloc_kb], 0"],
      [2198, ".move:"],
      [2207, "    test ax, 1"],
      [2215, "    call xms_prepare_endpoint"],
      [2223, "    call xms_prepare_endpoint"],
      [2269, "    mov ax, 0x8700"],
      [{a: "xms_move_bios_call"}, "    int 0x15"]],
    hi: [43, 113, 2037, 2084, 2095, 2108, 2174, 2194, {a: "xms_move_bios_call"}],
  ...
  },
  {
    ...
    code: [
      ...
      [2433, "int67_handler:"],
      [2457, ".frame:"],
      [2459, "    mov bx, EMS_FRAME_SEG"],
      [2461, ".pages:"],
      [2462, "    mov bx, EMS_TOTAL_PAGES"],
      [2467, ".alloc:"],
      [2472, "    cmp bx, EMS_TOTAL_PAGES"],
      [2474, "    mov [cs:ems_alloc_pages], bx"],
      [2482, ".map:"],
      [2487, "    cmp al, 3"],
      [2489, "    cmp bx, [cs:ems_alloc_pages]"],
      [2514, "    call ems_copy_16k"],
      [2525, "    call ems_copy_16k"],
      [2532, "    mov [cs:si+ems_map_pages], bx"],
      [2560, "    mov word [cs:ems_alloc_pages], 0"],
      [2584, "ems_clear_map:"],
      [{a: "ems_clear_map_fill"}, "    mov word [cs:ems_map_pages], 0xFFFF"]],
    hi: [117, 1837, 2427, 2433, 2457, 2472, 2472, 2433, 2532],
