...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3343, "%if LOAD_SEG <= RELOC_SEG"],
      [3344, "%error \"LOAD_SEG must be above RELOC_SEG\""],
      [3346, "%if (kernel_end - kernel_entry) > ((LOAD_SEG - RELOC_SEG) * 16)"],
      [3347, "%error \"kernel exceeds boot relocation gap\""],
      [3349, "%if (kernel_end - kernel_entry) > ((SEC_BUF - RELOC_SEG) * 16)"],
      [3350, "%error \"kernel overlaps SEC_BUF\""],
      [3352, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3353, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3355, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3356, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3358, "%if (ROOT_SEG + ROOT_BUF_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3359, "%error \"ROOT buffer overlaps kernel stack\""],
      [3361, "%if (ROOT_SEG + ROOT_BUF_PARAS + STACK_ROOT_GUARD_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3362, "%error \"ROOT buffer leaves too little kernel stack guard\""],
      [3364, "%if (RELOC_SEG + (KERNEL_STACK_TOP / 16)) > MCB_START"],
      [3365, "%error \"kernel stack overlaps MCB arena\""],
      [3367, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3368, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3370, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3371, "%error \"EMS frame must be inside conventional arena\""],
      [3382, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3383, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
  ...
  },
  {
    ...
    code: [
      ...
      [1254, "    je .as_set"],
      [606, "    xor ax, ax"],
      [1258, "    mov al, [cs:alloc_strat]"],
      [1261, "    cmp bl, 2"],
      [1263, "    mov [cs:alloc_strat], bl"],
      [1300, ".alloc_mem:"],
      [1309, "    mov al, [cs:alloc_strat]"],
      [988, "    cmp al, 2"],
      [1313, "    jmp near .am_find_last"],
      [1315, "    cmp al, 1"],
      [1317, "    jmp near .am_find_best"],
      [1319, "    cmp word [cs:am_req], 1"],
      [1321, "    cmp word [cs:am_req], SMALL_ALLOC_HIGH_MAX"],
      [1313, "    jmp near .am_find_last"],
      [1325, "    mov si, [cs:mcb_first]"],
      [1338, "    cmp ax, [cs:am_req]"],
      [1340, ".am_use:"],
      [963, "    mov ax, [cs:cur_psp]"],
      [1406, ".am_find_last:"],
      [1430, ".am_find_best:"],
      [1487, ".am_nomem:"],
      [1490, ".am_scan_largest:"],
      [1511, "    mov ax, 8"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1543, "    mov si, es"],
      [1544, "    dec si"],
      [1328, "    cmp byte [ds:0], MCB_SIG_M"],
      [1330, "    cmp byte [ds:0], MCB_SIG_Z"],
      [1553, "    mov word [ds:1], 0"],
      [1554, "    call mcb_merge_free_forward"],
      [1561, ".resize_mem:"],
      [1568, "    mov [cs:rm_req], bx"],
      [1569, "    mov si, es"],
      [1337, "    mov ax, [ds:3]"],
      [1581, "    jae .rm_shrink"],
      [1589, "    cmp byte [es:0], MCB_SIG_M"],
      [1594, "    cmp word [es:1], 0"],
      [1598, "    add ax, cx"],
      [1501, "    cmp ax, bx"],
      [1613, "    mov byte [es:0], dl"],
      [1358, "    mov byte [ds:0], MCB_SIG_M"],
      [1357, "    mov word [es:3], cx"],
      [1620, "    mov word [ds:3], bx"],
      [1604, "    mov ax, [ds:3]"],
      [1634, "    mov byte [es:0], dl"],
      [1353, "    mov word [es:1], 0"],
      [1620, "    mov word [ds:3], bx"],
      [1682, "    mov [es:0x02], ax"],
      [1725, ".rm_cant_grow:"],
      [1503, "    mov bx, ax"],
