...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3957, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3958, "%error \"kernel leaves too little HMA stack guard\""],
      [3960, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3961, "%error \"kernel exceeds boot load area\""],
      [3963, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3964, "%error \"FAT buffer overlaps CD_BUF\""],
      [3966, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3967, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3969, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3970, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3972, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3973, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3975, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3976, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3978, "%if MCB_START >= MEM_TOP"],
      [3979, "%error \"MCB arena is empty\""],
      [3981, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3982, "%error \"EMS frame must be inside conventional arena\""],
      [3993, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [3957, 3960, 3963, 3957, 3957, 3957, 3960, 3963, 3957, 3993],
  ...
  },
  {
    ...
    code: [
      [2654, "do_terminate:"],
      [2664, "%if ENABLE_EMS"],
      [2521, "    mov word [cs:ems_alloc_pages], 0"],
      [2668, "    mov word [cs:xms_alloc_kb], 0"],
      [2669, "    call release_inherited_handles"],
      [2670, "    call close_owned_handles"],
      [2672, "    mov si, [cs:mcb_first]"],
      [2731, "    mov ds, si"],
      [2630, "    mov ax, [cs:cur_psp]"],
      [2676, "    cmp word [ds:1], ax"],
      [2678, "    mov word [ds:1], 0"],
      [2673, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
      [2681, "    call mcb_coalesce_all_free"],
      [2686, "    mov ax, [0x16]"],
      [{a: "do_terminate_restore_parent_psp"}, "    mov [cs:cur_psp], ax"]],
    hi: [2654, 2664, 2664, 2664, 2654, 2669, {a: "do_terminate_restore_parent_psp"}],
  ...
  },
  {
    ...
    code: [
      ...
      [1998, "init_xms_size:"],
      [2000, "    mov word [cs:xms_total_kb], 0"],
      [2001, "    mov ah, 0x88"],
      [2002, "    int 0x15"],
      [2007, "    cmp ax, XMS_MAX_KB - 64"],
      [2009, "    mov ax, XMS_MAX_KB - 64"],
      [2011, "    mov [cs:xms_total_kb], ax"],
      [2033, "int2f_handler:"],
      [2045, "    cmp ax, 0x4300"],
      [2047, "    cmp ax, 0x4310"],
      [2053, "    mov al, 0x80"],
      [2056, "    mov bx, xms_entry"],
      [2069, "xms_entry:"],
      [2086, "    cmp ah, 0x08"],
      [2088, "    cmp ah, 0x09"],
      [2090, "    cmp ah, 0x0A"],
      [2092, "    cmp ah, 0x0B"],
      [2129, "    mov ax, [cs:xms_total_kb]"],
      [2135, "    cmp word [cs:xms_alloc_kb], 0"],
      [2139, "    cmp dx, [cs:xms_total_kb]"],
      [2141, "    mov [cs:xms_alloc_kb], dx"],
      [2117, "    mov ax, 1"],
      [2106, "    mov dx, 1"],
      [2155, "    mov word [cs:xms_alloc_kb], 0"],
      [2159, ".move:"],
      [2168, "    test ax, 1"],
      [2176, "    call xms_prepare_endpoint"],
      [2176, "    call xms_prepare_endpoint"],
      [2230, "    mov ax, 0x8700"],
      [{a: "xms_move_bios_call"}, "    int 0x15"]],
    hi: [43, 113, 1998, 2045, 2056, 2069, 2135, 2155, {a: "xms_move_bios_call"}],
  ...
  },
  {
    ...
    code: [
      ...
      [1798, "%if ENABLE_EMS"],
      [1799, "    mov [es:0x67*4], word int67_handler"],
      [1801, "%else"],
      [1802, "    mov [es:0x67*4], word int67_absent_handler"],
      [2387, "%if !ENABLE_EMS"],
      [2388, "int67_absent_handler:"],
      [2389, "    mov ah, 0x80"],
      [2393, "%if ENABLE_EMS"],
      [2394, "int67_handler:"],
      [2418, ".frame:"],
      [2420, "    mov bx, EMS_FRAME_SEG"],
      [2422, ".pages:"],
      [2423, "    mov bx, EMS_TOTAL_PAGES"],
      [2428, ".alloc:"],
      [2433, "    cmp bx, EMS_TOTAL_PAGES"],
      [2435, "    mov [cs:ems_alloc_pages], bx"],
      [2443, ".map:"],
      [2448, "    cmp al, 3"],
      [2450, "    cmp bx, [cs:ems_alloc_pages]"],
      [2475, "    call ems_copy_16k"],
      [2475, "    call ems_copy_16k"],
      [2493, "    mov [cs:si+ems_map_pages], bx"],
      [2521, "    mov word [cs:ems_alloc_pages], 0"],
      [2545, "ems_clear_map:"],
      [{a: "ems_clear_map_fill"}, "    mov word [cs:ems_map_pages], 0xFFFF"]],
    hi: [117, 1798, 2388, 2394, 2418, 2433, 2433, 2394, 2493],
