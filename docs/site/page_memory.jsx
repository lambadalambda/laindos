...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3695, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3696, "%error \"kernel leaves too little HMA stack guard\""],
      [3698, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3699, "%error \"kernel exceeds boot load area\""],
      [3701, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3702, "%error \"FAT buffer overlaps CD_BUF\""],
      [3704, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3705, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3707, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3708, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3710, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3711, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3713, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3714, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3716, "%if MCB_START >= MEM_TOP"],
      [3717, "%error \"MCB arena is empty\""],
      [3719, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3720, "%error \"EMS frame must be inside conventional arena\""],
      [3731, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      ...
      [633, "    mov word [cs:exec_env_seg], 0"],
      [635, "    mov bx, ENV_PARAS"],
      [636, "    call alloc_mem_direct"],
      [638, "    mov [cs:exec_env_seg], ax"],
      [641, "    dec ax"],
      [642, "    mov ds, ax"],
      ...
      [684, "assign_exec_environment_owner:"],
      [688, "    mov bx, [cs:exec_env_seg]"],
      [693, "    dec bx"],
      [698, "    mov ax, [cs:cur_psp]"],
      [699, "    mov [ds:1], ax"]],
