...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4311, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4312, "%error \"kernel leaves too little HMA stack guard\""],
      [4314, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4315, "%error \"kernel exceeds boot load area\""],
      [4317, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4318, "%error \"FAT buffer overlaps CD_BUF\""],
      [4320, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4321, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4323, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4324, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4326, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [4327, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4329, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4330, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4332, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4333, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4335, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4336, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4338, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4339, "%error \"CD cache overlaps MCB arena\""],
      [4341, "%if MCB_START >= MEM_TOP"],
      [4342, "%error \"MCB arena is empty\""],
      [4344, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4345, "%error \"EMS frame must be inside conventional arena\""],
      [4356, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4311, 4314, 4317, 4332, 4335, 4338, 4341, 4344, 4356],
