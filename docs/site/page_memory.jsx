...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3949, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3950, "%error \"kernel leaves too little HMA stack guard\""],
      [3952, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3953, "%error \"kernel exceeds boot load area\""],
      [3955, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3956, "%error \"FAT buffer overlaps CD_BUF\""],
      [3958, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3959, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3961, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3962, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3964, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3965, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3967, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3968, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3970, "%if MCB_START >= MEM_TOP"],
      [3971, "%error \"MCB arena is empty\""],
      [3973, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3974, "%error \"EMS frame must be inside conventional arena\""],
      [3985, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [3949, 3952, 3955, 3950, 3949, 3949, 3952, 3955, 3958, 3985],
