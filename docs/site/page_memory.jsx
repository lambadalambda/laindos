...
const MEM_SECTIONS = [
  {
    ...
    code: [
      ...
      [26, "WRITE_CACHE_BUF equ 0x06A0"],
      [27, "CD_CACHE_BUF equ (WRITE_CACHE_BUF + SECTOR_BUF_PARAS)"],
      [28, "SUBDIR_CACHE_BUF equ (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS))"],
      [36, "MCB_START equ 0x0B00"],
      [37, "MEM_TOP equ 0xA000"],
      [38, "ENV_PARAS equ 16"],
      [39, "ENV_SIZE_BYTES equ (ENV_PARAS * 16)"],
      [40, "ENV_OWNER_TEMP equ 0xFFFF"],
      [41, "%if (MEM_TOP & 0xFF) != 0"],
      [42, "%error \"MEM_TOP must be 256-byte aligned\""],
      [44, "MCB_SIG_M equ 'M'"],
      [45, "MCB_SIG_Z equ 'Z'"]],
    hi: [4, 6, 7, 8, 9, 11, 12, 16, 17, 26, 27, 28, 36, 40, 41],
  ...
  },
  {
    ...
    code: [
      ...
      [148, "    call enable_a20"],
      [156, "    mov ax, HMA_SEG"],
      [163, "    jmp HMA_SEG:.relocated"],
      [164, ".relocated:"],
      [168, "    mov ss, ax"],
      [169, "    mov sp, KERNEL_STACK_TOP"],
      [180, "    int 0x12"],
      [181, "    mov [mem_kib], ax"],
      [189, "%if ENABLE_XMS"],
      [190, "    call init_xms_size"],
      [206, "    mov ax, MCB_START"],
      [207, "    mov es, ax"],
      [208, "    mov byte [es:0], MCB_SIG_Z"],
      [209, "    mov word [es:1], 0"],
      [212, "    mov ax, [mem_kib]"],
      [214, "    shl ax, cl"],
      [219, "    sub ax, MCB_START + 1"],
      [220, "    mov word [es:3], ax"],
      [220, "    mov word [es:3], ax"],
      [221, "    mov word [mcb_first], MCB_START"],
      [222, "    mov word [cur_psp], 0"]],
    hi: [148, 164, 156, 206, 206, 219, 219],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [5072, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [5073, "%error \"kernel leaves too little HMA stack guard\""],
      [5075, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [5076, "%error \"kernel exceeds boot load area\""],
      [5078, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [5079, "%error \"FAT buffer overlaps CD_BUF\""],
      [5081, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [5082, "%error \"CD_BUF overlaps SEC_BUF\""],
      [5084, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [5085, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [5087, "%if (READ_CACHE_BUF + READ_CACHE_PARAS) > ROOT_SEG"],
      [5088, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [5093, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [5094, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [5096, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [5097, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [5099, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [5100, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [5102, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [5103, "%error \"CD cache overlaps MCB arena\""],
      [5105, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > SUBDIR_CACHE_BUF"],
      [5106, "%error \"CD cache overlaps subdirectory cache\""],
      [5108, "%if (SUBDIR_CACHE_BUF + (SUBDIR_CACHE_SLOTS * SUBDIR_CACHE_SLOT_PARAS)) > MCB_START"],
      [5109, "%error \"subdirectory cache overlaps MCB arena\""],
      [5111, "%if MCB_START >= MEM_TOP"],
      [5112, "%error \"MCB arena is empty\""],
      [5114, "%if ENABLE_EMS && EMS_FRAME_SEG < 0xD000"],
      [5115, "%error \"EMS frame must use an upper-memory window at D000h or above\""],
      [5126, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [5072, 5072, 5072, 5072, 5072, 5072, 5072, 5072, 5072, 5075, 5078, 5081, 5084, 5087, 5102],
  ...
  },
  {
    ...
    code: [
      [3099, "do_terminate:"],
      [3110, "%if ENABLE_EMS"],
      [5048, "    mov word [cs:ems_alloc_pages], 0"],
      [3113, "    mov word [cs:xms_alloc_kb], 0"],
      [3114, "    call release_inherited_handles"],
      [3115, "    call close_owned_handles"],
      [3117, "    mov si, [cs:mcb_first]"],
      [3118, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
      [3056, "    mov ax, [cs:cur_psp]"],
      [3121, "    cmp word [ds:1], ax"],
      [3123, "    mov word [ds:1], 0"],
      [3126, "    call mcb_coalesce_all_free"],
      [3131, "    mov ax, [0x16]"],
      [{a: "do_terminate_restore_parent_psp"}, "    mov [cs:cur_psp], ax"]],
    hi: [3056, 3099, 5048, 3099, 3099, 3099, 3099, 3099, {a: "do_terminate_restore_parent_psp"}],
  ...
  },
  {
    ...
    code: [
      ...
      [124, "%ifndef ENABLE_XMS"],
      [125, "%define ENABLE_XMS 1"],
      [2044, "init_xms_size:"],
      [2046, "    mov word [cs:xms_total_kb], 0"],
      [2047, "    mov ah, 0x88"],
      [2048, "    int 0x15"],
      [2053, "    call init_xms_store"],
      [4779, "init_xms_store:"],
      [4784, "    cmp ax, (EMS_TOTAL_PAGES * 16)"],
      [4789, "    mov ax, XMS_MAX_KB - 64"],
      [4800, "    mov [cs:ems_backing_hi], bx"],
      [4796, "    mov [cs:xms_total_kb], ax"],
      [2081, "int2f_handler:"],
      [2095, "    cmp ax, 0x4300"],
      [2097, "    cmp ax, 0x4310"],
      [2103, "    mov al, 0x80"],
      [2106, "    mov bx, xms_entry"],
      [2119, "xms_entry:"],
      [2136, "    cmp ah, 0x08"],
      [2138, "    cmp ah, 0x09"],
      [2140, "    cmp ah, 0x0A"],
      [2142, "    cmp ah, 0x0B"],
      [2179, "    mov ax, [cs:xms_total_kb]"],
      [2185, "    cmp word [cs:xms_alloc_kb], 0"],
      [2189, "    cmp dx, [cs:xms_total_kb]"],
      [2191, "    mov [cs:xms_alloc_kb], dx"],
      [2167, "    mov ax, 1"],
      [2156, "    mov dx, 1"],
      [2205, "    mov word [cs:xms_alloc_kb], 0"],
      [2209, ".move:"],
      [2218, "    test ax, 1"],
      [2226, "    call xms_prepare_endpoint"],
      [2234, "    call xms_prepare_endpoint"],
      [2280, "    mov ax, 0x8700"],
      [{a: "xms_move_bios_call"}, "    int 0x15"]],
    hi: [45, 124, 2044, 4779, 2095, 2103, 2119, 2179, 2205, {a: "xms_move_bios_call"}],
    tests: ["scripts/test_xms.py", "scripts/test_free.py", "scripts/test_shell.py"],
  },
  {
    id: "ems",
    title: "EMS is default with an upper frame",
    summary: "Normal builds install INT 67h with 32 handles, four page-frame slots, and a 6 MiB backing pool when the D000h frame probe succeeds.",
    body: [
    ...
    code: [
      ...
      [56, "EMS_BOUNCE_PHYS_LO equ ((EMS_BOUNCE_BUF << 4) & 0xFFFF)"],
      [62, "EMS_COPY_GDT_BUF equ SEC_BUF"],
      [128, "%ifndef ENABLE_EMS"],
      [129, "%define ENABLE_EMS 1"],
      [1849, "%if ENABLE_EMS"],
      [1850, "    mov [es:0x67*4], word int67_handler"],
      [1852, "%else"],
      [1853, "    mov [es:0x67*4], word int67_absent_handler"],
      [2438, "%if !ENABLE_EMS"],
      [2439, "int67_absent_handler:"],
      [2440, "    mov ah, 0x80"],
      [2444, "%if ENABLE_EMS"],
      [2445, "EMS_MAX_HANDLES equ 32"],
      [2447, "int67_handler:"],
      [2480, "    cmp ah, 0x50"],
      [2481, "    je .map_multi"],
      [2491, ".pages:"],
      [2492, "    mov bx, [cs:ems_total_pages]"],
      [2497, ".alloc:"],
      [2500, "    mov ax, [cs:ems_total_pages]"],
      [2501, "    sub ax, [cs:ems_alloc_pages]"],
      [2502, "    cmp bx, ax"],
      [2504, "    call ems_find_free_handle"],
      [2506, "    call ems_find_page_run"],
      [2508, "    mov [cs:si+ems_handle_pages], bx"],
      [2509, "    mov [cs:si+ems_handle_base], ax"],
      [2510, "    add [cs:ems_alloc_pages], bx"],
      [2520, ".map:"],
      [2521, "    call ems_map_request"],
      [2523, ".map_multi:"],
      [2524, "    call ems_map_multi_request"],
      [2529, ".free:"],
      [2532, "    call ems_clear_handle_maps"],
      [2533, "    mov ax, [cs:si+ems_handle_base]"],
      [2535, "    call ems_clear_owner_pages"],
      [2536, "    sub [cs:ems_alloc_pages], bx"],
      [2544, ".handles:"],
      [2546, "    mov bx, EMS_MAX_HANDLES"],
      [2548, ".info:"],
      [2551, "    mov bx, [cs:si+ems_handle_pages]"],
      [2631, "ems_map_request:"],
      [2632, "    call ems_handle_offset"],
      [2634, "    cmp al, 3"],
      [2636, "    cmp bx, [cs:si+ems_handle_pages]"],
      [2648, "    cmp bx, [cs:si+ems_map_pages]"],
      [2650, "    je .map_already"],
      [2674, "    call ems_copy_16k"],
      [2692, "    mov [cs:si+ems_map_pages], bx"],
      [2704, ".map_already:"],
      [2730, "ems_unmap_phys_page:"],
      [2755, "    call ems_copy_16k"],
      [2761, "    mov word [cs:si+ems_map_pages], 0xFFFF"],
      [2789, "ems_map_multi_request:"],
      [2790, "    cmp al, 0"],
      [2798, "    mov bx, [ds:si]"],
      [2800, "    cmp bx, 0xFFFF"],
      [2804, "    call ems_map_request"],
      [2810, "    call ems_unmap_phys_page"],
      [2813, "    add word [cs:ems_multi_ptr], 4"],
      [2830, "ems_copy_16k:"],
      [2841, "    cmp word [cs:xms_dst_phys+2], EMS_FRAME_PHYS_HI"],
      [2858, "    mov dx, EMS_BOUNCE_BUF"],
      [2863, "    call ems_flat_copy_chunk"],
      [2884, "    call ems_flat_copy_chunk"],
      [2891, "    call ems_copy_real_chunk"],
      [5048, "    mov word [cs:ems_alloc_pages], 0"],
      [5049, "    mov di, ems_handle_pages"],
      [5052, "    mov di, ems_handle_base"],
      [5055, "    mov di, ems_page_owner"],
      [2596, "ems_clear_map:"],
      [{a: "ems_clear_map_fill"}, "    mov word [cs:ems_map_pages], 0xFFFF"]],
    hi: [128, 1849, 2438, 2440, 2445, 2480, 2492, 2501, 2502, 2504, 2506, 2520, 2523, 2532, 2546, 2631, 2648, 2674, 2789, 2790, 2800, 2804, 2810, 2830, 2858, 2863, 5048, 5048, 5048],
    tests: ["scripts/test_ems.py", "scripts/test_emsmulti.py", "scripts/test_emsmap40.py", "scripts/test_emspreserve.py", "scripts/test_emsmem.py", "scripts/test_emslarge.py", "scripts/test_emsxms.py"],
  },
  {
    id: "report",
...
const MEM_TESTS = [
  ...
  ["EMS 4.0 map", "scripts/test_emsmap40.py", "Checks 17+ handles and AH=50h physical-page map/unmap entries."],
