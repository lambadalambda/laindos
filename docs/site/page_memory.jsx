...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3476, "%if LOAD_SEG <= RELOC_SEG"],
      [3477, "%error \"LOAD_SEG must be above RELOC_SEG\""],
      [3479, "%if (kernel_end - kernel_entry) > ((LOAD_SEG - RELOC_SEG) * 16)"],
      [3480, "%error \"kernel exceeds boot relocation gap\""],
      [3482, "%if (kernel_end - kernel_entry) > ((SEC_BUF - RELOC_SEG) * 16)"],
      [3483, "%error \"kernel overlaps SEC_BUF\""],
      [3485, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3486, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3488, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3489, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3491, "%if (ROOT_SEG + ROOT_BUF_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3492, "%error \"ROOT buffer overlaps kernel stack\""],
      [3494, "%if (ROOT_SEG + ROOT_BUF_PARAS + STACK_ROOT_GUARD_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3495, "%error \"ROOT buffer leaves too little kernel stack guard\""],
      [3503, "%if (RELOC_SEG + (KERNEL_STACK_TOP / 16)) > MCB_START"],
      [3504, "%error \"kernel stack overlaps MCB arena\""],
      [3506, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3507, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3509, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3510, "%error \"EMS frame must be inside conventional arena\""],
      [3521, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3522, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
