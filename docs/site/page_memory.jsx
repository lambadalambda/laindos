...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3742, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3743, "%error \"kernel leaves too little HMA stack guard\""],
      [3745, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3746, "%error \"kernel exceeds boot load area\""],
      [3748, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3749, "%error \"FAT buffer overlaps CD_BUF\""],
      [3751, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3752, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3754, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3755, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3757, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3758, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3760, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3761, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3763, "%if MCB_START >= MEM_TOP"],
      [3764, "%error \"MCB arena is empty\""],
      [3766, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3767, "%error \"EMS frame must be inside conventional arena\""],
      [3778, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
