...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3715, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3716, "%error \"kernel leaves too little HMA stack guard\""],
      [3718, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3719, "%error \"kernel exceeds boot load area\""],
      [3721, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3722, "%error \"FAT buffer overlaps CD_BUF\""],
      [3724, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3725, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3727, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3728, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3730, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3731, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3733, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3734, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3736, "%if MCB_START >= MEM_TOP"],
      [3737, "%error \"MCB arena is empty\""],
      [3739, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
