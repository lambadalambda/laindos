...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4327, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4328, "%error \"kernel leaves too little HMA stack guard\""],
      [4330, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4331, "%error \"kernel exceeds boot load area\""],
      [4333, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4334, "%error \"FAT buffer overlaps CD_BUF\""],
      [4336, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4337, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4339, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4340, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4342, "%if (READ_CACHE_BUF + READ_CACHE_PARAS) > ROOT_SEG"],
      [4343, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4345, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4346, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4348, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4349, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4351, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4352, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4354, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4355, "%error \"CD cache overlaps MCB arena\""],
      [4357, "%if MCB_START >= MEM_TOP"],
      [4358, "%error \"MCB arena is empty\""],
      [4360, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4361, "%error \"EMS frame must be inside conventional arena\""],
      [4372, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4327, 4330, 4333, 4348, 4351, 4354, 4357, 4360, 4372],
