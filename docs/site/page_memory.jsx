...
const MEM_REGIONS = [
  ...
  ["0660:0000", "CD_CACHE_BUF holds the four 2 KiB CD file-read cache slots."],
...
const MEM_SECTIONS = [
  {
    ...
    body: [
      "`src/memory.inc` is the first file to read before moving buffers. These equates decide where the boot sector loads the kernel, where the kernel relocates (the High Memory Area at FFFF:0010), where CD/FAT/read/write-cache scratch sectors live, where the DOS arena begins, and where conventional memory ends.",
    ...
    code: [
      ...
      [9, "CD_CACHE_SLOTS equ 4"],
      [10, "%if (CD_CACHE_SLOTS & (CD_CACHE_SLOTS - 1)) != 0"],
      [11, "%error \"CD_CACHE_SLOTS must be a power of two\""],
      [13, "CD_CACHE_SLOT_PARAS equ CD_BUF_PARAS"],
      [14, "CD_BUF equ 0x0180"],
      [15, "SEC_BUF equ 0x0200"],
      [16, "READ_CACHE_BUF equ (SEC_BUF + SECTOR_BUF_PARAS)"],
      [17, "WRITE_CACHE_BUF equ 0x0640"],
      [18, "CD_CACHE_BUF equ (WRITE_CACHE_BUF + SECTOR_BUF_PARAS)"],
      [26, "MCB_START equ 0x0B00"],
      [27, "MEM_TOP equ 0xA000"],
      [28, "ENV_PARAS equ 16"],
      [29, "ENV_SIZE_BYTES equ (ENV_PARAS * 16)"],
      [30, "ENV_OWNER_TEMP equ 0xFFFF"],
      [31, "%if (MEM_TOP & 0xFF) != 0"],
      [32, "%error \"MEM_TOP must be 256-byte aligned\""],
      [34, "MCB_SIG_M equ 'M'"],
      [35, "MCB_SIG_Z equ 'Z'"]],
    hi: [4, 6, 7, 8, 9, 10, 17, 18, 26, 30, 31],
  ...
  },
  {
    ...
    body: [
      ...
      "These guards keep the HMA-resident kernel clear of its own stack, keep the image small enough for the boot loader's staging area at `LOAD_SEG`, keep the low FAT/CD/sector/read/root/write/CD-cache buffers ordered, and ensure the cache buffers stay below `MCB_START`."],
    file: "src/kernel.asm",
    code: [
      [4277, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4278, "%error \"kernel leaves too little HMA stack guard\""],
      [4280, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4281, "%error \"kernel exceeds boot load area\""],
      [4283, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4284, "%error \"FAT buffer overlaps CD_BUF\""],
      [4286, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4287, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4289, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4290, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4292, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [4293, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4295, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4296, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4298, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4299, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4301, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4302, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4304, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4305, "%error \"CD cache overlaps MCB arena\""],
      [4307, "%if MCB_START >= MEM_TOP"],
      [4308, "%error \"MCB arena is empty\""],
      [4310, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4311, "%error \"EMS frame must be inside conventional arena\""],
      [4322, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4277, 4280, 4283, 4298, 4301, 4304, 4307, 4310, 4322],
  ...
  },
  {
    ...
    code: [
      [2684, "do_terminate:"],
      [2694, "%if ENABLE_EMS"],
      [2551, "    mov word [cs:ems_alloc_pages], 0"],
      [2698, "    mov word [cs:xms_alloc_kb], 0"],
      [2699, "    call release_inherited_handles"],
      [2700, "    call close_owned_handles"],
      [2702, "    mov si, [cs:mcb_first]"],
      [2761, "    mov ds, si"],
      [2660, "    mov ax, [cs:cur_psp]"],
      [2706, "    cmp word [ds:1], ax"],
      [2708, "    mov word [ds:1], 0"],
      [2703, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
      [2711, "    call mcb_coalesce_all_free"],
      [2716, "    mov ax, [0x16]"],
      [{a: "do_terminate_restore_parent_psp"}, "    mov [cs:cur_psp], ax"]],
    hi: [2684, 2694, 2694, 2694, 2684, 2699, {a: "do_terminate_restore_parent_psp"}],
  ...
  },
  {
    ...
    code: [
      ...
      [2028, "init_xms_size:"],
      [2030, "    mov word [cs:xms_total_kb], 0"],
      [2031, "    mov ah, 0x88"],
      [2032, "    int 0x15"],
      [2037, "    cmp ax, XMS_MAX_KB - 64"],
      [2039, "    mov ax, XMS_MAX_KB - 64"],
      [2041, "    mov [cs:xms_total_kb], ax"],
      [2063, "int2f_handler:"],
      [2075, "    cmp ax, 0x4300"],
      [2077, "    cmp ax, 0x4310"],
      [2083, "    mov al, 0x80"],
      [2086, "    mov bx, xms_entry"],
      [2099, "xms_entry:"],
      [2116, "    cmp ah, 0x08"],
      [2118, "    cmp ah, 0x09"],
      [2120, "    cmp ah, 0x0A"],
      [2122, "    cmp ah, 0x0B"],
      [2159, "    mov ax, [cs:xms_total_kb]"],
      [2165, "    cmp word [cs:xms_alloc_kb], 0"],
      [2169, "    cmp dx, [cs:xms_total_kb]"],
      [2171, "    mov [cs:xms_alloc_kb], dx"],
      [2147, "    mov ax, 1"],
      [2136, "    mov dx, 1"],
      [2185, "    mov word [cs:xms_alloc_kb], 0"],
      [2189, ".move:"],
      [2198, "    test ax, 1"],
      [2206, "    call xms_prepare_endpoint"],
      [2214, "    call xms_prepare_endpoint"],
      [2260, "    mov ax, 0x8700"],
      [{a: "xms_move_bios_call"}, "    int 0x15"]],
    hi: [43, 113, 2028, 2075, 2086, 2099, 2165, 2185, {a: "xms_move_bios_call"}],
  ...
  },
  {
    ...
    code: [
      ...
      [1828, "%if ENABLE_EMS"],
      [1829, "    mov [es:0x67*4], word int67_handler"],
      [1831, "%else"],
      [1832, "    mov [es:0x67*4], word int67_absent_handler"],
      [2417, "%if !ENABLE_EMS"],
      [2418, "int67_absent_handler:"],
      [2419, "    mov ah, 0x80"],
      [2423, "%if ENABLE_EMS"],
      [2424, "int67_handler:"],
      [2448, ".frame:"],
      [2450, "    mov bx, EMS_FRAME_SEG"],
      [2452, ".pages:"],
      [2453, "    mov bx, EMS_TOTAL_PAGES"],
      [2458, ".alloc:"],
      [2463, "    cmp bx, EMS_TOTAL_PAGES"],
      [2465, "    mov [cs:ems_alloc_pages], bx"],
      [2473, ".map:"],
      [2478, "    cmp al, 3"],
      [2480, "    cmp bx, [cs:ems_alloc_pages]"],
      [2505, "    call ems_copy_16k"],
      [2516, "    call ems_copy_16k"],
      [2523, "    mov [cs:si+ems_map_pages], bx"],
      [2551, "    mov word [cs:ems_alloc_pages], 0"],
      [2575, "ems_clear_map:"],
      [{a: "ems_clear_map_fill"}, "    mov word [cs:ems_map_pages], 0xFFFF"]],
    hi: [117, 1828, 2418, 2424, 2448, 2463, 2463, 2424, 2523],
