...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3846, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3847, "%error \"kernel leaves too little HMA stack guard\""],
      [3849, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3850, "%error \"kernel exceeds boot load area\""],
      [3852, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3853, "%error \"FAT buffer overlaps CD_BUF\""],
      [3855, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3856, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3858, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3859, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3861, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3862, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3864, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3865, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3867, "%if MCB_START >= MEM_TOP"],
      [3868, "%error \"MCB arena is empty\""],
      [3870, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3871, "%error \"EMS frame must be inside conventional arena\""],
      [3882, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [3846, 3849, 3852, 3855, 3858, 3861, 3864, 3867, 3870, 3882],
  ...
  },
  {
    ...
    code: [
      [2559, "do_terminate:"],
      [2569, "%if ENABLE_EMS"],
      [2426, "    mov word [cs:ems_alloc_pages], 0"],
      [2573, "    mov word [cs:xms_alloc_kb], 0"],
      [2574, "    call release_inherited_handles"],
      [2575, "    call close_owned_handles"],
      [2577, "    mov si, [cs:mcb_first]"],
      [2636, "    mov ds, si"],
      [2535, "    mov ax, [cs:cur_psp]"],
      [2581, "    cmp word [ds:1], ax"],
      [2583, "    mov word [ds:1], 0"],
      [2578, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
      [2586, "    call mcb_coalesce_all_free"],
      [2591, "    mov ax, [0x16]"],
      [{a: "do_terminate_restore_parent_psp"}, "    mov [cs:cur_psp], ax"]],
    hi: [2559, 2573, 2577, 2581, 2559, 2586, {a: "do_terminate_restore_parent_psp"}],
  ...
  },
  {
    ...
    code: [
      ...
      [1905, "init_xms_size:"],
      [1907, "    mov word [cs:xms_total_kb], 0"],
      [1908, "    mov ah, 0x88"],
      [1909, "    int 0x15"],
      [1914, "    cmp ax, XMS_MAX_KB - 64"],
      [1916, "    mov ax, XMS_MAX_KB - 64"],
      [1918, "    mov [cs:xms_total_kb], ax"],
      [1940, "int2f_handler:"],
      [1950, "    cmp ax, 0x4300"],
      [1952, "    cmp ax, 0x4310"],
      [1958, "    mov al, 0x80"],
      [1961, "    mov bx, xms_entry"],
      [1974, "xms_entry:"],
      [1991, "    cmp ah, 0x08"],
      [1993, "    cmp ah, 0x09"],
      [1995, "    cmp ah, 0x0A"],
      [1997, "    cmp ah, 0x0B"],
      [2034, "    mov ax, [cs:xms_total_kb]"],
      [2058, "    cmp word [cs:xms_alloc_kb], 0"],
      [2044, "    cmp dx, [cs:xms_total_kb]"],
      [2046, "    mov [cs:xms_alloc_kb], dx"],
      [2030, "    mov ax, 1"],
      [2011, "    mov dx, 1"],
      [2060, "    mov word [cs:xms_alloc_kb], 0"],
      [2064, ".move:"],
      [2073, "    test ax, 1"],
      [2081, "    call xms_prepare_endpoint"],
      [2081, "    call xms_prepare_endpoint"],
      [2135, "    mov ax, 0x8700"],
      [{a: "xms_move_bios_call"}, "    int 0x15"]],
    hi: [43, 114, 1905, 1958, 1961, 1974, 2046, 2064, {a: "xms_move_bios_call"}],
  ...
  },
  {
    ...
    code: [
      ...
      [1705, "%if ENABLE_EMS"],
      [1706, "    mov [es:0x67*4], word int67_handler"],
      [1708, "%else"],
      [1709, "    mov [es:0x67*4], word int67_absent_handler"],
      [2292, "%if !ENABLE_EMS"],
      [2293, "int67_absent_handler:"],
      [2294, "    mov ah, 0x80"],
      [2298, "%if ENABLE_EMS"],
      [2299, "int67_handler:"],
      [2323, ".frame:"],
      [2325, "    mov bx, EMS_FRAME_SEG"],
      [2327, ".pages:"],
      [2328, "    mov bx, EMS_TOTAL_PAGES"],
      [2333, ".alloc:"],
      [2338, "    cmp bx, EMS_TOTAL_PAGES"],
      [2340, "    mov [cs:ems_alloc_pages], bx"],
      [2348, ".map:"],
      [2353, "    cmp al, 3"],
      [2355, "    cmp bx, [cs:ems_alloc_pages]"],
      [2380, "    call ems_copy_16k"],
      [2380, "    call ems_copy_16k"],
      [2398, "    mov [cs:si+ems_map_pages], bx"],
      [2426, "    mov word [cs:ems_alloc_pages], 0"],
      [2450, "ems_clear_map:"],
      [{a: "ems_clear_map_fill"}, "    mov word [cs:ems_map_pages], 0xFFFF"]],
    hi: [118, 1709, 2293, 2299, 2325, 2340, 2348, 2380, 2398],
