...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3714, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3715, "%error \"kernel leaves too little HMA stack guard\""],
      [3717, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3718, "%error \"kernel exceeds boot load area\""],
      [3720, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3721, "%error \"FAT buffer overlaps CD_BUF\""],
      [3723, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3724, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3726, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3727, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3729, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3730, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3732, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3733, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3735, "%if MCB_START >= MEM_TOP"],
      [3736, "%error \"MCB arena is empty\""],
      [3738, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3739, "%error \"EMS frame must be inside conventional arena\""],
      [3750, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      ...
      [720, "    mov word [cs:exec_env_seg], 0"],
      [544, "    mov bx, ENV_PARAS"],
      [726, "    call alloc_mem_direct"],
      [728, "    mov [cs:exec_env_seg], ax"],
      [731, "    dec ax"],
      [732, "    mov ds, ax"],
      ...
      [776, "assign_exec_environment_owner:"],
      [780, "    mov bx, [cs:exec_env_seg]"],
      [785, "    dec bx"],
      [790, "    mov ax, [cs:cur_psp]"],
  ...
  },
  {
    ...
    code: [
      ...
      [2442, "    mov word [cs:xms_alloc_kb], 0"],
      [2484, "    call release_inherited_handles"],
      [2485, "    call close_owned_handles"],
      [2549, "    mov si, [cs:mcb_first]"],
      [2524, "    mov ds, si"],
      [2552, "    mov ax, [cs:cur_psp]"],
      [2450, "    cmp word [ds:1], ax"],
      [2559, "    mov word [ds:1], 0"],
      [2447, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
  ...
  },
  {
    ...
    code: [
      ...
      [1774, "init_xms_size:"],
      [1776, "    mov word [cs:xms_total_kb], 0"],
      [1777, "    mov ah, 0x88"],
      [1778, "    int 0x15"],
      [1783, "    cmp ax, XMS_MAX_KB - 64"],
      [1785, "    mov ax, XMS_MAX_KB - 64"],
      [1787, "    mov [cs:xms_total_kb], ax"],
      [1809, "int2f_handler:"],
      [1819, "    cmp ax, 0x4300"],
      [1821, "    cmp ax, 0x4310"],
      [1836, "    mov al, 0x80"],
      [1830, "    mov bx, xms_entry"],
      [1843, "xms_entry:"],
      [1860, "    cmp ah, 0x08"],
      [1862, "    cmp ah, 0x09"],
      [1864, "    cmp ah, 0x0A"],
      [1866, "    cmp ah, 0x0B"],
      [1903, "    mov ax, [cs:xms_total_kb]"],
      [2052, "    cmp word [cs:xms_alloc_kb], 0"],
      [1913, "    cmp dx, [cs:xms_total_kb]"],
      [1915, "    mov [cs:xms_alloc_kb], dx"],
      [2029, "    mov ax, 1"],
      [1917, "    mov dx, 1"],
      [1929, "    mov word [cs:xms_alloc_kb], 0"],
      [1933, ".move:"],
      [1942, "    test ax, 1"],
  ...
  },
  {
    ...
    code: [
      ...
      [1574, "%if ENABLE_EMS"],
      [1575, "    mov [es:0x67*4], word int67_handler"],
      [1577, "%else"],
      [1578, "    mov [es:0x67*4], word int67_absent_handler"],
      [2161, "%if !ENABLE_EMS"],
      [2162, "int67_absent_handler:"],
      [2187, "    mov ah, 0x80"],
      [2167, "%if ENABLE_EMS"],
      [2168, "int67_handler:"],
      [2192, ".frame:"],
      [2194, "    mov bx, EMS_FRAME_SEG"],
      [2196, ".pages:"],
      [2197, "    mov bx, EMS_TOTAL_PAGES"],
      [2202, ".alloc:"],
      [2207, "    cmp bx, EMS_TOTAL_PAGES"],
      [2209, "    mov [cs:ems_alloc_pages], bx"],
      [2217, ".map:"],
      [2222, "    cmp al, 3"],
      [2224, "    cmp bx, [cs:ems_alloc_pages]"],
      [2260, "    call ems_copy_16k"],
      [2260, "    call ems_copy_16k"],
      [2267, "    mov [cs:si+ems_map_pages], bx"],
      [2295, "    mov word [cs:ems_alloc_pages], 0"],
      [2319, "ems_clear_map:"],
