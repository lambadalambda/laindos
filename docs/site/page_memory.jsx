...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    code: [
      [94, "kernel_entry:"],
      [110, "    mov ax, RELOC_SEG"],
      [117, "    jmp RELOC_SEG:.relocated"],
      [118, ".relocated:"],
      [122, "    mov ss, ax"],
      [123, "    mov sp, KERNEL_STACK_TOP"],
      [134, "    int 0x12"],
      [135, "    mov [mem_kib], ax"],
      [143, "%if ENABLE_XMS"],
      [144, "    call init_xms_size"],
      [158, "    mov ax, MCB_START"],
      [159, "    mov es, ax"],
      [160, "    mov byte [es:0], MCB_SIG_Z"],
      [161, "    mov word [es:1], 0"],
      [162, "    mov ax, MEM_TOP - MCB_START - 1"],
      [163, "    mov word [es:3], ax"],
      [164, "    mov word [mcb_first], MCB_START"],
      [165, "    mov word [cur_psp], 0"]],
    hi: [110, 123, 144, 158, 160, 163, 164],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3298, "%if LOAD_SEG <= RELOC_SEG"],
      [3299, "%error \"LOAD_SEG must be above RELOC_SEG\""],
      [3301, "%if (kernel_end - kernel_entry) > ((LOAD_SEG - RELOC_SEG) * 16)"],
      [3302, "%error \"kernel exceeds boot relocation gap\""],
      [3304, "%if (kernel_end - kernel_entry) > ((SEC_BUF - RELOC_SEG) * 16)"],
      [3305, "%error \"kernel overlaps SEC_BUF\""],
      [3307, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3308, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3310, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3311, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3313, "%if (ROOT_SEG + ROOT_BUF_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3314, "%error \"ROOT buffer overlaps kernel stack\""],
      [3316, "%if (ROOT_SEG + ROOT_BUF_PARAS + STACK_ROOT_GUARD_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3317, "%error \"ROOT buffer leaves too little kernel stack guard\""],
      [3319, "%if (RELOC_SEG + (KERNEL_STACK_TOP / 16)) > MCB_START"],
      [3320, "%error \"kernel stack overlaps MCB arena\""],
      [3322, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3323, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3325, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3326, "%error \"EMS frame must be inside conventional arena\""],
      [3337, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3338, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [3301, 3304, 3307, 3310, 3316, 3319, 3325, 3337],
  ...
  },
  {
    ...
    code: [
      ...
      [1218, "    je .as_set"],
      [1221, "    xor ax, ax"],
      [1222, "    mov al, [cs:alloc_strat]"],
      [1225, "    cmp bl, 2"],
      [1227, "    mov [cs:alloc_strat], bl"],
      [1266, ".alloc_mem:"],
      [1275, "    mov al, [cs:alloc_strat]"],
      [1277, "    cmp al, 2"],
      [1279, "    jmp near .am_find_last"],
      [1281, "    cmp al, 1"],
      [1283, "    jmp near .am_find_best"],
      [1285, "    cmp word [cs:am_req], 1"],
      [1287, "    cmp word [cs:am_req], SMALL_ALLOC_HIGH_MAX"],
      [1289, "    jmp near .am_find_last"],
      [1291, "    mov si, [cs:mcb_first]"],
      [1304, "    cmp ax, [cs:am_req]"],
      [1306, ".am_use:"],
      [1330, "    mov ax, [cs:cur_psp]"],
      [1372, ".am_find_last:"],
      [1396, ".am_find_best:"],
      [1453, ".am_nomem:"],
      [1456, ".am_scan_largest:"],
      [1477, "    mov ax, 8"]],
    hi: [1192, 1227, 1266, 1287, 1372, 1396, 1456],
  ...
  },
  {
    ...
    code: [
      ...
      [1509, "    mov si, es"],
      [1510, "    dec si"],
      [1512, "    cmp byte [ds:0], MCB_SIG_M"],
      [1514, "    cmp byte [ds:0], MCB_SIG_Z"],
      [1519, "    mov word [ds:1], 0"],
      [1520, "    call mcb_merge_free_forward"],
      [1527, ".resize_mem:"],
      [1534, "    mov [cs:rm_req], bx"],
      [1535, "    mov si, es"],
      [1545, "    mov ax, [ds:3]"],
      [1547, "    jae .rm_shrink"],
      [1555, "    cmp byte [es:0], MCB_SIG_M"],
      [1560, "    cmp word [es:1], 0"],
      [1564, "    add ax, cx"],
      [1565, "    cmp ax, bx"],
      [1579, "    mov byte [es:0], dl"],
      [1580, "    mov byte [ds:0], MCB_SIG_M"],
      [1585, "    mov word [es:3], cx"],
      [1586, "    mov word [ds:3], bx"],
      [1590, "    mov ax, [ds:3]"],
      [1600, "    mov byte [es:0], dl"],
      [1601, "    mov word [es:1], 0"],
      [1605, "    mov word [ds:3], bx"],
      [1648, "    mov [es:0x02], ax"],
      [1691, ".rm_cant_grow:"],
      [1692, "    mov bx, ax"],
      [1693, "    mov ax, 8"]],
    hi: [1503, 1519, 1520, 1527, 1560, 1601, 1648, 1691],
  ...
  },
  {
    ...
    code: [
      [418, "alloc_exec_environment:"],
      [421, "    mov word [cs:exec_env_seg], 0"],
      [423, "    mov bx, ENV_PARAS"],
      [424, "    call alloc_mem_direct"],
      [426, "    mov [cs:exec_env_seg], ax"],
      [429, "    dec ax"],
      [430, "    mov ds, ax"],
      [431, "    mov word [ds:1], ENV_OWNER_TEMP"],
      [443, "free_exec_environment:"],
      [451, "    dec ax"],
      [458, "    mov word [ds:1], 0"],
      [469, "assign_exec_environment_owner:"],
      [473, "    mov bx, [cs:exec_env_seg]"],
      [478, "    dec bx"],
      [485, "    mov ax, [cs:cur_psp]"],
      [486, "    mov [ds:1], ax"]],
    hi: [418, 431, 443, 469, 485, 486],
  ...
  },
  {
    ...
    code: [
      [2131, "do_terminate:"],
      [2140, "%if ENABLE_EMS"],
      [2141, "    mov word [cs:ems_alloc_pages], 0"],
      [2144, "    mov word [cs:xms_alloc_kb], 0"],
      [2145, "    call release_inherited_handles"],
      [2146, "    call close_owned_handles"],
      [2148, "    mov si, [cs:mcb_first]"],
      [2150, "    mov ds, si"],
      [2157, "    mov ax, [cs:cur_psp]"],
      [2158, "    cmp word [ds:1], ax"],
      [2160, "    mov word [ds:1], 0"],
      [2164, "    call mcb_walk_next"],
      [2168, "    call mcb_coalesce_all_free"],
      [2173, "    mov ax, [0x16]"],
      [2174, "    mov [cs:cur_psp], ax"]],
    hi: [2131, 2144, 2148, 2158, 2160, 2168, 2174],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [41, "%ifndef XMS_MAX_KB"],
      [42, "%define XMS_MAX_KB 15360"],
      [86, "%ifndef ENABLE_XMS"],
      [87, "%define ENABLE_XMS 1"],
      [1570, "init_xms_size:"],
      [1572, "    mov word [cs:xms_total_kb], 0"],
      [1573, "    mov ah, 0x88"],
      [1574, "    int 0x15"],
      [1578, "    cmp ax, XMS_MAX_KB"],
      [1580, "    mov ax, XMS_MAX_KB"],
      [1582, "    mov [cs:xms_total_kb], ax"],
      [1588, "int2f_handler:"],
      [1590, "    cmp ax, 0x4300"],
      [1592, "    cmp ax, 0x4310"],
      [1597, "    mov al, 0x80"],
      [1600, "    mov bx, xms_entry"],
      [1610, "xms_entry:"],
      [1613, "    cmp ah, 0x08"],
      [1615, "    cmp ah, 0x09"],
      [1617, "    cmp ah, 0x0A"],
      [1619, "    cmp ah, 0x0B"],
      [1636, "    mov ax, [cs:xms_total_kb]"],
      [1642, "    cmp word [cs:xms_alloc_kb], 0"],
      [1646, "    cmp dx, [cs:xms_total_kb]"],
      [1648, "    mov [cs:xms_alloc_kb], dx"],
      [1649, "    mov ax, 1"],
      [1650, "    mov dx, 1"],
      [1662, "    mov word [cs:xms_alloc_kb], 0"],
      [1666, ".move:"],
      [1675, "    test ax, 1"],
      [1683, "    call xms_prepare_endpoint"],
      [1691, "    call xms_prepare_endpoint"],
      [1737, "    mov ax, 0x8700"],
      [1742, "    int 0x15"]],
    hi: [42, 87, 1570, 1597, 1600, 1610, 1648, 1666, 1742],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [44, "EMS_TOTAL_PAGES equ 64"],
      [45, "%ifndef EMS_FRAME_SEG"],
      [46, "%define EMS_FRAME_SEG 0x9000"],
      [48, "EMS_FRAME_PARAS equ 0x1000"],
      [51, "EMS_BACKING_HI equ 0x0020"],
      [90, "%ifndef ENABLE_EMS"],
      [91, "%define ENABLE_EMS 0"],
      [1436, "%if ENABLE_EMS"],
      [1437, "    mov [es:0x67*4], word int67_handler"],
      [1439, "%else"],
      [1440, "    mov [es:0x67*4], word int67_absent_handler"],
      [1893, "%if !ENABLE_EMS"],
      [1894, "int67_absent_handler:"],
      [1895, "    mov ah, 0x80"],
      [1899, "%if ENABLE_EMS"],
      [1900, "int67_handler:"],
      [1924, ".frame:"],
      [1926, "    mov bx, EMS_FRAME_SEG"],
      [1928, ".pages:"],
      [1929, "    mov bx, EMS_TOTAL_PAGES"],
      [1934, ".alloc:"],
      [1939, "    cmp bx, EMS_TOTAL_PAGES"],
      [1941, "    mov [cs:ems_alloc_pages], bx"],
      [1949, ".map:"],
      [1954, "    cmp al, 3"],
      [1956, "    cmp bx, [cs:ems_alloc_pages]"],
      [1981, "    call ems_copy_16k"],
      [1992, "    call ems_copy_16k"],
      [1999, "    mov [cs:si+ems_map_pages], bx"],
      [2027, "    mov word [cs:ems_alloc_pages], 0"],
      [2051, "ems_clear_map:"],
      [2052, "    mov word [cs:ems_map_pages], 0xFFFF"]],
    hi: [91, 1440, 1894, 1900, 1926, 1941, 1949, 1981, 1999],
