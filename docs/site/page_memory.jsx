const MEM_FLOW = [
  ["Layout", "The kernel relocates to 0200h, keeps scratch buffers below 1000h, and reserves A000h for VGA."],
...
const MEM_REGIONS = [
  ...
  ["0200:0000", "The relocated kernel and stack live here; compile-time guards keep them below buffers and MCB_START."],
...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3485, "%if LOAD_SEG <= RELOC_SEG"],
      [3486, "%error \"LOAD_SEG must be above RELOC_SEG\""],
      [3488, "%if (kernel_end - kernel_entry) > ((LOAD_SEG - RELOC_SEG) * 16)"],
      [3489, "%error \"kernel exceeds boot relocation gap\""],
      [3491, "%if (kernel_end - kernel_entry) > ((SEC_BUF - RELOC_SEG) * 16)"],
      [3492, "%error \"kernel overlaps SEC_BUF\""],
      [3494, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3495, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3497, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3498, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3500, "%if (ROOT_SEG + ROOT_BUF_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3501, "%error \"ROOT buffer overlaps kernel stack\""],
      [3503, "%if (ROOT_SEG + ROOT_BUF_PARAS + STACK_ROOT_GUARD_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3504, "%error \"ROOT buffer leaves too little kernel stack guard\""],
      [3512, "%if (RELOC_SEG + (KERNEL_STACK_TOP / 16)) > MCB_START"],
      [3513, "%error \"kernel stack overlaps MCB arena\""],
      [3515, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3516, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3518, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3519, "%error \"EMS frame must be inside conventional arena\""],
      [3530, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3531, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
  ...
  },
  {
    ...
    code: [
      ...
      [1278, "    je .as_set"],
      [630, "    xor ax, ax"],
      [1282, "    mov al, [cs:alloc_strat]"],
      [1285, "    cmp bl, 2"],
      [1287, "    mov [cs:alloc_strat], bl"],
      [1324, ".alloc_mem:"],
      [1333, "    mov al, [cs:alloc_strat]"],
      [1012, "    cmp al, 2"],
      [1337, "    jmp near .am_find_last"],
      [1339, "    cmp al, 1"],
      [1341, "    jmp near .am_find_best"],
      [1343, "    cmp word [cs:am_req], 1"],
      [1345, "    cmp word [cs:am_req], SMALL_ALLOC_HIGH_MAX"],
      [1337, "    jmp near .am_find_last"],
      [1349, "    mov si, [cs:mcb_first]"],
      [1362, "    cmp ax, [cs:am_req]"],
      [1364, ".am_use:"],
      [987, "    mov ax, [cs:cur_psp]"],
      [1430, ".am_find_last:"],
      [1454, ".am_find_best:"],
      [1511, ".am_nomem:"],
      [1514, ".am_scan_largest:"],
      [1535, "    mov ax, 8"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1567, "    mov si, es"],
      [1568, "    dec si"],
      [1352, "    cmp byte [ds:0], MCB_SIG_M"],
      [1354, "    cmp byte [ds:0], MCB_SIG_Z"],
      [1577, "    mov word [ds:1], 0"],
      [1578, "    call mcb_merge_free_forward"],
      [1585, ".resize_mem:"],
      [1592, "    mov [cs:rm_req], bx"],
      [1593, "    mov si, es"],
      [1361, "    mov ax, [ds:3]"],
      [1605, "    jae .rm_shrink"],
      [1613, "    cmp byte [es:0], MCB_SIG_M"],
      [1618, "    cmp word [es:1], 0"],
      [1622, "    add ax, cx"],
      [1525, "    cmp ax, bx"],
      [1637, "    mov byte [es:0], dl"],
      [1382, "    mov byte [ds:0], MCB_SIG_M"],
      [1381, "    mov word [es:3], cx"],
      [1644, "    mov word [ds:3], bx"],
      [1628, "    mov ax, [ds:3]"],
      [1658, "    mov byte [es:0], dl"],
      [1377, "    mov word [es:1], 0"],
      [1644, "    mov word [ds:3], bx"],
      [1706, "    mov [es:0x02], ax"],
      [1749, ".rm_cant_grow:"],
      [1527, "    mov bx, ax"],
  ...
  },
  {
    ...
    code: [
      ...
      [609, "    mov word [cs:exec_env_seg], 0"],
      [611, "    mov bx, ENV_PARAS"],
      [612, "    call alloc_mem_direct"],
      [614, "    mov [cs:exec_env_seg], ax"],
      [490, "    dec ax"],
      [470, "    mov ds, ax"],
      ...
      [660, "assign_exec_environment_owner:"],
      [664, "    mov bx, [cs:exec_env_seg]"],
      [669, "    dec bx"],
      [674, "    mov ax, [cs:cur_psp]"],
      [675, "    mov [ds:1], ax"]],
