...
const MEM_SECTIONS = [
  {
    ...
    code: [
      ...
      [10, "ENV_PARAS equ 16"],
      [11, "ENV_OWNER_TEMP equ 0xFFFF"],
      [12, "%if (MEM_TOP & 0xFF) != 0"],
      [13, "%error \"MEM_TOP must be 256-byte aligned\""],
      [15, "MCB_SIG_M equ 'M'"],
      [16, "MCB_SIG_Z equ 'Z'"],
    ],
    hi: [4, 6, 7, 8, 9, 11, 15, 16],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3299, "%if LOAD_SEG <= RELOC_SEG"],
      [3300, "%error \"LOAD_SEG must be above RELOC_SEG\""],
      [3302, "%if (kernel_end - kernel_entry) > ((LOAD_SEG - RELOC_SEG) * 16)"],
      [3303, "%error \"kernel exceeds boot relocation gap\""],
      [3305, "%if (kernel_end - kernel_entry) > ((SEC_BUF - RELOC_SEG) * 16)"],
      [3306, "%error \"kernel overlaps SEC_BUF\""],
      [3308, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3309, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3311, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3312, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3314, "%if (ROOT_SEG + ROOT_BUF_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3315, "%error \"ROOT buffer overlaps kernel stack\""],
      [3317, "%if (ROOT_SEG + ROOT_BUF_PARAS + STACK_ROOT_GUARD_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3318, "%error \"ROOT buffer leaves too little kernel stack guard\""],
      [3320, "%if (RELOC_SEG + (KERNEL_STACK_TOP / 16)) > MCB_START"],
      [3321, "%error \"kernel stack overlaps MCB arena\""],
      [3323, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3324, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3326, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3327, "%error \"EMS frame must be inside conventional arena\""],
      [3338, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3339, "%error \"XMS BIOS move backing must remain below 16 MiB\""],
    ],
    hi: [3302, 3305, 3308, 3311, 3317, 3320, 3326, 3338],
  ...
  },
  {
    ...
    code: [
      [390, "alloc_exec_environment:"],
      [393, "    mov word [cs:exec_env_seg], 0"],
      [395, "    mov bx, ENV_PARAS"],
      [396, "    call alloc_mem_direct"],
      [398, "    mov [cs:exec_env_seg], ax"],
      [401, "    dec ax"],
      [402, "    mov ds, ax"],
      [403, "    mov word [ds:1], ENV_OWNER_TEMP"],
      [415, "free_exec_environment:"],
      [423, "    dec ax"],
      [430, "    mov word [ds:1], 0"],
      [439, "assign_exec_environment_owner:"],
      [443, "    mov bx, [cs:exec_env_seg]"],
      [448, "    dec bx"],
      [455, "    mov ax, [cs:cur_psp]"],
      [456, "    mov [ds:1], ax"],
    ],
    hi: [390, 403, 415, 439, 455, 456],
    tests: ["scripts/test_envmcb.py", "scripts/test_execenv.py", "scripts/test_memrelease.py"],
  },
  {
    id: "terminate",
    ...
    code: [
      [2132, "do_terminate:"],
      [2141, "%if ENABLE_EMS"],
      [2142, "    mov word [cs:ems_alloc_pages], 0"],
      [2145, "    mov word [cs:xms_alloc_kb], 0"],
      [2146, "    call release_inherited_handles"],
      [2147, "    call close_owned_handles"],
      [2149, "    mov si, [cs:mcb_first]"],
      [2151, "    mov ds, si"],
      [2158, "    mov ax, [cs:cur_psp]"],
      [2159, "    cmp word [ds:1], ax"],
      [2161, "    mov word [ds:1], 0"],
      [2165, "    call mcb_walk_next"],
      [2169, "    call mcb_coalesce_all_free"],
      [2174, "    mov ax, [0x16]"],
      [2175, "    mov [cs:cur_psp], ax"],
    ],
    hi: [2132, 2145, 2149, 2159, 2161, 2169, 2175],
  ...
  },
  {
    ...
    code: [
      ...
      [1571, "init_xms_size:"],
      [1573, "    mov word [cs:xms_total_kb], 0"],
      [1574, "    mov ah, 0x88"],
      [1575, "    int 0x15"],
      [1579, "    cmp ax, XMS_MAX_KB"],
      [1581, "    mov ax, XMS_MAX_KB"],
      [1583, "    mov [cs:xms_total_kb], ax"],
      [1589, "int2f_handler:"],
      [1591, "    cmp ax, 0x4300"],
      [1593, "    cmp ax, 0x4310"],
      [1598, "    mov al, 0x80"],
      [1601, "    mov bx, xms_entry"],
      [1611, "xms_entry:"],
      [1614, "    cmp ah, 0x08"],
      [1616, "    cmp ah, 0x09"],
      [1618, "    cmp ah, 0x0A"],
      [1620, "    cmp ah, 0x0B"],
      [1637, "    mov ax, [cs:xms_total_kb]"],
      [1643, "    cmp word [cs:xms_alloc_kb], 0"],
      [1647, "    cmp dx, [cs:xms_total_kb]"],
      [1649, "    mov [cs:xms_alloc_kb], dx"],
      [1650, "    mov ax, 1"],
      [1651, "    mov dx, 1"],
      [1663, "    mov word [cs:xms_alloc_kb], 0"],
      [1667, ".move:"],
      [1676, "    test ax, 1"],
      [1684, "    call xms_prepare_endpoint"],
      [1692, "    call xms_prepare_endpoint"],
      [1738, "    mov ax, 0x8700"],
      [1743, "    int 0x15"],
    ],
    hi: [43, 88, 1571, 1598, 1601, 1611, 1649, 1667, 1743],
  ...
  },
  {
    ...
    code: [
      ...
      [1437, "%if ENABLE_EMS"],
      [1438, "    mov [es:0x67*4], word int67_handler"],
      [1440, "%else"],
      [1441, "    mov [es:0x67*4], word int67_absent_handler"],
      [1894, "%if !ENABLE_EMS"],
      [1895, "int67_absent_handler:"],
      [1896, "    mov ah, 0x80"],
      [1900, "%if ENABLE_EMS"],
      [1901, "int67_handler:"],
      [1925, ".frame:"],
      [1927, "    mov bx, EMS_FRAME_SEG"],
      [1929, ".pages:"],
      [1930, "    mov bx, EMS_TOTAL_PAGES"],
      [1935, ".alloc:"],
      [1940, "    cmp bx, EMS_TOTAL_PAGES"],
      [1942, "    mov [cs:ems_alloc_pages], bx"],
      [1950, ".map:"],
      [1955, "    cmp al, 3"],
      [1957, "    cmp bx, [cs:ems_alloc_pages]"],
      [1982, "    call ems_copy_16k"],
      [1993, "    call ems_copy_16k"],
      [2000, "    mov [cs:si+ems_map_pages], bx"],
      [2028, "    mov word [cs:ems_alloc_pages], 0"],
      [2052, "ems_clear_map:"],
      [2053, "    mov word [cs:ems_map_pages], 0xFFFF"],
    ],
    hi: [92, 1441, 1895, 1901, 1927, 1942, 1950, 1982, 2000],
...
function MemSection({ section }) {
  ...
  return (
    <section id={section.id} style={{ borderTop: `1px solid ${T.line}`, padding: "28px 0" }}>
      ...
      {section.body.map((p, i) => <p key={i} style={memP(T)}>{p}</p>)}
