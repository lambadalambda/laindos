...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4352, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4353, "%error \"kernel leaves too little HMA stack guard\""],
      [4355, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4356, "%error \"kernel exceeds boot load area\""],
      [4358, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4359, "%error \"FAT buffer overlaps CD_BUF\""],
      [4361, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4362, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4364, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4365, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4367, "%if (READ_CACHE_BUF + READ_CACHE_PARAS) > ROOT_SEG"],
      [4368, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4370, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4371, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4373, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4374, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4376, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4377, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4379, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4380, "%error \"CD cache overlaps MCB arena\""],
      [4382, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > SUBDIR_CACHE_BUF"],
      [4383, "%error \"CD cache overlaps subdirectory cache\""],
      [4385, "%if (SUBDIR_CACHE_BUF + (SUBDIR_CACHE_SLOTS * SUBDIR_CACHE_SLOT_PARAS)) > MCB_START"],
      [4386, "%error \"subdirectory cache overlaps MCB arena\""],
      [4388, "%if MCB_START >= MEM_TOP"],
      [4389, "%error \"MCB arena is empty\""],
      [4391, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4392, "%error \"EMS frame must be inside conventional arena\""],
      [4403, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4352, 4352, 4352, 4367, 4370, 4373, 4376, 4379, 4382, 4385, 4392],
