...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3948, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3949, "%error \"kernel leaves too little HMA stack guard\""],
      [3951, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3952, "%error \"kernel exceeds boot load area\""],
      [3954, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3955, "%error \"FAT buffer overlaps CD_BUF\""],
      [3957, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3958, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3960, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3961, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3963, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3964, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3966, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3967, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3969, "%if MCB_START >= MEM_TOP"],
      [3970, "%error \"MCB arena is empty\""],
      [3972, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3973, "%error \"EMS frame must be inside conventional arena\""],
      [3984, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [3948, 3951, 3954, 3948, 3948, 3948, 3951, 3954, 3957, 3984],
