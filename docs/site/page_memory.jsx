...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3712, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3713, "%error \"kernel leaves too little HMA stack guard\""],
      [3715, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3716, "%error \"kernel exceeds boot load area\""],
      [3718, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3719, "%error \"FAT buffer overlaps CD_BUF\""],
      [3721, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3722, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3724, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3725, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3727, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3728, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3730, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3731, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3733, "%if MCB_START >= MEM_TOP"],
      [3734, "%error \"MCB arena is empty\""],
      [3736, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3737, "%error \"EMS frame must be inside conventional arena\""],
      [3748, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      ...
      [1452, "    je .as_set"],
      [790, "    xor ax, ax"],
      [1544, "    mov al, [cs:alloc_strat]"],
      [1459, "    cmp bl, 2"],
      [1461, "    mov [cs:alloc_strat], bl"],
      [1498, ".alloc_mem:"],
      [1507, "    call mcb_chain_validate"],
      [1512, "    je .am_strat_best"],
      [1514, "    je .am_strat_high"],
      [1517, "    cmp word [cs:am_req], SMALL_ALLOC_HIGH_MAX"],
      [1518, "    jbe .am_strat_high"],
      [1519, ".am_strat_first:"],
      [1522, ".am_strat_best:"],
      [1525, ".am_strat_high:"],
      [1561, ".am_nomem:"],
      [1562, "    call find_largest_free_block"],
      [1563, "    mov ax, 8"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1598, "    MCB_IS_VALID"],
      [1622, "    MCB_IS_VALID"],
      [1603, "    mov word [ds:1], 0"],
      [1665, "    call mcb_merge_free_forward"],
      [1611, ".resize_mem:"],
      [1618, "    mov [cs:rm_req], bx"],
      [1619, "    mov si, es"],
      [1655, "    mov ax, [ds:3]"],
      [1629, "    jae .rm_shrink"],
      [1637, "    cmp byte [es:0], MCB_SIG_M"],
      [1642, "    cmp word [es:1], 0"],
      [1678, "    add ax, cx"],
      [1647, "    cmp ax, bx"],
      [1659, "    call mcb_split_low"],
      [1652, "    call mcb_split_low"],
      [1665, "    call mcb_merge_free_forward"],
      [1734, "    mov ax, [ds:3]"],
      [1659, "    call mcb_split_low"],
      [1665, "    call mcb_merge_free_forward"],
  ...
  },
  {
    ...
    code: [
      ...
      [779, "assign_exec_environment_owner:"],
      [783, "    mov bx, [cs:exec_env_seg]"],
      [788, "    dec bx"],
      [793, "    mov ax, [cs:cur_psp]"],
  ...
  },
  {
    ...
    code: [
      ...
      [2483, "    call release_inherited_handles"],
      [2484, "    call close_owned_handles"],
      [2548, "    mov si, [cs:mcb_first]"],
      [2523, "    mov ds, si"],
      [2551, "    mov ax, [cs:cur_psp]"],
      [2449, "    cmp word [ds:1], ax"],
      [2558, "    mov word [ds:1], 0"],
      [2446, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
      [2454, "    call mcb_coalesce_all_free"],
      [2459, "    mov ax, [0x16]"],
