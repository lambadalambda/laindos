...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4283, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4284, "%error \"kernel leaves too little HMA stack guard\""],
      [4286, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4287, "%error \"kernel exceeds boot load area\""],
      [4289, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4290, "%error \"FAT buffer overlaps CD_BUF\""],
      [4292, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4293, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4295, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4296, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4298, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [4299, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4301, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4302, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4304, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4305, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4307, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4308, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4310, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4311, "%error \"CD cache overlaps MCB arena\""],
      [4313, "%if MCB_START >= MEM_TOP"],
      [4314, "%error \"MCB arena is empty\""],
      [4316, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4317, "%error \"EMS frame must be inside conventional arena\""],
      [4328, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4283, 4286, 4289, 4304, 4307, 4310, 4313, 4316, 4328],
