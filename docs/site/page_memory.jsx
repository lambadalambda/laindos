...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4688, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4689, "%error \"kernel leaves too little HMA stack guard\""],
      [4691, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4692, "%error \"kernel exceeds boot load area\""],
      [4694, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4695, "%error \"FAT buffer overlaps CD_BUF\""],
      [4697, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4698, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4700, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4701, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4703, "%if (READ_CACHE_BUF + READ_CACHE_PARAS) > ROOT_SEG"],
      [4704, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4706, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4707, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4709, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4710, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4712, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4713, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4715, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4716, "%error \"CD cache overlaps MCB arena\""],
      [4718, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > SUBDIR_CACHE_BUF"],
      [4719, "%error \"CD cache overlaps subdirectory cache\""],
      [4721, "%if (SUBDIR_CACHE_BUF + (SUBDIR_CACHE_SLOTS * SUBDIR_CACHE_SLOT_PARAS)) > MCB_START"],
      [4722, "%error \"subdirectory cache overlaps MCB arena\""],
      [4724, "%if MCB_START >= MEM_TOP"],
      [4725, "%error \"MCB arena is empty\""],
      [4727, "%if ENABLE_EMS && EMS_FRAME_SEG < 0xD000"],
      [4728, "%error \"EMS frame must use an upper-memory window at D000h or above\""],
      [4739, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4688, 4691, 4694, 4697, 4700, 4703, 4706, 4709, 4712, 4715, 4718, 4721, 4724, 4727, 4739],
  ...
  },
  {
    ...
    code: [
      [2727, "do_terminate:"],
      [2737, "%if ENABLE_EMS"],
      [4664, "    mov word [cs:ems_alloc_pages], 0"],
      [2740, "    mov word [cs:xms_alloc_kb], 0"],
      [2741, "    call release_inherited_handles"],
      [2742, "    call close_owned_handles"],
      [2744, "    mov si, [cs:mcb_first]"],
      [2745, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
      [2703, "    mov ax, [cs:cur_psp]"],
      [2748, "    cmp word [ds:1], ax"],
      [2750, "    mov word [ds:1], 0"],
      [2753, "    call mcb_coalesce_all_free"],
      [2758, "    mov ax, [0x16]"],
      [{a: "do_terminate_restore_parent_psp"}, "    mov [cs:cur_psp], ax"]],
    hi: [2727, 2737, 4664, 2740, 2744, 2748, 2750, 2753, {a: "do_terminate_restore_parent_psp"}],
  ...
  },
  {
    ...
    code: [
      ...
      [4395, "init_xms_store:"],
      [4400, "    cmp ax, (EMS_TOTAL_PAGES * 16)"],
      [4405, "    mov ax, XMS_MAX_KB - 64"],
      [4416, "    mov [cs:ems_backing_hi], bx"],
      [4412, "    mov [cs:xms_total_kb], ax"],
      [2069, "int2f_handler:"],
      [2083, "    cmp ax, 0x4300"],
      [2085, "    cmp ax, 0x4310"],
      [2091, "    mov al, 0x80"],
    ...
    hi: [43, 113, 2032, 4395, 2083, 2094, 2107, 2173, 2193, {a: "xms_move_bios_call"}],
  ...
  },
  {
    ...
    code: [
      ...
      [2435, "int67_handler:"],
      [2464, ".frame:"],
      [2466, "    mov bx, EMS_FRAME_SEG"],
      [2468, ".pages:"],
      [2469, "    mov bx, [cs:ems_total_pages]"],
      [2474, ".alloc:"],
      [2477, "    mov ax, [cs:ems_total_pages]"],
      [2478, "    sub ax, [cs:ems_alloc_pages]"],
      [2479, "    cmp bx, ax"],
      [2481, "    call ems_find_free_handle"],
      [2483, "    call ems_find_page_run"],
      [2485, "    mov [cs:si+ems_handle_pages], bx"],
      [2486, "    mov [cs:si+ems_handle_base], ax"],
      [2487, "    add [cs:ems_alloc_pages], bx"],
      [2497, ".map:"],
      [2498, "    call ems_handle_offset"],
      [2500, "    cmp al, 3"],
      [2502, "    cmp bx, [cs:si+ems_handle_pages]"],
      [2504, "    push bx"],
      [2505, "    add bx, [cs:si+ems_handle_base]"],
      [2514, "    cmp bx, [cs:si+ems_map_pages]"],
      [2516, "    je .map_already"],
      [2540, "    call ems_copy_16k"],
      [2558, "    mov [cs:si+ems_map_pages], bx"],
      [2569, ".map_already:"],
      [2587, ".free:"],
      [2590, "    call ems_clear_handle_maps"],
      [2591, "    mov ax, [cs:si+ems_handle_base]"],
      [2593, "    call ems_clear_owner_pages"],
      [2594, "    sub [cs:ems_alloc_pages], bx"],
      [2605, "    mov bx, EMS_MAX_HANDLES"],
      [2610, "    mov bx, [cs:si+ems_handle_pages]"],
      [4664, "    mov word [cs:ems_alloc_pages], 0"],
      [4665, "    mov di, ems_handle_pages"],
      [4668, "    mov di, ems_handle_base"],
      [4671, "    mov di, ems_page_owner"],
      [2617, "ems_clear_map:"],
      [{a: "ems_clear_map_fill"}, "    mov word [cs:ems_map_pages], 0xFFFF"]],
    hi: [117, 1837, 2427, 2433, 2474, 2481, 2483, 2485, 2487, 2498, 2502, 2504, 2516, 2569, 2587, 2590, 2594, 2605, 4665, 4668, 4671],
    tests: ["scripts/test_ems.py", "scripts/test_emsmulti.py", "scripts/test_emsmem.py", "scripts/test_emslarge.py", "scripts/test_emsxms.py"],
  },
  {
    id: "report",
