...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3716, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3717, "%error \"kernel leaves too little HMA stack guard\""],
      [3719, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3720, "%error \"kernel exceeds boot load area\""],
      [3722, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3723, "%error \"FAT buffer overlaps CD_BUF\""],
      [3725, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3726, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3728, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3729, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3731, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3732, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3734, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3735, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3737, "%if MCB_START >= MEM_TOP"],
      [3738, "%error \"MCB arena is empty\""],
      [3740, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3741, "%error \"EMS frame must be inside conventional arena\""],
      [3752, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      ...
      [1399, "    je .as_set"],
      [751, "    xor ax, ax"],
      [1528, "    mov al, [cs:alloc_strat]"],
      [1406, "    cmp bl, 2"],
      [1408, "    mov [cs:alloc_strat], bl"],
      [1445, ".alloc_mem:"],
      [1528, "    mov al, [cs:alloc_strat]"],
      [1133, "    cmp al, 2"],
      [1468, "    jmp near .am_find_last"],
      [1460, "    cmp al, 1"],
      [1462, "    jmp near .am_find_best"],
      [1464, "    cmp word [cs:am_req], 1"],
      [1466, "    cmp word [cs:am_req], SMALL_ALLOC_HIGH_MAX"],
      [1468, "    jmp near .am_find_last"],
      [1578, "    mov si, [cs:mcb_first]"],
      [1591, "    cmp ax, [cs:am_req]"],
      [1485, ".am_use:"],
      [1108, "    mov ax, [cs:cur_psp]"],
      [1551, ".am_find_last:"],
      [1575, ".am_find_best:"],
      [1632, ".am_nomem:"],
      [1635, ".am_scan_largest:"],
      [1872, "    mov ax, 8"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1714, "    mov si, es"],
      [1715, "    dec si"],
      [1581, "    cmp byte [ds:0], MCB_SIG_M"],
      [1598, "    cmp byte [ds:0], MCB_SIG_Z"],
      [1698, "    mov word [ds:1], 0"],
      [1699, "    call mcb_merge_free_forward"],
      [1706, ".resize_mem:"],
      [1713, "    mov [cs:rm_req], bx"],
      [1714, "    mov si, es"],
      [1590, "    mov ax, [ds:3]"],
      [1726, "    jae .rm_shrink"],
      [1734, "    cmp byte [es:0], MCB_SIG_M"],
      [1739, "    cmp word [es:1], 0"],
      [1826, "    add ax, cx"],
      [1744, "    cmp ax, bx"],
      [1779, "    mov byte [es:0], dl"],
      [1622, "    mov byte [ds:0], MCB_SIG_M"],
      [1502, "    mov word [es:3], cx"],
      [1784, "    mov word [ds:3], bx"],
      [1769, "    mov ax, [ds:3]"],
      [1779, "    mov byte [es:0], dl"],
      [1498, "    mov word [es:1], 0"],
      [1784, "    mov word [ds:3], bx"],
      [1827, "    mov [es:0x02], ax"],
      [1870, ".rm_cant_grow:"],
      [1871, "    mov bx, ax"],
  ...
  },
  {
    ...
    code: [
      [2411, "do_terminate:"],
      [2421, "%if ENABLE_EMS"],
      [2278, "    mov word [cs:ems_alloc_pages], 0"],
      [2425, "    mov word [cs:xms_alloc_kb], 0"],
      [2426, "    call release_inherited_handles"],
      [2427, "    call close_owned_handles"],
      [2429, "    mov si, [cs:mcb_first]"],
      [2431, "    mov ds, si"],
      [2438, "    mov ax, [cs:cur_psp]"],
      [2439, "    cmp word [ds:1], ax"],
      [2441, "    mov word [ds:1], 0"],
      [2445, "    call mcb_walk_next"],
      [2449, "    call mcb_coalesce_all_free"],
      [2454, "    mov ax, [0x16]"],
  ...
  },
  {
    ...
    code: [
      ...
      [1761, "init_xms_size:"],
      [1763, "    mov word [cs:xms_total_kb], 0"],
      [1764, "    mov ah, 0x88"],
      [1765, "    int 0x15"],
      [1770, "    cmp ax, XMS_MAX_KB - 64"],
      [1772, "    mov ax, XMS_MAX_KB - 64"],
      [1774, "    mov [cs:xms_total_kb], ax"],
      [1796, "int2f_handler:"],
      [1806, "    cmp ax, 0x4300"],
      [1808, "    cmp ax, 0x4310"],
      [1813, "    mov al, 0x80"],
      [1816, "    mov bx, xms_entry"],
      [1826, "xms_entry:"],
      [1843, "    cmp ah, 0x08"],
      [1845, "    cmp ah, 0x09"],
      [1847, "    cmp ah, 0x0A"],
      [1849, "    cmp ah, 0x0B"],
      [1886, "    mov ax, [cs:xms_total_kb]"],
      [1910, "    cmp word [cs:xms_alloc_kb], 0"],
      [1896, "    cmp dx, [cs:xms_total_kb]"],
      [1898, "    mov [cs:xms_alloc_kb], dx"],
      [1913, "    mov ax, 1"],
      [1900, "    mov dx, 1"],
      [1912, "    mov word [cs:xms_alloc_kb], 0"],
      [1916, ".move:"],
      [1925, "    test ax, 1"],
      [1933, "    call xms_prepare_endpoint"],
      [1933, "    call xms_prepare_endpoint"],
      [1987, "    mov ax, 0x8700"],
  ...
  },
  {
    ...
    code: [
      ...
      [1627, "%if ENABLE_EMS"],
      [1628, "    mov [es:0x67*4], word int67_handler"],
      [1630, "%else"],
      [1631, "    mov [es:0x67*4], word int67_absent_handler"],
      [2144, "%if !ENABLE_EMS"],
      [2145, "int67_absent_handler:"],
      [2146, "    mov ah, 0x80"],
      [2150, "%if ENABLE_EMS"],
      [2151, "int67_handler:"],
      [2175, ".frame:"],
      [2177, "    mov bx, EMS_FRAME_SEG"],
      [2179, ".pages:"],
      [2180, "    mov bx, EMS_TOTAL_PAGES"],
      [2185, ".alloc:"],
      [2190, "    cmp bx, EMS_TOTAL_PAGES"],
      [2192, "    mov [cs:ems_alloc_pages], bx"],
      [2200, ".map:"],
      [2205, "    cmp al, 3"],
      [2207, "    cmp bx, [cs:ems_alloc_pages]"],
      [2232, "    call ems_copy_16k"],
      [2232, "    call ems_copy_16k"],
      [2250, "    mov [cs:si+ems_map_pages], bx"],
      [2278, "    mov word [cs:ems_alloc_pages], 0"],
      [2302, "ems_clear_map:"],
