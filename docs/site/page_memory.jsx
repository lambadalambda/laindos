...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3737, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3738, "%error \"kernel leaves too little HMA stack guard\""],
      [3740, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3741, "%error \"kernel exceeds boot load area\""],
      [3743, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3744, "%error \"FAT buffer overlaps CD_BUF\""],
      [3746, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3747, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3749, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3750, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3752, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3753, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3755, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3756, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3758, "%if MCB_START >= MEM_TOP"],
      [3759, "%error \"MCB arena is empty\""],
      [3761, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3762, "%error \"EMS frame must be inside conventional arena\""],
      [3773, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
