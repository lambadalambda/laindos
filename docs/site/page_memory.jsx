...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3743, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3744, "%error \"kernel leaves too little HMA stack guard\""],
      [3746, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3747, "%error \"kernel exceeds boot load area\""],
      [3749, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3750, "%error \"FAT buffer overlaps CD_BUF\""],
      [3752, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3753, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3755, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3756, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3758, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3759, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3761, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3762, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3764, "%if MCB_START >= MEM_TOP"],
      [3765, "%error \"MCB arena is empty\""],
      [3767, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3768, "%error \"EMS frame must be inside conventional arena\""],
      [3779, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      ...
      [1447, "    je .as_set"],
      [785, "    xor ax, ax"],
      [1576, "    mov al, [cs:alloc_strat]"],
      [1454, "    cmp bl, 2"],
      [1456, "    mov [cs:alloc_strat], bl"],
      [1493, ".alloc_mem:"],
      [1576, "    mov al, [cs:alloc_strat]"],
      [1179, "    cmp al, 2"],
      [1516, "    jmp near .am_find_last"],
      [1508, "    cmp al, 1"],
      [1510, "    jmp near .am_find_best"],
      [1512, "    cmp word [cs:am_req], 1"],
      [1514, "    cmp word [cs:am_req], SMALL_ALLOC_HIGH_MAX"],
      [1516, "    jmp near .am_find_last"],
      [1626, "    mov si, [cs:mcb_first]"],
      [1639, "    cmp ax, [cs:am_req]"],
      [1533, ".am_use:"],
      [1154, "    mov ax, [cs:cur_psp]"],
      [1599, ".am_find_last:"],
      [1623, ".am_find_best:"],
      [1680, ".am_nomem:"],
      [1683, ".am_scan_largest:"],
      [1920, "    mov ax, 8"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1762, "    mov si, es"],
      [1763, "    dec si"],
      [1629, "    cmp byte [ds:0], MCB_SIG_M"],
      [1646, "    cmp byte [ds:0], MCB_SIG_Z"],
      [1746, "    mov word [ds:1], 0"],
      [1747, "    call mcb_merge_free_forward"],
      [1754, ".resize_mem:"],
      [1761, "    mov [cs:rm_req], bx"],
      [1762, "    mov si, es"],
      [1638, "    mov ax, [ds:3]"],
      [1774, "    jae .rm_shrink"],
      [1782, "    cmp byte [es:0], MCB_SIG_M"],
      [1787, "    cmp word [es:1], 0"],
      [1874, "    add ax, cx"],
      [1792, "    cmp ax, bx"],
      [1827, "    mov byte [es:0], dl"],
      [1670, "    mov byte [ds:0], MCB_SIG_M"],
      [1550, "    mov word [es:3], cx"],
      [1832, "    mov word [ds:3], bx"],
      [1817, "    mov ax, [ds:3]"],
      [1827, "    mov byte [es:0], dl"],
      [1546, "    mov word [es:1], 0"],
      [1832, "    mov word [ds:3], bx"],
      [1875, "    mov [es:0x02], ax"],
      [1918, ".rm_cant_grow:"],
      [1919, "    mov bx, ax"],
