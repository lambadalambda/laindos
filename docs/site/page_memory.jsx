...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4346, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4347, "%error \"kernel leaves too little HMA stack guard\""],
      [4349, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4350, "%error \"kernel exceeds boot load area\""],
      [4352, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4353, "%error \"FAT buffer overlaps CD_BUF\""],
      [4355, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4356, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4358, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4359, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4361, "%if (READ_CACHE_BUF + READ_CACHE_PARAS) > ROOT_SEG"],
      [4362, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4364, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4365, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4367, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4368, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4370, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4371, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4373, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4374, "%error \"CD cache overlaps MCB arena\""],
      [4376, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > SUBDIR_CACHE_BUF"],
      [4377, "%error \"CD cache overlaps subdirectory cache\""],
      [4379, "%if (SUBDIR_CACHE_BUF + (SUBDIR_CACHE_SLOTS * SUBDIR_CACHE_SLOT_PARAS)) > MCB_START"],
      [4380, "%error \"subdirectory cache overlaps MCB arena\""],
      [4382, "%if MCB_START >= MEM_TOP"],
      [4383, "%error \"MCB arena is empty\""],
      [4385, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4386, "%error \"EMS frame must be inside conventional arena\""],
      [4397, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4346, 4349, 4352, 4367, 4370, 4373, 4376, 4379, 4382, 4385, 4397],
