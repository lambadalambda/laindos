...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    code: [
      ...
      [197, "    mov word [es:1], 0"],
      [198, "    mov ax, MEM_TOP - MCB_START - 1"],
      [199, "    mov word [es:3], ax"],
      [200, "    mov word [mcb_first], MCB_START"],
      [201, "    mov word [cur_psp], 0"]],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3745, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3746, "%error \"kernel leaves too little HMA stack guard\""],
      [3748, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3749, "%error \"kernel exceeds boot load area\""],
      [3751, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3752, "%error \"FAT buffer overlaps CD_BUF\""],
      [3754, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3755, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3757, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3758, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3760, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3761, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3763, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3764, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3766, "%if MCB_START >= MEM_TOP"],
      [3767, "%error \"MCB arena is empty\""],
      [3769, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3770, "%error \"EMS frame must be inside conventional arena\""],
      [3781, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      ...
      [1452, "    je .as_set"],
      [790, "    xor ax, ax"],
      [1581, "    mov al, [cs:alloc_strat]"],
      [1459, "    cmp bl, 2"],
      [1461, "    mov [cs:alloc_strat], bl"],
      [1498, ".alloc_mem:"],
      [1581, "    mov al, [cs:alloc_strat]"],
      [1184, "    cmp al, 2"],
      [1521, "    jmp near .am_find_last"],
      [1513, "    cmp al, 1"],
      [1515, "    jmp near .am_find_best"],
      [1517, "    cmp word [cs:am_req], 1"],
      [1519, "    cmp word [cs:am_req], SMALL_ALLOC_HIGH_MAX"],
      [1521, "    jmp near .am_find_last"],
      [1631, "    mov si, [cs:mcb_first]"],
      [1644, "    cmp ax, [cs:am_req]"],
      [1538, ".am_use:"],
      [1159, "    mov ax, [cs:cur_psp]"],
      [1604, ".am_find_last:"],
      [1628, ".am_find_best:"],
      [1685, ".am_nomem:"],
      [1688, ".am_scan_largest:"],
      [1925, "    mov ax, 8"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1767, "    mov si, es"],
      [1768, "    dec si"],
      [1634, "    cmp byte [ds:0], MCB_SIG_M"],
      [1651, "    cmp byte [ds:0], MCB_SIG_Z"],
      [1751, "    mov word [ds:1], 0"],
      [1752, "    call mcb_merge_free_forward"],
      [1759, ".resize_mem:"],
      [1766, "    mov [cs:rm_req], bx"],
      [1767, "    mov si, es"],
      [1643, "    mov ax, [ds:3]"],
      [1779, "    jae .rm_shrink"],
      [1787, "    cmp byte [es:0], MCB_SIG_M"],
      [1792, "    cmp word [es:1], 0"],
      [1879, "    add ax, cx"],
      [1797, "    cmp ax, bx"],
      [1832, "    mov byte [es:0], dl"],
      [1675, "    mov byte [ds:0], MCB_SIG_M"],
      [1555, "    mov word [es:3], cx"],
      [1837, "    mov word [ds:3], bx"],
      [1822, "    mov ax, [ds:3]"],
      [1832, "    mov byte [es:0], dl"],
      [1551, "    mov word [es:1], 0"],
      [1837, "    mov word [ds:3], bx"],
      [1880, "    mov [es:0x02], ax"],
      [1923, ".rm_cant_grow:"],
      [1924, "    mov bx, ax"],
  ...
  },
  {
    ...
    code: [
      ...
      [723, "    mov word [cs:exec_env_seg], 0"],
      [547, "    mov bx, ENV_PARAS"],
      [729, "    call alloc_mem_direct"],
      [731, "    mov [cs:exec_env_seg], ax"],
      [734, "    dec ax"],
      [735, "    mov ds, ax"],
      ...
      [777, "assign_exec_environment_owner:"],
      [781, "    mov bx, [cs:exec_env_seg]"],
      [786, "    dec bx"],
      [791, "    mov ax, [cs:cur_psp]"],
  ...
  },
  {
    ...
    code: [
      [2427, "do_terminate:"],
      [2437, "%if ENABLE_EMS"],
      [2294, "    mov word [cs:ems_alloc_pages], 0"],
      [2441, "    mov word [cs:xms_alloc_kb], 0"],
      [2494, "    call release_inherited_handles"],
      [2495, "    call close_owned_handles"],
      [2571, "    mov si, [cs:mcb_first]"],
      [2515, "    mov ds, si"],
      [2548, "    mov ax, [cs:cur_psp]"],
      [2455, "    cmp word [ds:1], ax"],
      [2586, "    mov word [ds:1], 0"],
      [2594, "    call mcb_walk_next"],
      [2465, "    call mcb_coalesce_all_free"],
      [2470, "    mov ax, [0x16]"],
  ...
  },
  {
    ...
    code: [
      ...
      [1773, "init_xms_size:"],
      [1775, "    mov word [cs:xms_total_kb], 0"],
      [1776, "    mov ah, 0x88"],
      [1777, "    int 0x15"],
      [1782, "    cmp ax, XMS_MAX_KB - 64"],
      [1784, "    mov ax, XMS_MAX_KB - 64"],
      [1786, "    mov [cs:xms_total_kb], ax"],
      [1808, "int2f_handler:"],
      [1818, "    cmp ax, 0x4300"],
      [1820, "    cmp ax, 0x4310"],
      [1835, "    mov al, 0x80"],
      [1829, "    mov bx, xms_entry"],
      [1842, "xms_entry:"],
      [1859, "    cmp ah, 0x08"],
      [1861, "    cmp ah, 0x09"],
      [1863, "    cmp ah, 0x0A"],
      [1865, "    cmp ah, 0x0B"],
      [1902, "    mov ax, [cs:xms_total_kb]"],
      [2051, "    cmp word [cs:xms_alloc_kb], 0"],
      [1912, "    cmp dx, [cs:xms_total_kb]"],
      [1914, "    mov [cs:xms_alloc_kb], dx"],
      [2028, "    mov ax, 1"],
      [1916, "    mov dx, 1"],
      [1928, "    mov word [cs:xms_alloc_kb], 0"],
      [1932, ".move:"],
      [1941, "    test ax, 1"],
      [1957, "    call xms_prepare_endpoint"],
      [1957, "    call xms_prepare_endpoint"],
      [2003, "    mov ax, 0x8700"],
  ...
  },
  {
    ...
    code: [
      ...
      [1573, "%if ENABLE_EMS"],
      [1574, "    mov [es:0x67*4], word int67_handler"],
      [1576, "%else"],
      [1577, "    mov [es:0x67*4], word int67_absent_handler"],
      [2160, "%if !ENABLE_EMS"],
      [2161, "int67_absent_handler:"],
      [2186, "    mov ah, 0x80"],
      [2166, "%if ENABLE_EMS"],
      [2167, "int67_handler:"],
      [2191, ".frame:"],
      [2193, "    mov bx, EMS_FRAME_SEG"],
      [2195, ".pages:"],
      [2196, "    mov bx, EMS_TOTAL_PAGES"],
      [2201, ".alloc:"],
      [2206, "    cmp bx, EMS_TOTAL_PAGES"],
      [2208, "    mov [cs:ems_alloc_pages], bx"],
      [2216, ".map:"],
      [2221, "    cmp al, 3"],
      [2223, "    cmp bx, [cs:ems_alloc_pages]"],
      [2259, "    call ems_copy_16k"],
      [2259, "    call ems_copy_16k"],
      [2266, "    mov [cs:si+ems_map_pages], bx"],
      [2294, "    mov word [cs:ems_alloc_pages], 0"],
      [2318, "ems_clear_map:"],
