...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4321, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4322, "%error \"kernel leaves too little HMA stack guard\""],
      [4324, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4325, "%error \"kernel exceeds boot load area\""],
      [4327, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4328, "%error \"FAT buffer overlaps CD_BUF\""],
      [4330, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4331, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4333, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4334, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4336, "%if (READ_CACHE_BUF + READ_CACHE_PARAS) > ROOT_SEG"],
      [4337, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4339, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4340, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4342, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4343, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4345, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4346, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4348, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4349, "%error \"CD cache overlaps MCB arena\""],
      [4351, "%if MCB_START >= MEM_TOP"],
      [4352, "%error \"MCB arena is empty\""],
      [4354, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4355, "%error \"EMS frame must be inside conventional arena\""],
      [4366, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4321, 4324, 4327, 4342, 4345, 4348, 4351, 4354, 4366],
