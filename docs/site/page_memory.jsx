...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3638, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3639, "%error \"kernel leaves too little HMA stack guard\""],
      [3641, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3642, "%error \"kernel exceeds boot load area\""],
      [3644, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3645, "%error \"FAT buffer overlaps CD_BUF\""],
      [3647, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3648, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3650, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3651, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3653, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3654, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3656, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3657, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3659, "%if MCB_START >= MEM_TOP"],
      [3660, "%error \"MCB arena is empty\""],
      [3662, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3663, "%error \"EMS frame must be inside conventional arena\""],
      [3674, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3675, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
  ...
  },
  {
    ...
    code: [
      [2357, "do_terminate:"],
      [2366, "%if ENABLE_EMS"],
      [2253, "    mov word [cs:ems_alloc_pages], 0"],
      [2370, "    mov word [cs:xms_alloc_kb], 0"],
      [2371, "    call release_inherited_handles"],
      [2372, "    call close_owned_handles"],
      [2374, "    mov si, [cs:mcb_first]"],
      [2376, "    mov ds, si"],
      [2383, "    mov ax, [cs:cur_psp]"],
      [2384, "    cmp word [ds:1], ax"],
      [2386, "    mov word [ds:1], 0"],
      [2390, "    call mcb_walk_next"],
      [2394, "    call mcb_coalesce_all_free"],
      [2399, "    mov ax, [0x16]"],
      [2400, "    mov [cs:cur_psp], ax"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1737, "init_xms_size:"],
      [1739, "    mov word [cs:xms_total_kb], 0"],
      [1740, "    mov ah, 0x88"],
      [1741, "    int 0x15"],
      [1746, "    cmp ax, XMS_MAX_KB - 64"],
      [1748, "    mov ax, XMS_MAX_KB - 64"],
      [1750, "    mov [cs:xms_total_kb], ax"],
      [1772, "int2f_handler:"],
      [1782, "    cmp ax, 0x4300"],
      [1784, "    cmp ax, 0x4310"],
      [1789, "    mov al, 0x80"],
      [1792, "    mov bx, xms_entry"],
      [1802, "xms_entry:"],
      [1819, "    cmp ah, 0x08"],
      [1821, "    cmp ah, 0x09"],
      [1823, "    cmp ah, 0x0A"],
      [1825, "    cmp ah, 0x0B"],
      [1862, "    mov ax, [cs:xms_total_kb]"],
      [1886, "    cmp word [cs:xms_alloc_kb], 0"],
      [1872, "    cmp dx, [cs:xms_total_kb]"],
      [1874, "    mov [cs:xms_alloc_kb], dx"],
      [1889, "    mov ax, 1"],
      [1876, "    mov dx, 1"],
      [1888, "    mov word [cs:xms_alloc_kb], 0"],
      [1892, ".move:"],
      [1901, "    test ax, 1"],
      [1909, "    call xms_prepare_endpoint"],
      [1909, "    call xms_prepare_endpoint"],
      [1963, "    mov ax, 0x8700"],
      [1968, "    int 0x15"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1603, "%if ENABLE_EMS"],
      [1604, "    mov [es:0x67*4], word int67_handler"],
      [1606, "%else"],
      [1607, "    mov [es:0x67*4], word int67_absent_handler"],
      [2119, "%if !ENABLE_EMS"],
      [2120, "int67_absent_handler:"],
      [2121, "    mov ah, 0x80"],
      [2125, "%if ENABLE_EMS"],
      [2126, "int67_handler:"],
      [2150, ".frame:"],
      [2152, "    mov bx, EMS_FRAME_SEG"],
      [2154, ".pages:"],
      [2155, "    mov bx, EMS_TOTAL_PAGES"],
      [2160, ".alloc:"],
      [2165, "    cmp bx, EMS_TOTAL_PAGES"],
      [2167, "    mov [cs:ems_alloc_pages], bx"],
      [2175, ".map:"],
      [2180, "    cmp al, 3"],
      [2182, "    cmp bx, [cs:ems_alloc_pages]"],
      [2207, "    call ems_copy_16k"],
      [2207, "    call ems_copy_16k"],
      [2225, "    mov [cs:si+ems_map_pages], bx"],
      [2253, "    mov word [cs:ems_alloc_pages], 0"],
      [2277, "ems_clear_map:"],
      [2278, "    mov word [cs:ems_map_pages], 0xFFFF"]],
