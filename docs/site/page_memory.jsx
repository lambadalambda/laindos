...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4363, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4364, "%error \"kernel leaves too little HMA stack guard\""],
      [4366, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4367, "%error \"kernel exceeds boot load area\""],
      [4369, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4370, "%error \"FAT buffer overlaps CD_BUF\""],
      [4372, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4373, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4375, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4376, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4378, "%if (READ_CACHE_BUF + READ_CACHE_PARAS) > ROOT_SEG"],
      [4379, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4381, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4382, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4384, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4385, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4387, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4388, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4390, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4391, "%error \"CD cache overlaps MCB arena\""],
      [4393, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > SUBDIR_CACHE_BUF"],
      [4394, "%error \"CD cache overlaps subdirectory cache\""],
      [4396, "%if (SUBDIR_CACHE_BUF + (SUBDIR_CACHE_SLOTS * SUBDIR_CACHE_SLOT_PARAS)) > MCB_START"],
      [4397, "%error \"subdirectory cache overlaps MCB arena\""],
      [4399, "%if MCB_START >= MEM_TOP"],
      [4400, "%error \"MCB arena is empty\""],
      [4402, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4403, "%error \"EMS frame must be inside conventional arena\""],
      [4414, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4363, 4363, 4363, 4378, 4381, 4384, 4387, 4390, 4393, 4396, 4403],
  ...
  },
  {
    ...
    code: [
      [2695, "do_terminate:"],
      [2705, "%if ENABLE_EMS"],
      [2706, "    mov word [cs:ems_alloc_pages], 0"],
      [2709, "    mov word [cs:xms_alloc_kb], 0"],
      [2710, "    call release_inherited_handles"],
      [2711, "    call close_owned_handles"],
      [2713, "    mov si, [cs:mcb_first]"],
      [2772, "    mov ds, si"],
      [2671, "    mov ax, [cs:cur_psp]"],
      [2717, "    cmp word [ds:1], ax"],
      [2719, "    mov word [ds:1], 0"],
      [2714, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
      [2722, "    call mcb_coalesce_all_free"],
      [2727, "    mov ax, [0x16]"],
      [{a: "do_terminate_restore_parent_psp"}, "    mov [cs:cur_psp], ax"]],
    hi: [2695, 2705, 2705, 2705, 2695, 2710, {a: "do_terminate_restore_parent_psp"}],
  ...
  },
  {
    ...
    hi: [43, 113, 2037, 2086, 2097, 2110, 2176, 2196, {a: "xms_move_bios_call"}],
  ...
  },
  {
    ...
    code: [
      ...
      [2428, "%if !ENABLE_EMS"],
      [2429, "int67_absent_handler:"],
      [2430, "    mov ah, 0x80"],
      [2434, "%if ENABLE_EMS"],
      [2435, "int67_handler:"],
      [2459, ".frame:"],
      [2461, "    mov bx, EMS_FRAME_SEG"],
      [2463, ".pages:"],
      [2464, "    mov bx, EMS_TOTAL_PAGES"],
      [2469, ".alloc:"],
      [2474, "    cmp bx, EMS_TOTAL_PAGES"],
      [2476, "    mov [cs:ems_alloc_pages], bx"],
      [2484, ".map:"],
      [2489, "    cmp al, 3"],
      [2491, "    cmp bx, [cs:ems_alloc_pages]"],
      [2516, "    call ems_copy_16k"],
      [2527, "    call ems_copy_16k"],
      [2534, "    mov [cs:si+ems_map_pages], bx"],
      [2562, "    mov word [cs:ems_alloc_pages], 0"],
      [2586, "ems_clear_map:"],
      [{a: "ems_clear_map_fill"}, "    mov word [cs:ems_map_pages], 0xFFFF"]],
    hi: [117, 1837, 2429, 2435, 2459, 2474, 2474, 2435, 2534],
