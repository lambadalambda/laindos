...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3750, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3751, "%error \"kernel leaves too little HMA stack guard\""],
      [3753, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3754, "%error \"kernel exceeds boot load area\""],
      [3756, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3757, "%error \"FAT buffer overlaps CD_BUF\""],
      [3759, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3760, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3762, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3763, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3765, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3766, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3768, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3769, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3771, "%if MCB_START >= MEM_TOP"],
      [3772, "%error \"MCB arena is empty\""],
      [3774, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3775, "%error \"EMS frame must be inside conventional arena\""],
      [3786, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      [2436, "do_terminate:"],
      [2446, "%if ENABLE_EMS"],
      [2303, "    mov word [cs:ems_alloc_pages], 0"],
      [2450, "    mov word [cs:xms_alloc_kb], 0"],
      [2451, "    call release_inherited_handles"],
      [2452, "    call close_owned_handles"],
      [2454, "    mov si, [cs:mcb_first]"],
      [2456, "    mov ds, si"],
      [2463, "    mov ax, [cs:cur_psp]"],
      [2464, "    cmp word [ds:1], ax"],
      [2466, "    mov word [ds:1], 0"],
      [2470, "    call mcb_walk_next"],
      [2474, "    call mcb_coalesce_all_free"],
      [2479, "    mov ax, [0x16]"],
  ...
  },
  {
    ...
    code: [
      ...
      [1829, "    cmp ax, 0x4310"],
      [1835, "    mov al, 0x80"],
      [1838, "    mov bx, xms_entry"],
      [1851, "xms_entry:"],
      [1868, "    cmp ah, 0x08"],
      [1870, "    cmp ah, 0x09"],
      [1872, "    cmp ah, 0x0A"],
      [1874, "    cmp ah, 0x0B"],
      [1911, "    mov ax, [cs:xms_total_kb]"],
      [1935, "    cmp word [cs:xms_alloc_kb], 0"],
      [1921, "    cmp dx, [cs:xms_total_kb]"],
      [1923, "    mov [cs:xms_alloc_kb], dx"],
      [1938, "    mov ax, 1"],
      [1925, "    mov dx, 1"],
      [1937, "    mov word [cs:xms_alloc_kb], 0"],
      [1941, ".move:"],
      [1950, "    test ax, 1"],
  ...
  },
  {
    ...
    code: [
      ...
      [2169, "%if !ENABLE_EMS"],
      [2170, "int67_absent_handler:"],
      [2171, "    mov ah, 0x80"],
      [2175, "%if ENABLE_EMS"],
      [2176, "int67_handler:"],
      [2200, ".frame:"],
      [2202, "    mov bx, EMS_FRAME_SEG"],
      [2204, ".pages:"],
      [2205, "    mov bx, EMS_TOTAL_PAGES"],
      [2210, ".alloc:"],
      [2215, "    cmp bx, EMS_TOTAL_PAGES"],
      [2217, "    mov [cs:ems_alloc_pages], bx"],
      [2225, ".map:"],
      [2230, "    cmp al, 3"],
      [2232, "    cmp bx, [cs:ems_alloc_pages]"],
      [2257, "    call ems_copy_16k"],
      [2257, "    call ems_copy_16k"],
      [2275, "    mov [cs:si+ems_map_pages], bx"],
      [2303, "    mov word [cs:ems_alloc_pages], 0"],
      [2327, "ems_clear_map:"],
