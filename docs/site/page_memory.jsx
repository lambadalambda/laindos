...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3705, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3706, "%error \"kernel leaves too little HMA stack guard\""],
      [3708, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3709, "%error \"kernel exceeds boot load area\""],
      [3711, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3712, "%error \"FAT buffer overlaps CD_BUF\""],
      [3714, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3715, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3717, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3718, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3720, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3721, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3723, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3724, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3726, "%if MCB_START >= MEM_TOP"],
      [3727, "%error \"MCB arena is empty\""],
      [3729, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3730, "%error \"EMS frame must be inside conventional arena\""],
      [3741, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
