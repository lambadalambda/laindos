...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3653, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3654, "%error \"kernel leaves too little HMA stack guard\""],
      [3656, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3657, "%error \"kernel exceeds boot load area\""],
      [3659, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3660, "%error \"FAT buffer overlaps CD_BUF\""],
      [3662, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3663, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3665, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3666, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3668, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3669, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3671, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3672, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3674, "%if MCB_START >= MEM_TOP"],
      [3675, "%error \"MCB arena is empty\""],
      [3677, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3678, "%error \"EMS frame must be inside conventional arena\""],
      [3689, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      ...
      [2383, "    mov word [cs:xms_alloc_kb], 0"],
      [2384, "    call release_inherited_handles"],
      [2385, "    call close_owned_handles"],
      [2387, "    mov si, [cs:mcb_first]"],
      [2389, "    mov ds, si"],
      [2396, "    mov ax, [cs:cur_psp]"],
      [2397, "    cmp word [ds:1], ax"],
      [2399, "    mov word [ds:1], 0"],
      [2403, "    call mcb_walk_next"],
      [2407, "    call mcb_coalesce_all_free"],
      [2412, "    mov ax, [0x16]"],
