...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3323, "%if LOAD_SEG <= RELOC_SEG"],
      [3324, "%error \"LOAD_SEG must be above RELOC_SEG\""],
      [3326, "%if (kernel_end - kernel_entry) > ((LOAD_SEG - RELOC_SEG) * 16)"],
      [3327, "%error \"kernel exceeds boot relocation gap\""],
      [3329, "%if (kernel_end - kernel_entry) > ((SEC_BUF - RELOC_SEG) * 16)"],
      [3330, "%error \"kernel overlaps SEC_BUF\""],
      [3332, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3333, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3335, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3336, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3338, "%if (ROOT_SEG + ROOT_BUF_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3339, "%error \"ROOT buffer overlaps kernel stack\""],
      [3341, "%if (ROOT_SEG + ROOT_BUF_PARAS + STACK_ROOT_GUARD_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3342, "%error \"ROOT buffer leaves too little kernel stack guard\""],
      [3344, "%if (RELOC_SEG + (KERNEL_STACK_TOP / 16)) > MCB_START"],
      [3345, "%error \"kernel stack overlaps MCB arena\""],
      [3347, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3348, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3350, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3351, "%error \"EMS frame must be inside conventional arena\""],
      [3362, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3363, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
  ...
  },
  {
    ...
    code: [
      [2155, "do_terminate:"],
      [2164, "%if ENABLE_EMS"],
      [2165, "    mov word [cs:ems_alloc_pages], 0"],
      [2168, "    mov word [cs:xms_alloc_kb], 0"],
      [2169, "    call release_inherited_handles"],
      [2170, "    call close_owned_handles"],
      [2172, "    mov si, [cs:mcb_first]"],
      [2174, "    mov ds, si"],
      [2181, "    mov ax, [cs:cur_psp]"],
      [2182, "    cmp word [ds:1], ax"],
      [2184, "    mov word [ds:1], 0"],
      [2188, "    call mcb_walk_next"],
      [2192, "    call mcb_coalesce_all_free"],
      [2197, "    mov ax, [0x16]"],
      [2198, "    mov [cs:cur_psp], ax"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1594, "init_xms_size:"],
      [1596, "    mov word [cs:xms_total_kb], 0"],
      [1597, "    mov ah, 0x88"],
      [1598, "    int 0x15"],
      [1602, "    cmp ax, XMS_MAX_KB"],
      [1604, "    mov ax, XMS_MAX_KB"],
      [1606, "    mov [cs:xms_total_kb], ax"],
      [1612, "int2f_handler:"],
      [1614, "    cmp ax, 0x4300"],
      [1616, "    cmp ax, 0x4310"],
      [1621, "    mov al, 0x80"],
      [1624, "    mov bx, xms_entry"],
      [1634, "xms_entry:"],
      [1637, "    cmp ah, 0x08"],
      [1639, "    cmp ah, 0x09"],
      [1641, "    cmp ah, 0x0A"],
      [1643, "    cmp ah, 0x0B"],
      [1660, "    mov ax, [cs:xms_total_kb]"],
      [1666, "    cmp word [cs:xms_alloc_kb], 0"],
      [1670, "    cmp dx, [cs:xms_total_kb]"],
      [1672, "    mov [cs:xms_alloc_kb], dx"],
      [1673, "    mov ax, 1"],
      [1674, "    mov dx, 1"],
      [1686, "    mov word [cs:xms_alloc_kb], 0"],
      [1690, ".move:"],
      [1699, "    test ax, 1"],
      [1707, "    call xms_prepare_endpoint"],
      [1707, "    call xms_prepare_endpoint"],
      [1761, "    mov ax, 0x8700"],
      [1766, "    int 0x15"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1460, "%if ENABLE_EMS"],
      [1461, "    mov [es:0x67*4], word int67_handler"],
      [1463, "%else"],
      [1464, "    mov [es:0x67*4], word int67_absent_handler"],
      [1917, "%if !ENABLE_EMS"],
      [1918, "int67_absent_handler:"],
      [1919, "    mov ah, 0x80"],
      [1923, "%if ENABLE_EMS"],
      [1924, "int67_handler:"],
      [1948, ".frame:"],
      [1950, "    mov bx, EMS_FRAME_SEG"],
      [1952, ".pages:"],
      [1953, "    mov bx, EMS_TOTAL_PAGES"],
      [1958, ".alloc:"],
      [1963, "    cmp bx, EMS_TOTAL_PAGES"],
      [1965, "    mov [cs:ems_alloc_pages], bx"],
      [1973, ".map:"],
      [1978, "    cmp al, 3"],
      [1980, "    cmp bx, [cs:ems_alloc_pages]"],
      [2005, "    call ems_copy_16k"],
      [2005, "    call ems_copy_16k"],
      [2023, "    mov [cs:si+ems_map_pages], bx"],
      [2051, "    mov word [cs:ems_alloc_pages], 0"],
      [2075, "ems_clear_map:"],
      [2076, "    mov word [cs:ems_map_pages], 0xFFFF"]],
