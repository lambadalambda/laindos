...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3684, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3685, "%error \"kernel leaves too little HMA stack guard\""],
      [3687, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3688, "%error \"kernel exceeds boot load area\""],
      [3690, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3691, "%error \"FAT buffer overlaps CD_BUF\""],
      [3693, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3694, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3696, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3697, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3699, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3700, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3702, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3703, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3705, "%if MCB_START >= MEM_TOP"],
      [3706, "%error \"MCB arena is empty\""],
      [3708, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3709, "%error \"EMS frame must be inside conventional arena\""],
      [3720, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
