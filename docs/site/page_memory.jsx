...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4358, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4359, "%error \"kernel leaves too little HMA stack guard\""],
      [4361, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4362, "%error \"kernel exceeds boot load area\""],
      [4364, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4365, "%error \"FAT buffer overlaps CD_BUF\""],
      [4367, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4368, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4370, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4371, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4373, "%if (READ_CACHE_BUF + READ_CACHE_PARAS) > ROOT_SEG"],
      [4374, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4376, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4377, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4379, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4380, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4382, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4383, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4385, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4386, "%error \"CD cache overlaps MCB arena\""],
      [4388, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > SUBDIR_CACHE_BUF"],
      [4389, "%error \"CD cache overlaps subdirectory cache\""],
      [4391, "%if (SUBDIR_CACHE_BUF + (SUBDIR_CACHE_SLOTS * SUBDIR_CACHE_SLOT_PARAS)) > MCB_START"],
      [4392, "%error \"subdirectory cache overlaps MCB arena\""],
      [4394, "%if MCB_START >= MEM_TOP"],
      [4395, "%error \"MCB arena is empty\""],
      [4397, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4398, "%error \"EMS frame must be inside conventional arena\""],
      [4409, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4358, 4358, 4358, 4373, 4376, 4379, 4382, 4385, 4388, 4391, 4398],
  ...
  },
  {
    ...
    code: [
      [2690, "do_terminate:"],
      [2700, "%if ENABLE_EMS"],
      [2701, "    mov word [cs:ems_alloc_pages], 0"],
      [2704, "    mov word [cs:xms_alloc_kb], 0"],
      [2705, "    call release_inherited_handles"],
      [2706, "    call close_owned_handles"],
      [2708, "    mov si, [cs:mcb_first]"],
      [2767, "    mov ds, si"],
      [2666, "    mov ax, [cs:cur_psp]"],
      [2712, "    cmp word [ds:1], ax"],
      [2714, "    mov word [ds:1], 0"],
      [2709, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
      [2717, "    call mcb_coalesce_all_free"],
      [2722, "    mov ax, [0x16]"],
      [{a: "do_terminate_restore_parent_psp"}, "    mov [cs:cur_psp], ax"]],
    hi: [2690, 2700, 2700, 2700, 2690, 2705, {a: "do_terminate_restore_parent_psp"}],
  ...
  },
  {
    ...
    code: [
      ...
      [2035, "    mov ah, 0x88"],
      [2036, "    int 0x15"],
      [2041, "    cmp ax, XMS_MAX_KB - 64"],
      [2043, "    mov ax, XMS_MAX_KB - 64"],
      [2045, "    mov [cs:xms_total_kb], ax"],
      [2067, "int2f_handler:"],
      [2081, "    cmp ax, 0x4300"],
      [2083, "    cmp ax, 0x4310"],
      [2089, "    mov al, 0x80"],
      [2092, "    mov bx, xms_entry"],
      [2105, "xms_entry:"],
      [2122, "    cmp ah, 0x08"],
      [2124, "    cmp ah, 0x09"],
      [2126, "    cmp ah, 0x0A"],
      [2128, "    cmp ah, 0x0B"],
      [2165, "    mov ax, [cs:xms_total_kb]"],
      [2171, "    cmp word [cs:xms_alloc_kb], 0"],
      [2175, "    cmp dx, [cs:xms_total_kb]"],
      [2177, "    mov [cs:xms_alloc_kb], dx"],
      [2153, "    mov ax, 1"],
      [2142, "    mov dx, 1"],
      [2191, "    mov word [cs:xms_alloc_kb], 0"],
      [2195, ".move:"],
      [2204, "    test ax, 1"],
      [2212, "    call xms_prepare_endpoint"],
      [2220, "    call xms_prepare_endpoint"],
      [2266, "    mov ax, 0x8700"],
      [{a: "xms_move_bios_call"}, "    int 0x15"]],
    hi: [43, 113, 2032, 2081, 2092, 2105, 2171, 2191, {a: "xms_move_bios_call"}],
  ...
  },
  {
    ...
    code: [
      ...
      [2423, "%if !ENABLE_EMS"],
      [2424, "int67_absent_handler:"],
      [2425, "    mov ah, 0x80"],
      [2429, "%if ENABLE_EMS"],
      [2430, "int67_handler:"],
      [2454, ".frame:"],
      [2456, "    mov bx, EMS_FRAME_SEG"],
      [2458, ".pages:"],
      [2459, "    mov bx, EMS_TOTAL_PAGES"],
      [2464, ".alloc:"],
      [2469, "    cmp bx, EMS_TOTAL_PAGES"],
      [2471, "    mov [cs:ems_alloc_pages], bx"],
      [2479, ".map:"],
      [2484, "    cmp al, 3"],
      [2486, "    cmp bx, [cs:ems_alloc_pages]"],
      [2511, "    call ems_copy_16k"],
      [2522, "    call ems_copy_16k"],
      [2529, "    mov [cs:si+ems_map_pages], bx"],
      [2557, "    mov word [cs:ems_alloc_pages], 0"],
      [2581, "ems_clear_map:"],
      [{a: "ems_clear_map_fill"}, "    mov word [cs:ems_map_pages], 0xFFFF"]],
    hi: [117, 1837, 2424, 2430, 2454, 2469, 2469, 2430, 2529],
