...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3938, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3939, "%error \"kernel leaves too little HMA stack guard\""],
      [3941, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3942, "%error \"kernel exceeds boot load area\""],
      [3944, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3945, "%error \"FAT buffer overlaps CD_BUF\""],
      [3947, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3948, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3950, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3951, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3953, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3954, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3956, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3957, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3959, "%if MCB_START >= MEM_TOP"],
      [3960, "%error \"MCB arena is empty\""],
      [3962, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3963, "%error \"EMS frame must be inside conventional arena\""],
      [3974, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [3938, 3941, 3944, 3939, 3942, 3945, 3948, 3951, 3954, 3974],
  ...
  },
  {
    ...
    code: [
      [2648, "do_terminate:"],
      [2658, "%if ENABLE_EMS"],
      [2515, "    mov word [cs:ems_alloc_pages], 0"],
      [2662, "    mov word [cs:xms_alloc_kb], 0"],
      [2663, "    call release_inherited_handles"],
      [2664, "    call close_owned_handles"],
      [2666, "    mov si, [cs:mcb_first]"],
      [2725, "    mov ds, si"],
      [2624, "    mov ax, [cs:cur_psp]"],
      [2670, "    cmp word [ds:1], ax"],
      [2672, "    mov word [ds:1], 0"],
      [2667, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
      [2675, "    call mcb_coalesce_all_free"],
      [2680, "    mov ax, [0x16]"],
      [{a: "do_terminate_restore_parent_psp"}, "    mov [cs:cur_psp], ax"]],
    hi: [2648, 2662, 2658, 2662, 2648, 2667, {a: "do_terminate_restore_parent_psp"}],
  ...
  },
  {
    ...
    code: [
      ...
      [2041, "    cmp ax, 0x4310"],
      [2047, "    mov al, 0x80"],
      [2050, "    mov bx, xms_entry"],
      [2063, "xms_entry:"],
      [2080, "    cmp ah, 0x08"],
      [2082, "    cmp ah, 0x09"],
      [2084, "    cmp ah, 0x0A"],
      [2086, "    cmp ah, 0x0B"],
      [2123, "    mov ax, [cs:xms_total_kb]"],
      [2129, "    cmp word [cs:xms_alloc_kb], 0"],
      [2133, "    cmp dx, [cs:xms_total_kb]"],
      [2135, "    mov [cs:xms_alloc_kb], dx"],
      [2111, "    mov ax, 1"],
      [2100, "    mov dx, 1"],
      [2149, "    mov word [cs:xms_alloc_kb], 0"],
      [2153, ".move:"],
      [2162, "    test ax, 1"],
      [2170, "    call xms_prepare_endpoint"],
      [2170, "    call xms_prepare_endpoint"],
      [2224, "    mov ax, 0x8700"],
      [{a: "xms_move_bios_call"}, "    int 0x15"]],
    hi: [43, 113, 1994, 2039, 2050, 2063, 2135, 2153, {a: "xms_move_bios_call"}],
  ...
  },
  {
    ...
    code: [
      ...
      [2381, "%if !ENABLE_EMS"],
      [2382, "int67_absent_handler:"],
      [2383, "    mov ah, 0x80"],
      [2387, "%if ENABLE_EMS"],
      [2388, "int67_handler:"],
      [2412, ".frame:"],
      [2414, "    mov bx, EMS_FRAME_SEG"],
      [2416, ".pages:"],
      [2417, "    mov bx, EMS_TOTAL_PAGES"],
      [2422, ".alloc:"],
      [2427, "    cmp bx, EMS_TOTAL_PAGES"],
      [2429, "    mov [cs:ems_alloc_pages], bx"],
      [2437, ".map:"],
      [2442, "    cmp al, 3"],
      [2444, "    cmp bx, [cs:ems_alloc_pages]"],
      [2469, "    call ems_copy_16k"],
      [2469, "    call ems_copy_16k"],
      [2487, "    mov [cs:si+ems_map_pages], bx"],
      [2515, "    mov word [cs:ems_alloc_pages], 0"],
      [2539, "ems_clear_map:"],
      [{a: "ems_clear_map_fill"}, "    mov word [cs:ems_map_pages], 0xFFFF"]],
    hi: [117, 1798, 2382, 2388, 2414, 2429, 2429, 2388, 2487],
