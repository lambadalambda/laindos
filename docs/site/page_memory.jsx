...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3798, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3799, "%error \"kernel leaves too little HMA stack guard\""],
      [3801, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3802, "%error \"kernel exceeds boot load area\""],
      [3804, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3805, "%error \"FAT buffer overlaps CD_BUF\""],
      [3807, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3808, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3810, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3811, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3813, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3814, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3816, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3817, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3819, "%if MCB_START >= MEM_TOP"],
      [3820, "%error \"MCB arena is empty\""],
      [3822, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3823, "%error \"EMS frame must be inside conventional arena\""],
      [3834, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      ...
      [1443, "    je .as_set"],
      [781, "    xor ax, ax"],
      [1572, "    mov al, [cs:alloc_strat]"],
      [1450, "    cmp bl, 2"],
      [1452, "    mov [cs:alloc_strat], bl"],
      [1489, ".alloc_mem:"],
      [1572, "    mov al, [cs:alloc_strat]"],
      [1175, "    cmp al, 2"],
      [1512, "    jmp near .am_find_last"],
      [1504, "    cmp al, 1"],
      [1506, "    jmp near .am_find_best"],
      [1508, "    cmp word [cs:am_req], 1"],
      [1510, "    cmp word [cs:am_req], SMALL_ALLOC_HIGH_MAX"],
      [1512, "    jmp near .am_find_last"],
      [1622, "    mov si, [cs:mcb_first]"],
      [1635, "    cmp ax, [cs:am_req]"],
      [1529, ".am_use:"],
      [1150, "    mov ax, [cs:cur_psp]"],
      [1595, ".am_find_last:"],
      [1619, ".am_find_best:"],
      [1676, ".am_nomem:"],
      [1679, ".am_scan_largest:"],
      [1916, "    mov ax, 8"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1758, "    mov si, es"],
      [1759, "    dec si"],
      [1625, "    cmp byte [ds:0], MCB_SIG_M"],
      [1642, "    cmp byte [ds:0], MCB_SIG_Z"],
      [1742, "    mov word [ds:1], 0"],
      [1743, "    call mcb_merge_free_forward"],
      [1750, ".resize_mem:"],
      [1757, "    mov [cs:rm_req], bx"],
      [1758, "    mov si, es"],
      [1634, "    mov ax, [ds:3]"],
      [1770, "    jae .rm_shrink"],
      [1778, "    cmp byte [es:0], MCB_SIG_M"],
      [1783, "    cmp word [es:1], 0"],
      [1870, "    add ax, cx"],
      [1788, "    cmp ax, bx"],
      [1823, "    mov byte [es:0], dl"],
      [1666, "    mov byte [ds:0], MCB_SIG_M"],
      [1546, "    mov word [es:3], cx"],
      [1828, "    mov word [ds:3], bx"],
      [1813, "    mov ax, [ds:3]"],
      [1823, "    mov byte [es:0], dl"],
      [1542, "    mov word [es:1], 0"],
      [1828, "    mov word [ds:3], bx"],
      [1871, "    mov [es:0x02], ax"],
      [1914, ".rm_cant_grow:"],
      [1915, "    mov bx, ax"],
  ...
  },
  {
    ...
    code: [
      [2483, "do_terminate:"],
      [2493, "%if ENABLE_EMS"],
      [2350, "    mov word [cs:ems_alloc_pages], 0"],
      [2497, "    mov word [cs:xms_alloc_kb], 0"],
      [2498, "    call release_inherited_handles"],
      [2499, "    call close_owned_handles"],
      [2501, "    mov si, [cs:mcb_first]"],
      [2503, "    mov ds, si"],
      [2510, "    mov ax, [cs:cur_psp]"],
      [2511, "    cmp word [ds:1], ax"],
      [2513, "    mov word [ds:1], 0"],
      [2517, "    call mcb_walk_next"],
      [2521, "    call mcb_coalesce_all_free"],
      [2526, "    mov ax, [0x16]"],
  ...
  },
  {
    ...
    code: [
      ...
      [1829, "init_xms_size:"],
      [1831, "    mov word [cs:xms_total_kb], 0"],
      [1832, "    mov ah, 0x88"],
      [1833, "    int 0x15"],
      [1838, "    cmp ax, XMS_MAX_KB - 64"],
      [1840, "    mov ax, XMS_MAX_KB - 64"],
      [1842, "    mov [cs:xms_total_kb], ax"],
      [1864, "int2f_handler:"],
      [1874, "    cmp ax, 0x4300"],
      [1876, "    cmp ax, 0x4310"],
      [1882, "    mov al, 0x80"],
      [1885, "    mov bx, xms_entry"],
      [1898, "xms_entry:"],
      [1915, "    cmp ah, 0x08"],
      [1917, "    cmp ah, 0x09"],
      [1919, "    cmp ah, 0x0A"],
      [1921, "    cmp ah, 0x0B"],
      [1958, "    mov ax, [cs:xms_total_kb]"],
      [1982, "    cmp word [cs:xms_alloc_kb], 0"],
      [1968, "    cmp dx, [cs:xms_total_kb]"],
      [1970, "    mov [cs:xms_alloc_kb], dx"],
      [1985, "    mov ax, 1"],
      [1972, "    mov dx, 1"],
      [1984, "    mov word [cs:xms_alloc_kb], 0"],
      [1988, ".move:"],
      [1997, "    test ax, 1"],
      [2005, "    call xms_prepare_endpoint"],
      [2005, "    call xms_prepare_endpoint"],
      [2059, "    mov ax, 0x8700"],
  ...
  },
  {
    ...
    code: [
      ...
      [2216, "%if !ENABLE_EMS"],
      [2217, "int67_absent_handler:"],
      [2218, "    mov ah, 0x80"],
      [2222, "%if ENABLE_EMS"],
      [2223, "int67_handler:"],
      [2247, ".frame:"],
      [2249, "    mov bx, EMS_FRAME_SEG"],
      [2251, ".pages:"],
      [2252, "    mov bx, EMS_TOTAL_PAGES"],
      [2257, ".alloc:"],
      [2262, "    cmp bx, EMS_TOTAL_PAGES"],
      [2264, "    mov [cs:ems_alloc_pages], bx"],
      [2272, ".map:"],
      [2277, "    cmp al, 3"],
      [2279, "    cmp bx, [cs:ems_alloc_pages]"],
      [2304, "    call ems_copy_16k"],
      [2304, "    call ems_copy_16k"],
      [2322, "    mov [cs:si+ems_map_pages], bx"],
      [2350, "    mov word [cs:ems_alloc_pages], 0"],
      [2374, "ems_clear_map:"],
