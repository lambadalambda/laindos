...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    code: [
      [115, "kernel_entry:"],
      [131, "    mov ax, RELOC_SEG"],
      [138, "    jmp RELOC_SEG:.relocated"],
      [139, ".relocated:"],
      [143, "    mov ss, ax"],
      [144, "    mov sp, KERNEL_STACK_TOP"],
      [155, "    int 0x12"],
      [156, "    mov [mem_kib], ax"],
      [164, "%if ENABLE_XMS"],
      [165, "    call init_xms_size"],
      [179, "    mov ax, MCB_START"],
      [142, "    mov es, ax"],
      [181, "    mov byte [es:0], MCB_SIG_Z"],
      [182, "    mov word [es:1], 0"],
      [183, "    mov ax, MEM_TOP - MCB_START - 1"],
      [184, "    mov word [es:3], ax"],
      [185, "    mov word [mcb_first], MCB_START"],
      [186, "    mov word [cur_psp], 0"]],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3346, "%if LOAD_SEG <= RELOC_SEG"],
      [3347, "%error \"LOAD_SEG must be above RELOC_SEG\""],
      [3349, "%if (kernel_end - kernel_entry) > ((LOAD_SEG - RELOC_SEG) * 16)"],
      [3350, "%error \"kernel exceeds boot relocation gap\""],
      [3352, "%if (kernel_end - kernel_entry) > ((SEC_BUF - RELOC_SEG) * 16)"],
      [3353, "%error \"kernel overlaps SEC_BUF\""],
      [3355, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3356, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3358, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3359, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3361, "%if (ROOT_SEG + ROOT_BUF_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3362, "%error \"ROOT buffer overlaps kernel stack\""],
      [3364, "%if (ROOT_SEG + ROOT_BUF_PARAS + STACK_ROOT_GUARD_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3365, "%error \"ROOT buffer leaves too little kernel stack guard\""],
      [3367, "%if (RELOC_SEG + (KERNEL_STACK_TOP / 16)) > MCB_START"],
      [3368, "%error \"kernel stack overlaps MCB arena\""],
      [3370, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3371, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3373, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3374, "%error \"EMS frame must be inside conventional arena\""],
      [3385, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3386, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
  ...
  },
  {
    ...
    code: [
      ...
      [1258, "    je .as_set"],
      [610, "    xor ax, ax"],
      [1262, "    mov al, [cs:alloc_strat]"],
      [1265, "    cmp bl, 2"],
      [1267, "    mov [cs:alloc_strat], bl"],
      [1304, ".alloc_mem:"],
      [1313, "    mov al, [cs:alloc_strat]"],
      [992, "    cmp al, 2"],
      [1317, "    jmp near .am_find_last"],
      [1319, "    cmp al, 1"],
      [1321, "    jmp near .am_find_best"],
      [1323, "    cmp word [cs:am_req], 1"],
      [1325, "    cmp word [cs:am_req], SMALL_ALLOC_HIGH_MAX"],
      [1317, "    jmp near .am_find_last"],
      [1329, "    mov si, [cs:mcb_first]"],
      [1342, "    cmp ax, [cs:am_req]"],
      [1344, ".am_use:"],
      [967, "    mov ax, [cs:cur_psp]"],
      [1410, ".am_find_last:"],
      [1434, ".am_find_best:"],
      [1491, ".am_nomem:"],
      [1494, ".am_scan_largest:"],
      [1515, "    mov ax, 8"]],
    hi: [1220, 1255, 1294, 1315, 1400, 1424, 1484],
  ...
  },
  {
    ...
    code: [
      ...
      [1547, "    mov si, es"],
      [1548, "    dec si"],
      [1332, "    cmp byte [ds:0], MCB_SIG_M"],
      [1334, "    cmp byte [ds:0], MCB_SIG_Z"],
      [1557, "    mov word [ds:1], 0"],
      [1558, "    call mcb_merge_free_forward"],
      [1565, ".resize_mem:"],
      [1572, "    mov [cs:rm_req], bx"],
      [1573, "    mov si, es"],
      [1341, "    mov ax, [ds:3]"],
      [1585, "    jae .rm_shrink"],
      [1593, "    cmp byte [es:0], MCB_SIG_M"],
      [1598, "    cmp word [es:1], 0"],
      [1602, "    add ax, cx"],
      [1505, "    cmp ax, bx"],
      [1617, "    mov byte [es:0], dl"],
      [1362, "    mov byte [ds:0], MCB_SIG_M"],
      [1361, "    mov word [es:3], cx"],
      [1624, "    mov word [ds:3], bx"],
      [1608, "    mov ax, [ds:3]"],
      [1638, "    mov byte [es:0], dl"],
      [1357, "    mov word [es:1], 0"],
      [1624, "    mov word [ds:3], bx"],
      [1686, "    mov [es:0x02], ax"],
      [1729, ".rm_cant_grow:"],
      [1507, "    mov bx, ax"],
      [1731, "    mov ax, 8"]],
    hi: [1531, 1547, 1548, 1555, 1588, 1629, 1676, 1719],
  ...
  },
  {
    ...
    code: [
      [2175, "do_terminate:"],
      [2184, "%if ENABLE_EMS"],
      [2185, "    mov word [cs:ems_alloc_pages], 0"],
      [2188, "    mov word [cs:xms_alloc_kb], 0"],
      [2189, "    call release_inherited_handles"],
      [2190, "    call close_owned_handles"],
      [2192, "    mov si, [cs:mcb_first]"],
      [2194, "    mov ds, si"],
      [2201, "    mov ax, [cs:cur_psp]"],
      [2202, "    cmp word [ds:1], ax"],
      [2204, "    mov word [ds:1], 0"],
      [2208, "    call mcb_walk_next"],
      [2212, "    call mcb_coalesce_all_free"],
      [2217, "    mov ax, [0x16]"],
      [2218, "    mov [cs:cur_psp], ax"]],
  ...
  },
  {
    ...
    code: [
      ...
      [107, "%ifndef ENABLE_XMS"],
      [108, "%define ENABLE_XMS 1"],
      [1614, "init_xms_size:"],
      [1616, "    mov word [cs:xms_total_kb], 0"],
      [1617, "    mov ah, 0x88"],
      [1618, "    int 0x15"],
      [1622, "    cmp ax, XMS_MAX_KB"],
      [1624, "    mov ax, XMS_MAX_KB"],
      [1626, "    mov [cs:xms_total_kb], ax"],
      [1632, "int2f_handler:"],
      [1634, "    cmp ax, 0x4300"],
      [1636, "    cmp ax, 0x4310"],
      [1641, "    mov al, 0x80"],
      [1644, "    mov bx, xms_entry"],
      [1654, "xms_entry:"],
      [1657, "    cmp ah, 0x08"],
      [1659, "    cmp ah, 0x09"],
      [1661, "    cmp ah, 0x0A"],
      [1663, "    cmp ah, 0x0B"],
      [1680, "    mov ax, [cs:xms_total_kb]"],
      [1686, "    cmp word [cs:xms_alloc_kb], 0"],
      [1690, "    cmp dx, [cs:xms_total_kb]"],
      [1692, "    mov [cs:xms_alloc_kb], dx"],
      [1693, "    mov ax, 1"],
      [1694, "    mov dx, 1"],
      [1706, "    mov word [cs:xms_alloc_kb], 0"],
      [1710, ".move:"],
      [1719, "    test ax, 1"],
      [1727, "    call xms_prepare_endpoint"],
      [1727, "    call xms_prepare_endpoint"],
      [1781, "    mov ax, 0x8700"],
      [1786, "    int 0x15"]],
  ...
  },
  {
    ...
    code: [
      ...
      [111, "%ifndef ENABLE_EMS"],
      [112, "%define ENABLE_EMS 0"],
      [1480, "%if ENABLE_EMS"],
      [1481, "    mov [es:0x67*4], word int67_handler"],
      [1483, "%else"],
      [1484, "    mov [es:0x67*4], word int67_absent_handler"],
      [1937, "%if !ENABLE_EMS"],
      [1938, "int67_absent_handler:"],
      [1939, "    mov ah, 0x80"],
      [1943, "%if ENABLE_EMS"],
      [1944, "int67_handler:"],
      [1968, ".frame:"],
      [1970, "    mov bx, EMS_FRAME_SEG"],
      [1972, ".pages:"],
      [1973, "    mov bx, EMS_TOTAL_PAGES"],
      [1978, ".alloc:"],
      [1983, "    cmp bx, EMS_TOTAL_PAGES"],
      [1985, "    mov [cs:ems_alloc_pages], bx"],
      [1993, ".map:"],
      [1998, "    cmp al, 3"],
      [2000, "    cmp bx, [cs:ems_alloc_pages]"],
      [2025, "    call ems_copy_16k"],
      [2025, "    call ems_copy_16k"],
      [2043, "    mov [cs:si+ems_map_pages], bx"],
      [2071, "    mov word [cs:ems_alloc_pages], 0"],
      [2095, "ems_clear_map:"],
      [2096, "    mov word [cs:ems_map_pages], 0xFFFF"]],
