...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3945, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3946, "%error \"kernel leaves too little HMA stack guard\""],
      [3948, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3949, "%error \"kernel exceeds boot load area\""],
      [3951, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3952, "%error \"FAT buffer overlaps CD_BUF\""],
      [3954, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3955, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3957, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3958, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3960, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3961, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3963, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3964, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3966, "%if MCB_START >= MEM_TOP"],
      [3967, "%error \"MCB arena is empty\""],
      [3969, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3970, "%error \"EMS frame must be inside conventional arena\""],
      [3981, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [3945, 3948, 3951, 3946, 3949, 3945, 3948, 3951, 3954, 3981],
  ...
  },
  {
    ...
    code: [
      [2650, "do_terminate:"],
      [2660, "%if ENABLE_EMS"],
      [2517, "    mov word [cs:ems_alloc_pages], 0"],
      [2664, "    mov word [cs:xms_alloc_kb], 0"],
      [2665, "    call release_inherited_handles"],
      [2666, "    call close_owned_handles"],
      [2668, "    mov si, [cs:mcb_first]"],
      [2727, "    mov ds, si"],
      [2626, "    mov ax, [cs:cur_psp]"],
      [2672, "    cmp word [ds:1], ax"],
      [2674, "    mov word [ds:1], 0"],
      [2669, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
      [2677, "    call mcb_coalesce_all_free"],
      [2682, "    mov ax, [0x16]"],
      [{a: "do_terminate_restore_parent_psp"}, "    mov [cs:cur_psp], ax"]],
    hi: [2650, 2664, 2660, 2664, 2650, 2669, {a: "do_terminate_restore_parent_psp"}],
  ...
  },
  {
    ...
    code: [
      ...
      [1994, "init_xms_size:"],
      [1996, "    mov word [cs:xms_total_kb], 0"],
      [1997, "    mov ah, 0x88"],
      [1998, "    int 0x15"],
      [2003, "    cmp ax, XMS_MAX_KB - 64"],
      [2005, "    mov ax, XMS_MAX_KB - 64"],
      [2007, "    mov [cs:xms_total_kb], ax"],
      [2029, "int2f_handler:"],
      [2041, "    cmp ax, 0x4300"],
      [2043, "    cmp ax, 0x4310"],
      [2049, "    mov al, 0x80"],
      [2052, "    mov bx, xms_entry"],
      [2065, "xms_entry:"],
      [2082, "    cmp ah, 0x08"],
      [2084, "    cmp ah, 0x09"],
      [2086, "    cmp ah, 0x0A"],
      [2088, "    cmp ah, 0x0B"],
      [2125, "    mov ax, [cs:xms_total_kb]"],
      [2131, "    cmp word [cs:xms_alloc_kb], 0"],
      [2135, "    cmp dx, [cs:xms_total_kb]"],
      [2137, "    mov [cs:xms_alloc_kb], dx"],
      [2113, "    mov ax, 1"],
      [2102, "    mov dx, 1"],
      [2151, "    mov word [cs:xms_alloc_kb], 0"],
      [2155, ".move:"],
      [2164, "    test ax, 1"],
      [2172, "    call xms_prepare_endpoint"],
      [2172, "    call xms_prepare_endpoint"],
      [2226, "    mov ax, 0x8700"],
      [{a: "xms_move_bios_call"}, "    int 0x15"]],
    hi: [43, 113, 1994, 2041, 2052, 2065, 2135, 2155, {a: "xms_move_bios_call"}],
  ...
  },
  {
    ...
    code: [
      ...
      [1794, "%if ENABLE_EMS"],
      [1795, "    mov [es:0x67*4], word int67_handler"],
      [1797, "%else"],
      [1798, "    mov [es:0x67*4], word int67_absent_handler"],
      [2383, "%if !ENABLE_EMS"],
      [2384, "int67_absent_handler:"],
      [2385, "    mov ah, 0x80"],
      [2389, "%if ENABLE_EMS"],
      [2390, "int67_handler:"],
      [2414, ".frame:"],
      [2416, "    mov bx, EMS_FRAME_SEG"],
      [2418, ".pages:"],
      [2419, "    mov bx, EMS_TOTAL_PAGES"],
      [2424, ".alloc:"],
      [2429, "    cmp bx, EMS_TOTAL_PAGES"],
      [2431, "    mov [cs:ems_alloc_pages], bx"],
      [2439, ".map:"],
      [2444, "    cmp al, 3"],
      [2446, "    cmp bx, [cs:ems_alloc_pages]"],
      [2471, "    call ems_copy_16k"],
      [2471, "    call ems_copy_16k"],
      [2489, "    mov [cs:si+ems_map_pages], bx"],
      [2517, "    mov word [cs:ems_alloc_pages], 0"],
      [2541, "ems_clear_map:"],
      [{a: "ems_clear_map_fill"}, "    mov word [cs:ems_map_pages], 0xFFFF"]],
    hi: [117, 1798, 2384, 2390, 2414, 2429, 2429, 2390, 2489],
