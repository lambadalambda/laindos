...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4496, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4497, "%error \"kernel leaves too little HMA stack guard\""],
      [4499, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4500, "%error \"kernel exceeds boot load area\""],
      [4502, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4503, "%error \"FAT buffer overlaps CD_BUF\""],
      [4505, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4506, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4508, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4509, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4511, "%if (READ_CACHE_BUF + READ_CACHE_PARAS) > ROOT_SEG"],
      [4512, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4514, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4515, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4517, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4518, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4520, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4521, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4523, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4524, "%error \"CD cache overlaps MCB arena\""],
      [4526, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > SUBDIR_CACHE_BUF"],
      [4527, "%error \"CD cache overlaps subdirectory cache\""],
      [4529, "%if (SUBDIR_CACHE_BUF + (SUBDIR_CACHE_SLOTS * SUBDIR_CACHE_SLOT_PARAS)) > MCB_START"],
      [4530, "%error \"subdirectory cache overlaps MCB arena\""],
      [4532, "%if MCB_START >= MEM_TOP"],
      [4533, "%error \"MCB arena is empty\""],
      [4535, "%if ENABLE_EMS && EMS_FRAME_SEG < 0xD000"],
      [4536, "%error \"EMS frame must use an upper-memory window at D000h or above\""],
      [4547, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4496, 4496, 4496, 4511, 4514, 4517, 4520, 4523, 4526, 4529, 4536],
  ...
  },
  {
    ...
    code: [
      [2698, "do_terminate:"],
      [2708, "%if ENABLE_EMS"],
      [2709, "    mov word [cs:ems_alloc_pages], 0"],
      [2712, "    mov word [cs:xms_alloc_kb], 0"],
      [2713, "    call release_inherited_handles"],
      [2714, "    call close_owned_handles"],
      [2716, "    mov si, [cs:mcb_first]"],
      [2717, "    MCB_WALK_EACH .dt_mcb_walk, .dt_mcb_check, .dt_mcb_next, .dt_mcb_done, .dt_mcb_done, .dt_mcb_done"],
      [2719, "    mov ax, [cs:cur_psp]"],
      [2720, "    cmp word [ds:1], ax"],
      [2722, "    mov word [ds:1], 0"],
      [2725, "    call mcb_coalesce_all_free"],
      [2730, "    mov ax, [0x16]"],
      [{a: "do_terminate_restore_parent_psp"}, "    mov [cs:cur_psp], ax"]],
    hi: [2698, 2708, 2708, 2708, 2698, 2713, {a: "do_terminate_restore_parent_psp"}],
  ...
  },
  {
    ...
    code: [
      ...
      [4364, "init_xms_store:"],
      [4369, "    cmp ax, (EMS_TOTAL_PAGES * 16)"],
      [4374, "    mov ax, XMS_MAX_KB - 64"],
      [4385, "    mov [cs:ems_backing_hi], bx"],
      [4395, "    mov [cs:xms_total_kb], ax"],
      [2069, "int2f_handler:"],
      [2083, "    cmp ax, 0x4300"],
      [2085, "    cmp ax, 0x4310"],
      [2091, "    mov al, 0x80"],
    ...
    hi: [43, 113, 2032, 4364, 2083, 2094, 2107, 2173, 2193, {a: "xms_move_bios_call"}],
    tests: ["scripts/test_xms.py", "scripts/test_free.py", "scripts/test_shell.py"],
  },
  {
    id: "ems",
    title: "EMS is default with an upper frame",
    summary: "Normal builds install INT 67h with one handle, four page-frame slots, and a 6 MiB backing pool when the D000h frame probe succeeds.",
    body: [
    ...
    code: [
      ...
      [2433, "int67_handler:"],
      [2462, ".frame:"],
      [2464, "    mov bx, EMS_FRAME_SEG"],
      [2466, ".pages:"],
      [2467, "    mov bx, [cs:ems_total_pages]"],
      [2472, ".alloc:"],
      [2477, "    cmp bx, [cs:ems_total_pages]"],
      [2479, "    mov [cs:ems_alloc_pages], bx"],
      [2487, ".map:"],
      [2492, "    cmp al, 3"],
      [2494, "    cmp bx, [cs:ems_alloc_pages]"],
      [2519, "    call ems_copy_16k"],
      [2530, "    call ems_copy_16k"],
      [2537, "    mov [cs:si+ems_map_pages], bx"],
      [2565, "    mov word [cs:ems_alloc_pages], 0"],
      [2589, "ems_clear_map:"],
      [{a: "ems_clear_map_fill"}, "    mov word [cs:ems_map_pages], 0xFFFF"]],
    hi: [117, 1837, 2427, 2433, 2462, 2477, 2477, 2433, 2537],
    tests: ["scripts/test_ems.py", "scripts/test_emsmem.py", "scripts/test_emslarge.py", "scripts/test_emsxms.py"],
  },
  {
    id: "report",
