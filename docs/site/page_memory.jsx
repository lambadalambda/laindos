...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3823, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3824, "%error \"kernel leaves too little HMA stack guard\""],
      [3826, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3827, "%error \"kernel exceeds boot load area\""],
      [3829, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3830, "%error \"FAT buffer overlaps CD_BUF\""],
      [3832, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3833, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3835, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3836, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3838, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3839, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3841, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3842, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3844, "%if MCB_START >= MEM_TOP"],
      [3845, "%error \"MCB arena is empty\""],
      [3847, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3848, "%error \"EMS frame must be inside conventional arena\""],
      [3859, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      [2499, "do_terminate:"],
      [2509, "%if ENABLE_EMS"],
      [2366, "    mov word [cs:ems_alloc_pages], 0"],
      [2513, "    mov word [cs:xms_alloc_kb], 0"],
      [2514, "    call release_inherited_handles"],
      [2515, "    call close_owned_handles"],
      [2517, "    mov si, [cs:mcb_first]"],
      [2519, "    mov ds, si"],
      [2526, "    mov ax, [cs:cur_psp]"],
      [2527, "    cmp word [ds:1], ax"],
      [2529, "    mov word [ds:1], 0"],
      [2533, "    call mcb_walk_next"],
      [2537, "    call mcb_coalesce_all_free"],
      [2542, "    mov ax, [0x16]"],
  ...
  },
  {
    ...
    code: [
      ...
      [1845, "init_xms_size:"],
      [1847, "    mov word [cs:xms_total_kb], 0"],
      [1848, "    mov ah, 0x88"],
      [1849, "    int 0x15"],
      [1854, "    cmp ax, XMS_MAX_KB - 64"],
      [1856, "    mov ax, XMS_MAX_KB - 64"],
      [1858, "    mov [cs:xms_total_kb], ax"],
      [1880, "int2f_handler:"],
      [1890, "    cmp ax, 0x4300"],
      [1892, "    cmp ax, 0x4310"],
      [1898, "    mov al, 0x80"],
      [1901, "    mov bx, xms_entry"],
      [1914, "xms_entry:"],
      [1931, "    cmp ah, 0x08"],
      [1933, "    cmp ah, 0x09"],
      [1935, "    cmp ah, 0x0A"],
      [1937, "    cmp ah, 0x0B"],
      [1974, "    mov ax, [cs:xms_total_kb]"],
      [1998, "    cmp word [cs:xms_alloc_kb], 0"],
      [1984, "    cmp dx, [cs:xms_total_kb]"],
      [1986, "    mov [cs:xms_alloc_kb], dx"],
      [2001, "    mov ax, 1"],
      [1988, "    mov dx, 1"],
      [2000, "    mov word [cs:xms_alloc_kb], 0"],
      [2004, ".move:"],
      [2013, "    test ax, 1"],
      [2021, "    call xms_prepare_endpoint"],
      [2021, "    call xms_prepare_endpoint"],
      [2075, "    mov ax, 0x8700"],
  ...
  },
  {
    ...
    code: [
      ...
      [1645, "%if ENABLE_EMS"],
      [1646, "    mov [es:0x67*4], word int67_handler"],
      [1648, "%else"],
      [1649, "    mov [es:0x67*4], word int67_absent_handler"],
      [2232, "%if !ENABLE_EMS"],
      [2233, "int67_absent_handler:"],
      [2234, "    mov ah, 0x80"],
      [2238, "%if ENABLE_EMS"],
      [2239, "int67_handler:"],
      [2263, ".frame:"],
      [2265, "    mov bx, EMS_FRAME_SEG"],
      [2267, ".pages:"],
      [2268, "    mov bx, EMS_TOTAL_PAGES"],
      [2273, ".alloc:"],
      [2278, "    cmp bx, EMS_TOTAL_PAGES"],
      [2280, "    mov [cs:ems_alloc_pages], bx"],
      [2288, ".map:"],
      [2293, "    cmp al, 3"],
      [2295, "    cmp bx, [cs:ems_alloc_pages]"],
      [2320, "    call ems_copy_16k"],
      [2320, "    call ems_copy_16k"],
      [2338, "    mov [cs:si+ems_map_pages], bx"],
      [2366, "    mov word [cs:ems_alloc_pages], 0"],
      [2390, "ems_clear_map:"],
