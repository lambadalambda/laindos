...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3683, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3684, "%error \"kernel leaves too little HMA stack guard\""],
      [3686, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3687, "%error \"kernel exceeds boot load area\""],
      [3689, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3690, "%error \"FAT buffer overlaps CD_BUF\""],
      [3692, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3693, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3695, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3696, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3698, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3699, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3701, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3702, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3704, "%if MCB_START >= MEM_TOP"],
      [3705, "%error \"MCB arena is empty\""],
      [3707, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3708, "%error \"EMS frame must be inside conventional arena\""],
      [3719, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      [2398, "do_terminate:"],
      [2408, "%if ENABLE_EMS"],
      [2265, "    mov word [cs:ems_alloc_pages], 0"],
      [2412, "    mov word [cs:xms_alloc_kb], 0"],
      [2413, "    call release_inherited_handles"],
      [2414, "    call close_owned_handles"],
      [2416, "    mov si, [cs:mcb_first]"],
      [2418, "    mov ds, si"],
      [2425, "    mov ax, [cs:cur_psp]"],
      [2426, "    cmp word [ds:1], ax"],
      [2428, "    mov word [ds:1], 0"],
      [2432, "    call mcb_walk_next"],
      [2436, "    call mcb_coalesce_all_free"],
      [2441, "    mov ax, [0x16]"],
  ...
  },
  {
    ...
    code: [
      ...
      [1748, "init_xms_size:"],
      [1750, "    mov word [cs:xms_total_kb], 0"],
      [1751, "    mov ah, 0x88"],
      [1752, "    int 0x15"],
      [1757, "    cmp ax, XMS_MAX_KB - 64"],
      [1759, "    mov ax, XMS_MAX_KB - 64"],
      [1761, "    mov [cs:xms_total_kb], ax"],
      [1783, "int2f_handler:"],
      [1793, "    cmp ax, 0x4300"],
      [1795, "    cmp ax, 0x4310"],
      [1800, "    mov al, 0x80"],
      [1803, "    mov bx, xms_entry"],
      [1813, "xms_entry:"],
      [1830, "    cmp ah, 0x08"],
      [1832, "    cmp ah, 0x09"],
      [1834, "    cmp ah, 0x0A"],
      [1836, "    cmp ah, 0x0B"],
      [1873, "    mov ax, [cs:xms_total_kb]"],
      [1897, "    cmp word [cs:xms_alloc_kb], 0"],
      [1883, "    cmp dx, [cs:xms_total_kb]"],
      [1885, "    mov [cs:xms_alloc_kb], dx"],
      [1900, "    mov ax, 1"],
      [1887, "    mov dx, 1"],
      [1899, "    mov word [cs:xms_alloc_kb], 0"],
      [1903, ".move:"],
      [1912, "    test ax, 1"],
      [1920, "    call xms_prepare_endpoint"],
      [1920, "    call xms_prepare_endpoint"],
      [1974, "    mov ax, 0x8700"],
  ...
  },
  {
    ...
    code: [
      ...
      [1614, "%if ENABLE_EMS"],
      [1615, "    mov [es:0x67*4], word int67_handler"],
      [1617, "%else"],
      [1618, "    mov [es:0x67*4], word int67_absent_handler"],
      [2131, "%if !ENABLE_EMS"],
      [2132, "int67_absent_handler:"],
      [2133, "    mov ah, 0x80"],
      [2137, "%if ENABLE_EMS"],
      [2138, "int67_handler:"],
      [2162, ".frame:"],
      [2164, "    mov bx, EMS_FRAME_SEG"],
      [2166, ".pages:"],
      [2167, "    mov bx, EMS_TOTAL_PAGES"],
      [2172, ".alloc:"],
      [2177, "    cmp bx, EMS_TOTAL_PAGES"],
      [2179, "    mov [cs:ems_alloc_pages], bx"],
      [2187, ".map:"],
      [2192, "    cmp al, 3"],
      [2194, "    cmp bx, [cs:ems_alloc_pages]"],
      [2219, "    call ems_copy_16k"],
      [2219, "    call ems_copy_16k"],
      [2237, "    mov [cs:si+ems_map_pages], bx"],
      [2265, "    mov word [cs:ems_alloc_pages], 0"],
      [2289, "ems_clear_map:"],
