...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3405, "%if LOAD_SEG <= RELOC_SEG"],
      [3406, "%error \"LOAD_SEG must be above RELOC_SEG\""],
      [3408, "%if (kernel_end - kernel_entry) > ((LOAD_SEG - RELOC_SEG) * 16)"],
      [3409, "%error \"kernel exceeds boot relocation gap\""],
      [3411, "%if (kernel_end - kernel_entry) > ((SEC_BUF - RELOC_SEG) * 16)"],
      [3412, "%error \"kernel overlaps SEC_BUF\""],
      [3414, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3415, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3417, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3418, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3420, "%if (ROOT_SEG + ROOT_BUF_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3421, "%error \"ROOT buffer overlaps kernel stack\""],
      [3423, "%if (ROOT_SEG + ROOT_BUF_PARAS + STACK_ROOT_GUARD_PARAS) > (RELOC_SEG + (KERNEL_STACK_TOP / 16))"],
      [3424, "%error \"ROOT buffer leaves too little kernel stack guard\""],
      [3432, "%if (RELOC_SEG + (KERNEL_STACK_TOP / 16)) > MCB_START"],
      [3433, "%error \"kernel stack overlaps MCB arena\""],
      [3435, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3436, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3438, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3439, "%error \"EMS frame must be inside conventional arena\""],
      [3450, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [3451, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
