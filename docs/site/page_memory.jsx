...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4276, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4277, "%error \"kernel leaves too little HMA stack guard\""],
      [4279, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4280, "%error \"kernel exceeds boot load area\""],
      [4282, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4283, "%error \"FAT buffer overlaps CD_BUF\""],
      [4285, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4286, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4288, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4289, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4291, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [4292, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4294, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4295, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4297, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4298, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4300, "%if MCB_START >= MEM_TOP"],
      [4301, "%error \"MCB arena is empty\""],
      [4303, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4304, "%error \"EMS frame must be inside conventional arena\""],
      [4315, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4276, 4279, 4282, 4276, 4276, 4276, 4279, 4282, 4276, 4315],
