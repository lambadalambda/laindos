...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3694, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3695, "%error \"kernel leaves too little HMA stack guard\""],
      [3697, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3698, "%error \"kernel exceeds boot load area\""],
      [3700, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3701, "%error \"FAT buffer overlaps CD_BUF\""],
      [3703, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3704, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3706, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3707, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3709, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3710, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3712, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3713, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3715, "%if MCB_START >= MEM_TOP"],
      [3716, "%error \"MCB arena is empty\""],
      [3718, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3719, "%error \"EMS frame must be inside conventional arena\""],
      [3730, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      [2408, "do_terminate:"],
      [2418, "%if ENABLE_EMS"],
      [2275, "    mov word [cs:ems_alloc_pages], 0"],
      [2422, "    mov word [cs:xms_alloc_kb], 0"],
      [2423, "    call release_inherited_handles"],
      [2424, "    call close_owned_handles"],
      [2426, "    mov si, [cs:mcb_first]"],
      [2428, "    mov ds, si"],
      [2435, "    mov ax, [cs:cur_psp]"],
      [2436, "    cmp word [ds:1], ax"],
      [2438, "    mov word [ds:1], 0"],
      [2442, "    call mcb_walk_next"],
      [2446, "    call mcb_coalesce_all_free"],
      [2451, "    mov ax, [0x16]"],
  ...
  },
  {
    ...
    code: [
      ...
      [1758, "init_xms_size:"],
      [1760, "    mov word [cs:xms_total_kb], 0"],
      [1761, "    mov ah, 0x88"],
      [1762, "    int 0x15"],
      [1767, "    cmp ax, XMS_MAX_KB - 64"],
      [1769, "    mov ax, XMS_MAX_KB - 64"],
      [1771, "    mov [cs:xms_total_kb], ax"],
      [1793, "int2f_handler:"],
      [1803, "    cmp ax, 0x4300"],
      [1805, "    cmp ax, 0x4310"],
      [1810, "    mov al, 0x80"],
      [1813, "    mov bx, xms_entry"],
      [1823, "xms_entry:"],
      [1840, "    cmp ah, 0x08"],
      [1842, "    cmp ah, 0x09"],
      [1844, "    cmp ah, 0x0A"],
      [1846, "    cmp ah, 0x0B"],
      [1883, "    mov ax, [cs:xms_total_kb]"],
      [1907, "    cmp word [cs:xms_alloc_kb], 0"],
      [1893, "    cmp dx, [cs:xms_total_kb]"],
      [1895, "    mov [cs:xms_alloc_kb], dx"],
      [1910, "    mov ax, 1"],
      [1897, "    mov dx, 1"],
      [1909, "    mov word [cs:xms_alloc_kb], 0"],
      [1913, ".move:"],
      [1922, "    test ax, 1"],
      [1930, "    call xms_prepare_endpoint"],
      [1930, "    call xms_prepare_endpoint"],
      [1984, "    mov ax, 0x8700"],
  ...
  },
  {
    ...
    code: [
      ...
      [1624, "%if ENABLE_EMS"],
      [1625, "    mov [es:0x67*4], word int67_handler"],
      [1627, "%else"],
      [1628, "    mov [es:0x67*4], word int67_absent_handler"],
      [2141, "%if !ENABLE_EMS"],
      [2142, "int67_absent_handler:"],
      [2143, "    mov ah, 0x80"],
      [2147, "%if ENABLE_EMS"],
      [2148, "int67_handler:"],
      [2172, ".frame:"],
      [2174, "    mov bx, EMS_FRAME_SEG"],
      [2176, ".pages:"],
      [2177, "    mov bx, EMS_TOTAL_PAGES"],
      [2182, ".alloc:"],
      [2187, "    cmp bx, EMS_TOTAL_PAGES"],
      [2189, "    mov [cs:ems_alloc_pages], bx"],
      [2197, ".map:"],
      [2202, "    cmp al, 3"],
      [2204, "    cmp bx, [cs:ems_alloc_pages]"],
      [2229, "    call ems_copy_16k"],
      [2229, "    call ems_copy_16k"],
      [2247, "    mov [cs:si+ems_map_pages], bx"],
      [2275, "    mov word [cs:ems_alloc_pages], 0"],
      [2299, "ems_clear_map:"],
