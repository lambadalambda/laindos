...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [3735, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [3736, "%error \"kernel leaves too little HMA stack guard\""],
      [3738, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [3739, "%error \"kernel exceeds boot load area\""],
      [3741, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [3742, "%error \"FAT buffer overlaps CD_BUF\""],
      [3744, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [3745, "%error \"CD_BUF overlaps SEC_BUF\""],
      [3747, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [3748, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [3750, "%if (READ_CACHE_BUF + SECTOR_BUF_PARAS) > ROOT_SEG"],
      [3751, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [3753, "%if (ROOT_SEG + ROOT_BUF_PARAS) > MCB_START"],
      [3754, "%error \"ROOT_SEG overlaps MCB arena\""],
      [3756, "%if MCB_START >= MEM_TOP"],
      [3757, "%error \"MCB arena is empty\""],
      [3759, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [3760, "%error \"EMS frame must be inside conventional arena\""],
      [3771, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
  ...
  },
  {
    ...
    code: [
      ...
      [1429, "    je .as_set"],
      [781, "    xor ax, ax"],
      [1558, "    mov al, [cs:alloc_strat]"],
      [1436, "    cmp bl, 2"],
      [1438, "    mov [cs:alloc_strat], bl"],
      [1475, ".alloc_mem:"],
      [1558, "    mov al, [cs:alloc_strat]"],
      [1163, "    cmp al, 2"],
      [1498, "    jmp near .am_find_last"],
      [1490, "    cmp al, 1"],
      [1492, "    jmp near .am_find_best"],
      [1494, "    cmp word [cs:am_req], 1"],
      [1496, "    cmp word [cs:am_req], SMALL_ALLOC_HIGH_MAX"],
      [1498, "    jmp near .am_find_last"],
      [1608, "    mov si, [cs:mcb_first]"],
      [1621, "    cmp ax, [cs:am_req]"],
      [1515, ".am_use:"],
      [1138, "    mov ax, [cs:cur_psp]"],
      [1581, ".am_find_last:"],
      [1605, ".am_find_best:"],
      [1662, ".am_nomem:"],
      [1665, ".am_scan_largest:"],
      [1902, "    mov ax, 8"]],
  ...
  },
  {
    ...
    code: [
      ...
      [1744, "    mov si, es"],
      [1745, "    dec si"],
      [1611, "    cmp byte [ds:0], MCB_SIG_M"],
      [1628, "    cmp byte [ds:0], MCB_SIG_Z"],
      [1728, "    mov word [ds:1], 0"],
      [1729, "    call mcb_merge_free_forward"],
      [1736, ".resize_mem:"],
      [1743, "    mov [cs:rm_req], bx"],
      [1744, "    mov si, es"],
      [1620, "    mov ax, [ds:3]"],
      [1756, "    jae .rm_shrink"],
      [1764, "    cmp byte [es:0], MCB_SIG_M"],
      [1769, "    cmp word [es:1], 0"],
      [1856, "    add ax, cx"],
      [1774, "    cmp ax, bx"],
      [1809, "    mov byte [es:0], dl"],
      [1652, "    mov byte [ds:0], MCB_SIG_M"],
      [1532, "    mov word [es:3], cx"],
      [1814, "    mov word [ds:3], bx"],
      [1799, "    mov ax, [ds:3]"],
      [1809, "    mov byte [es:0], dl"],
      [1528, "    mov word [es:1], 0"],
      [1814, "    mov word [ds:3], bx"],
      [1857, "    mov [es:0x02], ax"],
      [1900, ".rm_cant_grow:"],
      [1901, "    mov bx, ax"],
  ...
  },
  {
    ...
    code: [
      ...
      [1780, "init_xms_size:"],
      [1782, "    mov word [cs:xms_total_kb], 0"],
      [1783, "    mov ah, 0x88"],
      [1784, "    int 0x15"],
      [1789, "    cmp ax, XMS_MAX_KB - 64"],
      [1791, "    mov ax, XMS_MAX_KB - 64"],
      [1793, "    mov [cs:xms_total_kb], ax"],
      [1815, "int2f_handler:"],
      [1825, "    cmp ax, 0x4300"],
      [1827, "    cmp ax, 0x4310"],
      [1832, "    mov al, 0x80"],
      [1835, "    mov bx, xms_entry"],
      [1845, "xms_entry:"],
      [1862, "    cmp ah, 0x08"],
      [1864, "    cmp ah, 0x09"],
      [1866, "    cmp ah, 0x0A"],
