...
const MEM_SECTIONS = [
  {
    ...
    code: [
      ...
      [16, "CD_BUF equ 0x0180"],
      [17, "SEC_BUF equ 0x0200"],
      [18, "READ_CACHE_BUF equ (SEC_BUF + SECTOR_BUF_PARAS)"],
      [19, "WRITE_CACHE_BUF equ 0x06A0"],
      [20, "CD_CACHE_BUF equ (WRITE_CACHE_BUF + SECTOR_BUF_PARAS)"],
      [28, "MCB_START equ 0x0B00"],
      [29, "MEM_TOP equ 0xA000"],
      [30, "ENV_PARAS equ 16"],
      [31, "ENV_SIZE_BYTES equ (ENV_PARAS * 16)"],
      [32, "ENV_OWNER_TEMP equ 0xFFFF"],
      [33, "%if (MEM_TOP & 0xFF) != 0"],
      [34, "%error \"MEM_TOP must be 256-byte aligned\""],
      [36, "MCB_SIG_M equ 'M'"],
      [37, "MCB_SIG_Z equ 'Z'"]],
    hi: [4, 6, 7, 8, 9, 11, 12, 19, 20, 28, 32, 33],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4320, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4321, "%error \"kernel leaves too little HMA stack guard\""],
      [4323, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4324, "%error \"kernel exceeds boot load area\""],
      [4326, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4327, "%error \"FAT buffer overlaps CD_BUF\""],
      [4329, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4330, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4332, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4333, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4335, "%if (READ_CACHE_BUF + READ_CACHE_PARAS) > ROOT_SEG"],
      [4336, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4338, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4339, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4341, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4342, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4344, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4345, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4347, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4348, "%error \"CD cache overlaps MCB arena\""],
      [4350, "%if MCB_START >= MEM_TOP"],
      [4351, "%error \"MCB arena is empty\""],
      [4353, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4354, "%error \"EMS frame must be inside conventional arena\""],
      [4365, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4320, 4323, 4326, 4341, 4344, 4347, 4350, 4353, 4365],
