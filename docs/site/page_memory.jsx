...
const MEM_SECTIONS = [
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [4319, "%if (HMA_OFF + (kernel_end - kernel_entry)) > (KERNEL_STACK_TOP - KERNEL_STACK_GUARD_BYTES)"],
      [4320, "%error \"kernel leaves too little HMA stack guard\""],
      [4322, "%if (kernel_end - kernel_entry) > ((MEM_TOP - LOAD_SEG) * 16)"],
      [4323, "%error \"kernel exceeds boot load area\""],
      [4325, "%if (FAT_SEG + 0x120) > CD_BUF"],
      [4326, "%error \"FAT buffer overlaps CD_BUF\""],
      [4328, "%if (CD_BUF + CD_BUF_PARAS) > SEC_BUF"],
      [4329, "%error \"CD_BUF overlaps SEC_BUF\""],
      [4331, "%if (SEC_BUF + SECTOR_BUF_PARAS) > READ_CACHE_BUF"],
      [4332, "%error \"SEC_BUF overlaps READ_CACHE_BUF\""],
      [4334, "%if (READ_CACHE_BUF + READ_CACHE_PARAS) > ROOT_SEG"],
      [4335, "%error \"READ_CACHE_BUF overlaps ROOT_SEG\""],
      [4337, "%if (ROOT_SEG + ROOT_BUF_PARAS) > WRITE_CACHE_BUF"],
      [4338, "%error \"ROOT_SEG overlaps WRITE_CACHE_BUF\""],
      [4340, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > MCB_START"],
      [4341, "%error \"WRITE_CACHE_BUF overlaps MCB arena\""],
      [4343, "%if (WRITE_CACHE_BUF + SECTOR_BUF_PARAS) > CD_CACHE_BUF"],
      [4344, "%error \"WRITE_CACHE_BUF overlaps CD cache\""],
      [4346, "%if (CD_CACHE_BUF + (CD_CACHE_SLOTS * CD_CACHE_SLOT_PARAS)) > MCB_START"],
      [4347, "%error \"CD cache overlaps MCB arena\""],
      [4349, "%if MCB_START >= MEM_TOP"],
      [4350, "%error \"MCB arena is empty\""],
      [4352, "%if ENABLE_EMS && EMS_FRAME_SEG <= MCB_START"],
      [4353, "%error \"EMS frame must be inside conventional arena\""],
      [4364, "%if ENABLE_XMS && XMS_MAX_KB > 15360"],
      [{a: "xms_backing_limit_error"}, "%error \"XMS BIOS move backing must remain below 16 MiB\""]],
    hi: [4319, 4322, 4325, 4340, 4343, 4346, 4349, 4352, 4364],
