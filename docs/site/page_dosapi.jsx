...
const DOSAPI_GROUPS = [
  {
    ...
    code: [
      ...
      [109, "    je .print_string"],
      [110, "    cmp ah, 0x01"],
      [111, "    je .read_char_echo"],
      [114, "    cmp ah, 0x06"],
      [115, "    je .direct_console_io"],
      [120, "    cmp ah, 0x0A"],
      [121, "    je .read_line_buffered"],
    ],
    regs: [
    ...
    tests: ["scripts/test_console.py", "scripts/test_write.py", "scripts/test_shell.py", "scripts/test_autoexec.py"],
  },
  {
    id: "files",
  ...
  },
  {
    ...
    verdict: "Shell-launched COM/EXE programs and overlays are supported.",
    calls: "AH=4Bh,4Ch,4Dh,62h",
    prose: [
      "When SHELL.COM starts another program, it uses EXEC. LainDOS loads the child image, builds a PSP, supplies the command tail and environment, applies EXE relocations, and restores the parent after the child exits.",
      "For users this is why typing `midemo` at A:\\> works instead of needing a direct-boot image. The same path also supports overlay loads used by programs that page code or resources into an existing process."
    ],
    codeFile: "src/kernel/int21.inc",
    code: [
      [1649, ".exec:"],
      [1650, "    cmp al, 0"],
      [1651, "    je .exec_program"],
      [1652, "    cmp al, 3"],
      [1653, "    je .exec_overlay"],
      [1678, "    call load_exec_program"],
      [1682, "    call exec_com_dyn"],
      [1685, "    call setup_exe_dyn"],
    ],
    regs: [
      ["AH", "4Bh", "EXEC"],
    ...
    tests: ["scripts/test_shell.py", "scripts/test_execparam.py", "scripts/test_execenv.py", "scripts/test_overlay.py"],
  },
  {
    id: "find",
    title: "FindFirst, FindNext, and the DTA",
    verdict: "Directory searches work across the current directory and drive-qualified paths.",
    calls: "AH=1Ah,2Fh,4Eh,4Fh,47h",
    prose: [
    ...
    code: [
      ...
      [206, "    cmp ah, 0x4F"],
      [207, "    je .find_next"],
      [3788, "    mov ax, [cs:dta_seg]"],
      [3819, "    call store_find_dta"],
    ],
    regs: [
      ["DS:DX", "DTA/path", "DTA pointer for AH=1Ah; search path for AH=4Eh"],
      ["CX", "attrs", "FindFirst attribute mask"],
      ["CF", "status", "clear on match, set when no match or bad path"],
    ],
    tests: ["scripts/test_findnext.py", "scripts/test_findattr.py", "scripts/test_findtime.py", "scripts/test_pathcanon.py"],
  },
  {
    id: "memory",
  ...
  },
  {
    ...
    code: [
      ...
      [3228, "    je .ioctl_get"],
      [3231, "    cmp al, 6"],
      [3232, "    je .ioctl_input_status"],
      [3235, "    cmp al, 8"],
      [3236, "    je .ioctl_removable_drive"],
      [3282, "    mov dx, 0x80D3"],
    ],
    regs: [
    ...
    tests: ["scripts/test_devnames.py", "scripts/test_ioctlstat.py", "scripts/test_ioctlext.py", "scripts/test_dup.py"],
  },
  {
    id: "version",
    title: "Version, drive data, date/time, and vectors",
    verdict: "Compatibility probes return stable DOS-like answers.",
    calls: "AH=19h,1Bh,1Ch,25h,29h,2Ah-30h,33h,35h,36h,38h,52h,54h",
    prose: [
    ...
    tests: ["scripts/test_versionapi.py", "scripts/test_diskfree.py", "scripts/test_datetime.py", "scripts/test_drivedata.py"],
  },
];
...
const DOSAPI_DEFERRED = [
  ...
  "FCB compatibility is intentionally narrow: filename parsing is covered, but modern handle APIs are the primary supported file path.",
...
const DOSAPI_CALLS = [
  {
    ...
    rows: [
      ...
      ["0Eh", "Select default drive", "supported", "Updates the current drive letter and returns the drive count."],
    ],
  },
  {
  ...
  },
  {
    ...
    rows: [
      ...
      ["4Bh", "EXEC", "partial", "Supports AL=00h load-and-run and AL=03h overlay load."],
      ["4Ch", "Terminate with return code", "supported", "Stores AL and returns control to the parent or shell."],
      ["4Dh", "Get return code", "supported", "Returns and clears the last child return code."],
      ...
      ["67h", "Set handle count", "stub", "Accepted as success; LainDOS uses its fixed internal table."],
    ],
  },
  {
