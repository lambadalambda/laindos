...
const SHELL_UNSUPPORTED = [
  ...
  "Batch control is deliberately narrow: labels, GOTO, IF EXIST, and nested BAT files are supported, but there is no CALL, FOR, variable expansion, pipes, or general redirection.",
...
const SHELL_SECTIONS = [
  {
    ...
    body: [
      ...
      "Every interactive line is read with AH=0Ah, checked for a standalone drive switch, matched case-insensitively against the built-in command table, and finally passed to external command lookup without uppercasing arguments.",
    ],
    ...
    code: [
      ...
      [20, "    mov ah, 0x4A"],
      [16, "    int 0x21"],
      [28, "    call run_command_tail"],
      [30, "    call run_autoexec"],
      ["", "; ..."],
      [32, "prompt:"],
      [33, "    call print_prompt"],
      [34, "    call read_line"],
      [35, "    call execute_line"],
      [36, "    jmp prompt"],
      ["", "; ..."],
      [38, "execute_line:"],
      [41, "    mov si, line_buf"],
      [42, "    call skip_command_prefix"],
      [47, "    call change_drive_command"],
      [49, "    mov bx, command_table"],
      [55, "    call cmd_match"],
      [60, "    jmp word [bx+2]"],
      [61, ".external:"],
      [62, "    call prepare_command"],
      [63, "    call run_command"],
    ],
    hi: [25, 29, 35, 44, 52, 59, 60],
  ...
  },
  {
    ...
    body: [
      ...
      "Startup-script conveniences are minimal: `PAUSE` prints `Press any key to continue . . .` and waits on INT 16h AH=0, `BREAK` is accepted as a no-op (LainDOS does not implement user-mode Ctrl-Break), `MODE CO80` issues INT 10h AH=00h AL=03h, and `MORE < file` reads the file in 4 KiB chunks, prints each line, and waits on the more prompt between screens. `PAUSE`, `MODE`, and `MORE` accept a trailing `> nul` (with or without space) and suppress the prompt; `MORE` accepts a leading `< file` (with space) to redirect input. Batch control includes labels, `GOTO`, and `IF EXIST path command` for the vendor scripts that need them.",
    ...
    code: [
      [84, "do_dir:"],
      [85, "    call parse_dir_args"],
      [95, "    call print_dir_header"],
      [99, "    mov dx, dir_pattern"],
      [100, "    mov cx, ATTR_DIR"],
      [101, "    mov ah, 0x4E"],
      [107, "    call print_dir_entry"],
      [110, "    call print_dir_wide_entry"],
      [116, "    call finish_dir_wide_row"],
      [117, "    call print_dir_summary"],
      [873, "do_copy:"],
      [874, "    call parse_copy_args"],
      [881, "    mov ah, 0x3D"],
      [889, "    mov ah, 0x3C"],
      [897, "    mov ah, 0x3F"],
      [906, "    mov ah, 0x40"],
      [919, "    mov dx, copy_success_msg"],
      [1162, "do_cd:"],
      [1165, "    mov dx, si"],
      [1166, "    mov ah, 0x3B"],
      [1174, "    mov ah, 0x47"],
      ["", "; ..."],
      [1234, "do_del:"],
      [1238, "    call del_path_has_wildcard"],
      [1243, "    mov ah, 0x41"],
      [1302, "confirm_del_prompt:"],
      [1311, "    mov ah, 0x08"],
      [1327, "del_path_has_wildcard:"],
      [1336, "    cmp al, '*'"],
      ["", "; ..."],
      [1348, "do_ren:"],
      [1353, "    call ren_paths_have_wildcard"],
      [1355, "    call ren_dst_has_path"],
      [1361, "    mov ah, 0x56"],
      [1423, "ren_dst_has_path:"],
      [1429, "    cmp al, ':'"],
      ["", "; ..."],
      [1522, "do_goto:"],
      [1526, "    call batch_seek_label"],
      [1530, "do_if:"],
      [1532, "    mov di, exist_arg"],
      [1561, "    mov ax, 0x4300"],
      [1565, "    call if_tail_is_bare_label"],
      [1574, "    call execute_line"],
      [1577, "    call batch_seek_label"],
      [1614, "batch_seek_label:"],
      [1634, "    call batch_read_line"],
      [1642, "    call batch_label_match"],
      ["", "; ..."],
      [1731, "do_pause:"],
      [1735, "    mov dx, pause_msg"],
      [1737, "    call wait_key"],
      [1744, "do_mode:"],
      [1748, "    mov ax, 0x0003"],
      [1758, "do_more:"],
      [1764, "    call more_display_file"],
      [1850, "parse_more_args:"],
      [1869, "    cmp al, '<'"],
      [1882, "    mov di, nul_arg"],
      ["", "; ..."],
      [2133, "change_drive_command:"],
      [2137, "    mov al, [si]"],
      [2161, "    mov dl, al"],
      [2163, "    mov ah, 0x0E"],
      [2183, "print_prompt:"],
      [2187, "    mov ah, 0x47"],
      [2195, "    mov dx, prompt_end"],
      ["", "; ..."],
      [2976, "command_table:"],
      [2979, "    dw dir_cmd, do_dir"],
      [2980, "    dw cd_cmd, do_cd"],
      [2987, "    dw copy_cmd, do_copy"],
      [2988, "    dw del_cmd, do_del"],
      [2990, "    dw ren_cmd, do_ren"],
      [2996, "    dw if_cmd, do_if"],
      [2997, "    dw goto_cmd, do_goto"],
      [2998, "    dw pause_cmd, do_pause"],
      [3000, "    dw mode_cmd, do_mode"],
      [3001, "    dw more_cmd, do_more"],
    ],
  ...
  },
  {
    ...
    file: "programs/shell.asm",
    code: [
      [2274, "prepare_command:"],
      [2277, "    mov byte [command_has_ext], 0"],
      [2280, "    mov byte [tail_has_args], 0"],
      [2290, "    cmp al, '.'"],
      [2304, "    mov byte [command_has_path], 1"],
      [2317, "    mov al, '.'"],
      [2320, "    mov al, 'C'"],
      [2322, "    mov al, 'O'"],
      [2324, "    mov al, 'M'"],
      [2329, "    call build_cmd_tail"],
      ["", "; ..."],
      [2361, "run_command:"],
      [2364, "    call run_current_command"],
      [2372, "    call run_current_command"],
      [2380, "    call run_batch"],
      [2386, "    call run_path_exec"],
      [2386, "    call run_path_exec"],
      [2398, "    call run_path_batch"],
      [2402, "    call command_ext_is_bat"],
      [2421, "    mov ah, 0x4D"],
      [2422, "    int 0x21"],
      [2434, "command_ext_is_bat:"],
      [2439, "    and al, 0xDF"],
      [2439, "    and al, 0xDF"],
      [2462, "run_current_command:"],
      [2477, "    mov ax, 0x4B00"],
      [2478, "    int 0x21"],
      [2487, "run_autoexec:"],
      [2490, "    mov dx, autoexec_name"],
      [2491, "    call run_batch_named"],
      [2502, "run_batch_named:"],
      [2505, "    call batch_sync_position"],
      [2508, "    inc byte [batch_active]"],
      [2509, "    push word [batch_handle]"],
      [2511, "    mov ah, 0x3D"],
      [2515, "    mov word [batch_buf_pos], 0"],
      [2520, "    call batch_read_line"],
      [2524, "    call execute_line"],
      [2532, "    pop word [batch_handle]"],
      [2535, "    dec byte [batch_active]"],
      ["", "; ..."],
      [2749, "batch_read_line:"],
      [2753, "    call batch_read_char"],
      [2764, "    cmp cx, 63"],
      [2769, "    call batch_read_char"],
      [2777, "    xor al, al"],
      [2789, "batch_read_char:"],
      [2793, "    mov bx, [batch_buf_pos]"],
      [2799, "    mov ah, 0x3F"],
      [2800, "    int 0x21"],
      [2807, ".have_char:"],
      [2822, "batch_sync_position:"],
      [2834, "    mov ax, 0x4201"],
      ["", "; ..."],
      [2549, "run_path_exec:"],
      [2553, "run_path_batch:"],
      [2556, "run_path_command:"],
      [2557, "    cmp byte [command_has_path], 0"],
      [2559, "    call find_path_value"],
      [2562, "    call build_path_candidate"],
      [2566, "    call run_candidate_exec"],
      [2569, "    call run_batch_path"],
      ["", "; ..."],
      [2587, "find_path_value:"],
      [2600, "    mov ah, 0x62"],
      [2604, "    mov ax, [es:0x2C]"],
      [2614, "    mov si, path_env_name"],
      [2632, ".found:"],
      [2634, "    mov [path_env_seg], es"],
      ["", "; ..."],
      [2651, "build_path_candidate:"],
      [2676, "    cmp al, ';'"],
      [2711, "    mov al, '\\'"],
      [2715, "    mov si, command_name"],
      [2729, "    clc"],
    ],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [1483, "init_environment:"],
      [1492, "    call alloc_exec_environment"],
      [1500, "    call write_environment_vars"],
      [1501, "    mov ax, 1"],
      [1502, "    stosw"],
      [1509, "    mov si, fname_exe"],
      ["", "; ..."],
      [1543, "write_environment_vars:"],
      [1549, "    mov si, env_comspec_name"],
      [1554, "    mov si, env_path_name"],
      [1562, "    mov si, env_blaster"],
      [1564, "    mov si, env_prompt"],
      [1566, "    xor al, al"],
      [1589, "env_copy_drive_root:"],
      [1590, "    mov al, [cs:dos_drive_letter]"],
      ["", "; ..."],
      [3066, "env_comspec_name: db \"COMSPEC=\", 0"],
      [3067, "env_path_name: db \"PATH=\", 0"],
      [3068, "env_shell_name: db \"SHELL.COM\", 0"],
      [3069, "env_bin_dir: db \"BIN\", 0"],
      [3070, "env_blaster: db \"BLASTER=A220 I5 D1 H5 P330 T6\", 0"],
      [3071, "env_prompt: db \"PROMPT=$P$G\", 0"],
    ],
