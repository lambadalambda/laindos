...
const SHELL_COMMANDS = [
  ...
  ["DIR", "List *.* in the current directory, including subdirectories."],
...
const SHELL_EXAMPLES = [
  ["DIR", "List the boot floppy. The live page image includes SHELL.COM, KERNEL.SYS, and the Monkey Island demo files."],
...
const SHELL_TESTS = [
  ["Interactive shell", "scripts/test_shell.py", "Drives VER, CLS, DIR, TYPE, ECHO, REM, COM/EXE/BAT launch, directories, and error paths."],
...
const SHELL_SECTIONS = [
  ...
  },
  {
    ...
    body: [
      "The built-ins intentionally stay narrow. `DIR` lists the current directory with FindFirst/FindNext. `CD`, `MD`, and `RD` call the matching directory APIs. `TYPE` opens a file and copies it to handle 1.",
    ...
    code: [
      ...
      [103, "    mov ah, 0x3B"],
      ["", "; ..."],
      [246, "change_drive_command:"],
      [268, "    mov dl, al"],
      [270, "    mov ah, 0x0E"],
      [290, "print_prompt:"],
      [294, "    mov ah, 0x47"],
      [302, "    mov dx, prompt_end"],
      ["", "; ..."],
      [919, "command_table:"],
      [920, "    dw exit_cmd, exit_shell"],
      [922, "    dw dir_cmd, do_dir"],
      [923, "    dw cd_cmd, do_cd"],
      [926, "    dw type_cmd, do_type"],
      [928, "    dw echo_cmd, do_echo"],
      [929, "    dw rem_cmd, do_rem"],
    ],
    hi: [78, 84, 246, 270, 290, 294, 919],
  ...
  },
  {
    ...
    code: [
      [351, "prepare_command:"],
      [354, "    mov byte [command_has_ext], 0"],
      [392, "    cmp byte [command_has_ext], 0"],
      [394, "    mov al, '.'"],
      [397, "    mov al, 'C'"],
      [399, "    mov al, 'O'"],
      [401, "    mov al, 'M'"],
      [406, "    call build_cmd_tail"],
      ["", "; ..."],
      [438, "run_command:"],
      [441, "    call run_current_command"],
      [449, "    call run_current_command"],
      [457, "    call run_batch"],
      [463, "    call run_path_exec"],
      [475, "    call run_path_batch"],
      [498, "    mov ah, 0x4D"],
      [499, "    int 0x21"],
      [530, "run_current_command:"],
      [545, "    mov ax, 0x4B00"],
      [546, "    int 0x21"],
    ],
    hi: [351, 406, 438, 441, 457, 463, 475, 498, 545],
  ...
  },
  {
    ...
    code: [
      [555, "run_autoexec:"],
      [558, "    mov dx, autoexec_name"],
      [559, "    call run_batch_named"],
      ["", "; ..."],
      [570, "run_batch_named:"],
      [571, "    cmp byte [batch_active], 0"],
      [573, "    mov byte [batch_active], 1"],
      [575, "    mov ah, 0x3D"],
      [584, "    mov ah, 0x3F"],
      [596, "    mov si, batch_buf"],
      [598, "    call batch_read_line"],
      [604, "    call execute_line"],
      [610, "    mov byte [batch_active], 0"],
      ["", "; ..."],
      [801, "batch_read_line:"],
      [823, "    cmp cx, 63"],
      [825, "    stosb"],
    ...
    hi: [555, 559, 570, 571, 598, 604, 801, 823],
  ...
  },
  {
    ...
    code: [
      [623, "run_path_exec:"],
      [627, "run_path_batch:"],
      [630, "run_path_command:"],
      [631, "    cmp byte [command_has_path], 0"],
      [633, "    call find_path_value"],
      [636, "    call build_path_candidate"],
      [640, "    call run_candidate_exec"],
      [643, "    call run_batch_path"],
      ["", "; ..."],
      [663, "find_path_value:"],
      [670, "    mov ah, 0x62"],
      [676, "    mov ax, [es:0x2C]"],
      [684, "    mov si, path_env_name"],
      [704, ".found:"],
      [706, "    mov [path_env_seg], ax"],
      ["", "; ..."],
      [726, "build_path_candidate:"],
      [746, "    cmp al, ';'"],
      [773, "    mov al, '\\'"],
      [780, "    mov si, command_name"],
      [788, "    clc"],
    ],
    hi: [630, 631, 633, 636, 670, 676, 684, 726, 746, 773],
