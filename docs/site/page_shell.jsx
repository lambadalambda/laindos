...
const SHELL_TESTS = [
  ...
  ["Batch builtins", "scripts/test_shell_batch_builtins.py", "Exercises PAUSE, BREAK, MODE CO80, MORE < file, and the NUL redirect forms used by vendor batch files."],
...
const SHELL_SECTIONS = [
  ...
  },
  {
    ...
    body: [
      ...
      "Startup-script conveniences are minimal: `PAUSE` prints `Press any key to continue . . .` and waits on INT 16h AH=0, `BREAK` is accepted as a no-op (LainDOS does not implement user-mode Ctrl-Break), `MODE CO80` issues INT 10h AH=00h AL=03h, and `MORE < file` reads the file in 4 KiB chunks, prints each line, and waits on the more prompt between screens. `PAUSE`, `MODE`, and `MORE` accept a trailing `> nul` (with or without space) and suppress the prompt; `MORE` accepts a leading `< file` (with space) to redirect input.",
    ...
    code: [
      ...
      [1531, "    mov ax, 0x0003"],
      [1541, "do_more:"],
      [1551, "parse_pause_args:"],
      [1587, "parse_mode_args:"],
      [1633, "parse_more_args:"],
      ["", "; ..."],
      [1916, "change_drive_command:"],
      [1938, "    mov dl, al"],
      [1940, "    mov ah, 0x0E"],
      [1960, "print_prompt:"],
      [1964, "    mov ah, 0x47"],
      [1972, "    mov dx, prompt_end"],
      ["", "; ..."],
      [2662, "command_table:"],
      [2663, "    dw exit_cmd, exit_shell"],
      [2665, "    dw dir_cmd, do_dir"],
      [2666, "    dw cd_cmd, do_cd"],
      [2667, "    dw cd_parent_cmd, do_cd_parent"],
      [2668, "    dw cd_root_cmd, do_cd_root"],
      [2669, "    dw chdir_cmd, do_cd"],
      [2670, "    dw md_cmd, do_md"],
      [2671, "    dw mkdir_cmd, do_md"],
      [2672, "    dw rd_cmd, do_rd"],
      [2673, "    dw rmdir_cmd, do_rd"],
      [2674, "    dw copy_cmd, do_copy"],
      [2675, "    dw del_cmd, do_del"],
      [2676, "    dw erase_cmd, do_del"],
      [2677, "    dw ren_cmd, do_ren"],
      [2678, "    dw rename_cmd, do_ren"],
      [2679, "    dw type_cmd, do_type"],
      [2680, "    dw cls_cmd, do_cls"],
      [2681, "    dw echo_cmd, do_echo"],
      [2682, "    dw rem_cmd, do_rem"],
      [2683, "    dw pause_cmd, do_pause"],
      [2684, "    dw break_cmd, do_break"],
      [2685, "    dw mode_cmd, do_mode"],
      [2686, "    dw more_cmd, do_more"],
    ],
    hi: [80, 97, 103, 106, 112, 113, 867, 875, 883, 891, 900, 913, 1228, 1232, 1237, 1294, 1319, 1340, 1345, 1347, 1353, 1415, 1514, 1524, 1527, 1541, 1916, 1940, 1960, 1964, 2672, 2677, 2679, 2681, 2683, 2684, 2685, 2686, 2687, 2688, 2689, 2690, 2691, 2692, 2693, 2694, 2695, 2696],
  ...
  },
  {
    ...
    file: "programs/shell.asm",
    code: [
      [2066, "prepare_command:"],
      [2069, "    mov byte [command_has_ext], 0"],
      [2107, "    cmp byte [command_has_ext], 0"],
      [2109, "    mov al, '.'"],
      [2112, "    mov al, 'C'"],
      [2114, "    mov al, 'O'"],
      [2116, "    mov al, 'M'"],
      [2121, "    call build_cmd_tail"],
      ["", "; ..."],
      [2153, "run_command:"],
      [2156, "    call run_current_command"],
      [2164, "    call run_current_command"],
      [2172, "    call run_batch"],
      [2178, "    call run_path_exec"],
      [2190, "    call run_path_batch"],
      [2213, "    mov ah, 0x4D"],
      [2214, "    int 0x21"],
      [2245, "run_current_command:"],
      [2260, "    mov ax, 0x4B00"],
      [2261, "    int 0x21"],
      [2270, "run_autoexec:"],
      [2273, "    mov dx, autoexec_name"],
      [2274, "    call run_batch_named"],
      [2285, "run_batch_named:"],
      [2286, "    cmp byte [batch_active], 0"],
      [2288, "    mov byte [batch_active], 1"],
      [2290, "    mov ah, 0x3D"],
      [2299, "    mov ah, 0x3F"],
      [2302, "    mov si, batch_buf"],
      [2313, "    call batch_read_line"],
      [2319, "    call execute_line"],
      [2325, "    mov byte [batch_active], 0"],
      ["", "; ..."],
      [2516, "batch_read_line:"],
      [2538, "    cmp cx, 63"],
      [2540, "    stosb"],
      [2548, ".eof:"],
      [2549, "    stc"],
      ...
      [2338, "run_path_exec:"],
      [2342, "run_path_batch:"],
      [2345, "run_path_command:"],
      [2346, "    cmp byte [command_has_path], 0"],
      [2348, "    call find_path_value"],
      [2351, "    call build_path_candidate"],
      [2355, "    call run_candidate_exec"],
      [2358, "    call run_batch_path"],
      ["", "; ..."],
      [2378, "find_path_value:"],
      [2385, "    mov ah, 0x62"],
      [2391, "    mov ax, [es:0x2C]"],
      [2399, "    mov si, path_env_name"],
      [2419, ".found:"],
      [2421, "    mov [path_env_seg], ax"],
      ["", "; ..."],
      [2441, "build_path_candidate:"],
      [2461, "    cmp al, ';'"],
      [2488, "    mov al, '\\'"],
      [2495, "    mov si, command_name"],
      [2503, "    clc"],
    ],
    hi: [2355, 2356, 2358, 2361, 2395, 2401, 2409, 2451, 2471, 2498],
