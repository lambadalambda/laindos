...
const CASES = [
  {
    ...
    code: [
      [446, "close_root_handle:"],
      [448, "    mov ax, [cs:si+handles+H_DIR_LBA]"],
      [450, "    je .mark_free"],
      [454, "    mov al, [cs:si+handles+H_MODE]"],
      [455, "    and al, 7"],
      [456, "    jz .mark_free"],
      [457, "    call wf_flush_handle_dir_entry"],
      [461, ".mark_free:"],
      [463, "    call release_handle_slot"],
    ],
    hi: [454, 455, 456],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [202, "    ; arena ends at the BIOS conventional-memory line, not at 640K: the"],
      [212, "    mov ax, [mem_kib]"],
      [213, "    mov cl, 6"],
      [214, "    shl ax, cl"],
      [215, "    cmp ax, MEM_TOP"],
      [216, "    jbe .arena_top_ok"],
      [217, "    mov ax, MEM_TOP"],
      [218, ".arena_top_ok:"],
  ...
  },
  {
    ...
    file: "src/kernel.asm",
    code: [
      [1236, "floppy_media_check:"],
      [1252, "    sub ax, [cs:floppy_check_tick]"],
      [1253, "    cmp ax, 36"],
      [1255, "    mov ah, 0x16"],
      [1257, "    int 0x13"],
      [1258, "    cmp ah, 0x06"],
      [1279, "    repe cmpsw"],
      [1281, "    call floppy_media_remount"],
    ],
    hi: [1252, 1255, 1279],
  ...
  },
  {
    ...
    code: [
      [2041, "cd_audio_or_busy:"],
      [2044, "    call cd_audio_subchannel"],
      [2046, "    mov cx, CD_BUF"],
      [2048, "    cmp byte [es:1], 0x11"],
      [2050, "    or ax, 0x0200"],
    ],
    hi: [2048, 2050],
  ...
  },
  {
    ...
    background: [
      ...
      "LainDOS's boot launcher - the path that starts the test program named at build time - predated all of this nuance. It allocated a block sized to the file plus a few kilobytes of slack - placed high in the arena, just under the BIOS's own work area, which matters later - and the COM entry convention then started SP at the top of that small block, only a few kilobytes above the program's own data. The two-ends-of-the-room layout still held, but the room had shrunk to a closet: every boot-launched test program in the suite had its downward-growing stack parked a few kilobytes from its upward-growing buffers without knowing it.",
    ],
    scene:
