...
const PROGRAM_SECTIONS = [
  {
    ...
    code: [
      [1950, ".exec:"],
      [1951, "    cmp al, 0"],
      [1952, "    je .exec_program"],
      [1953, "    cmp al, 1"],
      [1954, "    je near .exec_load"],
      [1955, "    cmp al, 3"],
      [1956, "    je .exec_overlay"],
      [2038, "    mov [cs:exec_param_off], bx"],
      [2039, "    mov [cs:exec_param_seg], es"],
      [2040, "    mov [cs:exec_path_off], dx"],
      [2041, "    mov [cs:exec_path_seg], ds"],
      [2042, "    call load_exec_program"]],
    hi: [1955, 2042, 2042],
  ...
  },
  {
    ...
    file: "src/kernel/exec.inc",
    code: [
      [528, "    mov word [cs:exec_env_src_seg], 0"],
      [471, "    mov ax, [cs:exec_param_seg]"],
      [534, "    mov ax, [bx]"],
      [541, "    mov [cs:exec_env_src_seg], ax"],
      [543, ".inherit_parent_env:"],
      [552, "    mov ax, [0x2C]"],
      [559, "    mov [cs:exec_env_src_seg], ax"],
      [583, "    call alloc_exec_environment"],
      [594, ".copy_env:"],
      [609, "    call write_environment_vars"],
      [615, "    mov ax, 1"],
      [616, "    stosw"],
      [617, "    mov ds, [cs:exec_path_seg]"],
  ...
  },
  {
    ...
    code: [
      [1480, "build_psp:"],
      [1488, "    mov byte [es:0x00], 0xCD"],
      [1489, "    mov byte [es:0x01], 0x20"],
      [1502, "    mov ax, [0x22*4]"],
      [1518, "    mov word [es:0x32], MAX_HANDLES"],
      [1520, "    mov [es:0x36], ax"],
      [1579, "    mov bx, [cs:cur_psp]"],
      [1580, "    mov [es:0x16], bx"],
      [1582, "    mov bx, [cs:exec_env_seg]"],
      [1583, "    mov [es:0x2C], bx"],
      [1587, "    call assign_exec_environment_owner"],
      [{a: "build_psp_copy_fcbs"}, "    call exec_copy_default_fcbs"]],
    hi: [1480, 1480, 1518, 1579, 1579],
  ...
  },
  {
    ...
    file: "src/kernel/exec.inc",
    code: [
      [1708, "    mov ax, [cs:exe_reloc_count]"],
      [139, "    cmp dx, 0x1C"],
      [1750, "    mov ax, [cs:prog_seg]"],
      [1715, "    call build_psp"],
      [1732, "    mov di, [bx]"],
      [1733, "    mov ax, [bx+2]"],
      [1742, "    mov ax, [es:di]"],
      [1739, "    add ax, [cs:exe_load_seg]"],
      [1744, "    mov [es:di], ax"],
      [1778, "    call exec_exe_dyn"],
      [1830, "exec_com_dyn:"],
      [1810, "    mov ss, ax"],
      [1858, "    push word 0x0100"],
      [{a: "exec_com_entry_jump"}, "    retf"]],
    hi: [139, 1708, 1708, 1744, 1810, {a: "exec_com_entry_jump"}],
  ...
  },
  {
    ...
    code: [
      [2104, ".exec_overlay:"],
      [2112, "    mov [cs:ov_param_off], bx"],
      [2117, "    mov ax, [es:bx]"],
      [2118, "    mov [cs:ov_load_seg], ax"],
      [2119, "    mov ax, [es:bx+2]"],
      [2120, "    mov [cs:ov_reloc_seg], ax"],
      [2248, "    call resolve_path"],
      [2156, "    call load_overlay_direct"],
      [831, "    xor ax, ax"],
      [550, "    jmp iret_nc"]],
    hi: [2112, 2119, 2120, 2156],
  ...
  },
  {
    ...
    body: [
      "Normal termination first loads the child MCB's EXEC resume state, then clears transient hardware state, releases inherited handles, closes child-owned handles, frees child-owned MCBs, coalesces the arena, restores the parent PSP from PSP:16h, and jumps back through the saved EXEC frame.",
    ...
    code: [
      [3099, "do_terminate:"],
      [3103, "    call load_exec_resume_state"],
      [3105, "    call restore_irq1_null_mask"],
      ...
      [3131, "    mov ax, [0x16]"],
      [3133, "    mov [cs:cur_psp], ax"],
      [3142, "    mov ax, [cs:term_resume_ss]"],
      [3145, "    mov sp, [cs:term_resume_sp]"],
      [3146, "    mov ax, [cs:term_resume_kbase]"],
      [{a: "do_terminate_return_to_parent"}, "    jmp exec_resume_parent"]],
    hi: [3099, 3103, 3114, 3115, 3117, 3123, 3126, 3133, 3142, {a: "do_terminate_return_to_parent"}],
    tests: ["scripts/test_retcode.py", "scripts/test_termflush.py", "scripts/test_tsr.py", "scripts/test_nestexec.py"],
  }];
