...
- The CD-ROM code pushed the kernel close to the old `0300:0000` relocation gap, so the relocated kernel moved to `0280:0000` while keeping the stack top at physical `0FC00` and keeping the `0200:0000` CD scratch buffer below the kernel.
