...
- DONE: Stunt Island verified on the faithful kernel. The installer from `build/extras_hd.img` was driven to completion in QEMU (defaults: mouse, PC speaker; ENTER x3) producing `C:\STUNTISL`, and the installed image was saved back to `build/stunt_xmsfix_hd.img` for `scripts/run_stunt_island.py`. With the current kernel patched in, the game plays the Disney intro reel and reaches the interactive "Do you want to be in the competition?" prompt at ~2 minutes, with the BIOS tick advancing at every checkpoint. That is further than the shim-era kernel ever got under QEMU -- the 2026-05-30 entry recorded a post-intro black screen with a frozen tick there. The forced-IF shim was a partial workaround; the faithful model (programs start with IF=1, caller IF preserved, ticks serviced during DOS calls) resolves the post-intro wait outright.
...
- Added `tests/programs/cdfile.asm` and `scripts/test_cd_file.py`; the focused smoke mounts the generated ISO as `D:`, verifies `AH=0Eh`/`AH=19h`, rejects `AH=3Ch` create on read-only media, and opens/reads `D:\HELLO.TXT` through DOS handle APIs.
- Added `tests/programs/cdsubdir.asm` and `scripts/test_cd_subdir.py`; the focused smoke verifies DOS handle open/read for `D:\SUBDIR\HELLO.TXT`, matching the Sam & Max payload layout under `SAMNMAX`.
- Added `tests/programs/cdfind.asm` and `scripts/test_cd_find.py`; the focused smoke enumerates `D:\*.*` through DOS `AH=4Eh/4Fh`, verifies two ISO root files appear in DTA name order, and confirms `FindNext` eventually returns no more matches.
- Added `tests/programs/cdmscdex.asm` and `scripts/test_cd_mscdex.py`; the focused smoke verifies MSCDEX `INT 2Fh AX=1500h/150Bh/150Ch/150Dh` install, drive check, version, and drive-list probes for the mounted `D:` CD-ROM.
- Added `tests/programs/cdexec.asm` and `scripts/test_cd_exec.py`; the focused smoke verifies DOS `EXEC` can launch a COM file at `D:\CDHELLO.COM` and an EXE file at `D:\SUBDIR\HELLO.EXE` from a generated ISO.
- Extended the CD `EXEC` smoke to cover current-directory relative paths and `INT 21h AX=4B03h` overlay loads from `D:` after Sam & Max reached `DOS4GW.EXE` but failed its overlay load.
- Added `scripts/extract_mode1_2352.py`, `tests/programs/sammaxcd.asm`, and `scripts/test_sammax_cd_files.py`; the vendor-gated smoke extracts the Sam & Max cue/bin data track to 2048-byte sectors and verifies LainDOS can read `D:\SAM.EXE`, `D:\SAMNMAX\SAMNMAX.EXE`, and `D:\SAMNMAX\MONSTER.SOU`.
- Added `scripts/test_sammax_cd_start.py`; the vendor-gated startup smoke boots the shell, switches to `D:\SAMNMAX`, starts `SAMNMAX.EXE`, sends Enter at the sound-driver prompt, and verifies an active framebuffer after DOS/4GW starts.
- The CD BIOS read wrapper initially failed because the caller put the BIOS drive in `DL` and then zeroed `DX` for the high LBA before `INT 13h AH=42h`, so the BIOS read used drive 0. The fix stores the BIOS drive separately in `cd_io_drive` and reloads `DL` immediately before the BIOS call.
- The optional CD mount path also needed carry normalization: failure to find a CD must not propagate as an invalid FAT boot BPB. Floppy boots with no hard disk now still attempt to expose an attached CD as `D:`.
- The CD-ROM code pushed the kernel close to the old `0300:0000` relocation gap, so the relocated kernel first moved to `0280:0000` while keeping the stack top at physical `0FC00` and keeping the CD scratch buffer below the kernel.
- The relocation move exposed `tests/programs/mouseindos.asm`'s stale `KERNEL_SEG=0340h` constant. `scripts/test_mouseindos.py` now reads `RELOC_SEG` from `src/memory.inc` and passes `-DKERNEL_SEG`, matching its existing listing-derived offset flow.
- CD overlay support consumed the remaining guard before `SEC_BUF`, so `CD_BUF` moved down to `0180:0000`, the relocated kernel moved to `0200:0000`, and `KERNEL_STACK_TOP` moved to `DC00h` to keep the physical stack top at `0FC00h` without shrinking the MCB arena.
...
## 2026-06-11 Documentation Accuracy Audit

- A semantic audit of README.md and docs/site/ found prose that lagged the implementation: EXEC `AL=01h` load-only documented as unimplemented (it works and fills the parameter block), `AH=34h` missing from the dosapi tables, pre-CD buffer segments (`SEC_BUF 0B00` era) in the filesystem page, and pre-HMA/pre-0x0B00 values (arena at 0x1000, kernel CS 0340, 99BFh/614K free) in the boot walkthrough.
- Root cause for a site-wide rendering regression: the doc-line auto-fixer re-points `code:` excerpt entries but never updates `hi:` highlight arrays or annotation keys, so over successive renumberings every page's excerpt highlights stopped matching and annotation line chips went stale. `scripts/check_docs_sync.py` never validated those keys.
- Recovery was mostly mechanical: for each stale key, walk the doc file's git history to the newest revision where the block's keys matched its excerpt lines, map the key to the excerpt text it pointed at, and re-key it to that text's current line number (233 keys). About 25 keys whose target text had been rewritten (or which had drifted before the oldest matching revision) were re-keyed by hand; three now use `@anchor` entries.
...
## 2026-06-11 Civilization Bring-Up: EXEPACK Placement And LOADFIX

- A bare `CIV` launch under LainDOS fails with `Packed file is corrupt`: CIV.EXE is EXEPACK-compressed and its unpacker corrupts itself when loaded below segment 1000h. With `MCB_START=0x0B00` this is faithful behavior — real MS-DOS 5 in the HMA produced the same failure on the same class of executables, which is why DOS 5 shipped LOADFIX.COM. A probe kernel with the arena at 0x1000 starts the game normally, confirming the placement diagnosis.
- `programs/loadfix.asm` now implements LOADFIX: one-paragraph probes (which stay on the allocator's first-fit path; 2..SMALL_ALLOC_HIGH_MAX-paragraph requests are biased to last fit) are grown to fill each free gap below segment 1000h, the target is EXEC'd with the remaining command tail (with .COM/.EXE tried for extensionless names), and the child's exit code passes through. `scripts/test_loadfix.py` failed before the implementation and passes after: bare child PSP at 0B24h, LOADFIX child at 1012h, tail forwarded, error paths set exit code 1. One implementation bug en route: `append_ext` ran `rep movsb` with ES still pointing at the last freed probe block, so the extension landed outside the program segment and every EXEC failed; the fix reloads ES=CS.
- With `LOADFIX CIV` the game reaches its graphics/sound/input menus and the animating VGA intro. It then stalls at the "A MICROPROSE PRESENTATION" card: the game hooks INT 08 directly and the BIOS tick drops to one third of 18.2/s (a chain-every-3rd handler whose expected PIT speed-up never engages); the local SAHF-patched QEMU exits with `run-time error R6003 - integer divide by 0` after ~90s, stock QEMU holds the card indefinitely, and `-icount shift=6` and the AdLib sound path change nothing.
- The control experiment pins the stall class: FreeDOS 1.4 on the same QEMU (game on a `fat:rw:` virtual drive) dies with the identical R6003 at the same card, with the BIOS tick fully frozen — an emulator PIT-timing interaction, not a LainDOS DOS-semantics gap. Recorded as an open issue with an 86Box cross-check as the next step.
- `scripts/test_civ_smoke.py` (vendor-gated, `make test-civ-smoke`) verifies the era failure message for the bare launch, the menus via LOADFIX, the animating intro framebuffer, and an advancing BIOS tick.

## 2026-06-11 Headless 86Box With HTTP RPC

- Built a headless, scriptable 86Box for automated tests from the local 86Box checkout: the SDL (QT=OFF) frontend plus the localhost HTTP RPC from the checkout's in-progress PC-98 work (`86BOX_RPC_PORT` -> /status, /key, /screenshot, /monitor, /exit), extracted as a clean PC-only patch (docs/86box-rpc.patch, with the portable OpenAL include fix it needs to build on macOS). Run with `SDL_VIDEODRIVER=dummy` there is no window; `scripts/box86lib.py` wraps launch (through testlib.start_qemu, so serial chunk readers work unchanged), AT set-1 typing, screenshots, and a pure-stdlib PNG stats reader that matches ppm_stats byte for byte.
- One wedge cost most of the debugging time: a profile with `serial1_device = stdio` but without `[Virtual Console (COM) #1] mode = 0` makes the char device open its pseudoterminal mode instead, and the first guest serial byte spins forever in `char_stdio_write` against the reader-less PTY. The spin happens inside `serial_transmit_timer` under `pc_run`'s blit mutex, so the emulator freezes and RPC `/screenshot` (which takes `startblit()`) hangs with it — `sample` of the stuck process showed the whole chain. The fix is one config line; recorded in docs/emulator_workflows.md.
- Other gotchas: `86BOX_RPC_PORT` starts with a digit so it needs env(1) rather than a shell prefix assignment; fresh profiles stop at the Award CMOS-checksum F1 prompt on first boot (cleared via `/key?scan=0x3b`); the screenshot PNG is written by the blit path in the emulated-video thread, so it works fine under the dummy video driver.
- The first consumer settles the Civilization question: under 86Box the same image and LOADFIX launch run through the intro to the interactive title menu with no R6003 — the QEMU PIT/INT 08 stall class is confirmed emulator-side and the issue is archived with the 86Box workaround documented. `make test-civ-86box` automates the cross-check (vendor zip + headless binary + ROM set gated).
...
## 2026-06-12 Micro Machines 2: HD-Boot Floppy A: And Media Change

- MM2's four floppies hold `.SHR` bundles only the real Codemasters installer can unpack, so the bring-up required running the genuine floppy installer from `A:` — which immediately exposed that hard-disk boots aliased `A:`/`B:` to the boot disk: `A:\INSTALL` reported "Bad command or file name" while `DIR A:` listed the hard disk's own root.
- Fix 1: `mount_bios_floppy_a` mounts BIOS drive 00h as `A:` during hard-disk boot (BPB read + parse, slot 0 registration), with the C: globals restored afterwards via a full `activate_drive`; when no readable floppy is present the old alias behavior remains.
- Fix 2: the INT 13h retry loop swallowed the change-line error (AH=06h) and would re-read a swapped disk with the old volume state — stale FAT interpretation, silent corruption class. `floppy_media_remount` hooks error 06h in `sector_io_loop`: drive reset, BPB re-read, `load_active_volume_buffers`, FAT/read-cache invalidation, CWD-to-root for slot 0, then the in-flight I/O retries with its saved kio state.
- `scripts/test_hdfloppy.py` pins both behaviors (TDD: failed with "File not found" for the floppy file before the kernel work). Harness note: `pmemsave` text dumps silently fail with absolute paths (the known monitor parsing pitfall) — the test asserts on the serial stream positionally instead.
- With both fixes, the installer runs end to end (language, destination, SHR unpack with three prompted swaps picked up by the remount, sound setup), and the installed game launches through DOS/4GW 1.97 to its interactive manual-symbol copy-protection screen. The kernel insertions shifted `kernel.asm` line numbers enough to trip the new hi/annotation-key gate in check_docs_sync — the history-based rekey tool re-pointed 51 keys mechanically.
- `make test-mm2-smoke` covers the full pipeline; the suite is 141/141 with `test_hdfloppy` in the default ladder.

## 2026-06-12 Wing Commander: Media Check For Cache-Served Floppies

- Wing Commander's Origin installer prompts for disk 2 while `A:` is the *current* drive — and the swap was invisible: every lookup after the prompt was served from the cached FAT/root in RAM, no physical read ever happened, so the MM2-era change-line hook never fired and `DIR` listed the old disk forever. Reproduced minimally at the shell (`a:` → swap → `type disk2.txt` → "File not found", stale listing). The MM2 fix only covered swaps that surface as INT 13h error 06h on an actual read.
- The faithful answer is real DOS's MEDIA CHECK: before trusting buffers for a removable drive, ask the BIOS whether the disk changed. `floppy_media_check` runs in `activate_drive`'s same-drive path with DOS's own 2-second rule (a floppy verified within 36 ticks is assumed unchanged), then INT 13h AH=16h. A clean "no change" is trusted; the catch is that QEMU's change line *latches*: AH=16h returns 06h forever, even right after successful reads (probe program: `06 06 06 06 06 00 <read> 06 06 06`). Trusting it blindly remounted on every file op and wiped pending FAT state — five write tests failed (`trunc0`, `savewrite`, `dirmut`, `ovlbig`, `shellcopy`). So a "changed" answer is confirmed by content before anything is discarded: re-read the first root directory sector and compare against the cache (the boot sectors of the WC disks are byte-identical DOS 3.x format with no serial, so the root sector is the distinguishing fingerprint; a swapped truly-identical disk stays indistinguishable, as on real DOS). `ROOT_SEG` is write-through (root mutations happen in it and flush from it), so the compare is always against current state.
- `scripts/test_hdfloppy.py` grew the stay-on-A: scenario (TDD: failed before the kernel change), with a >2s wait after the swap to respect the 2-second rule — the same hole real DOS had.
- The game itself runs remarkably cleanly: installer (drive/graphics/sound menus, two swaps, VGA art expansion), opening credits, and the Claw Marks copy-protection quiz. The drawn question is random per launch and the bank is not plaintext in the EXE, but the *drawn* question is plaintext in guest RAM — the smoke pmemsaves 640 KiB, regex-extracts the question, and answers from the documented table (Origin's own press answer sheet, cross-checked with the CIC list, 19 keyword-keyed entries). One UI quirk: the Vega campaign/Secret Missions select treats ENTER as abort ("Abort from Wing Commander"); it wants a mouse click, so the smoke steers the PS/2 cursor by locating the cyan arrow in screendumps and closing the loop with relative `mouse_move`s. After the click: arcade intro, simulator name/callsign entry, ESC, and the animated bar scene (Shotglass, the pilot board, ~150 colors, 224k lit pixels). "No Expanded Memory Detected" is printed and harmless.
- `make test-wc-smoke` drives install-through-bar headlessly; the suite stays 141/141 with the extended `test_hdfloppy` in the default ladder.

## 2026-06-12 Settlers II Gold: Arena Top, First Fit, Mouse Info — Menu Reached, Input Pump Stalled

- Staged the Gold Edition CloneCD rip (mixed mode: MODE1/2352 data track + eight audio tracks; `extract_mode1_2352.py` learned the single-digit `INDEX 1` cue variant) and drove the Blue Byte CD installer end to end: graphics menu UI where ENTER activates the focused button but SPACE activates dialog buttons (ENTER cancels), Miles Sound System text-mode config (SB16 MIDI auto-detected; digital auto-detect reports hardware not found — unresolved side issue), install completes to `C:\BLUEBYTE\SIEDLER2`.
- First launch crashed with a DOS/4GW divide-by-zero in a VESA banked-window routine. The trail had three genuine kernel findings before the game-side cause:
  1. The MCB arena ran to a hardcoded 640K while INT 12h reports 639K — the last kilobyte is the EBDA, which the BIOS PS/2 services use. DOS/4GW's transfer buffer landed inside it. Real DOS sizes the arena from INT 12h; now we do too (`test_memtop.py`, TDD).
  2. The real divide-by-zero trigger: our small-allocation last-fit bias (an early MI2-demo heuristic the original debug log already flagged for replacement) handed DOS/4GW's transfer buffer segment 0x9F8B, and S2 converts real-mode far pointers to flat with a *signed* segment shift — `0x9F8B<<4` sign-extends to 0xFFF9F8B0 and its mode record reads zeros. Real DOS first fit gives low segments and hides the game bug. The bias is removed; MI2 demo and the vendor `make test-mi2-save` still pass, and the removal exposed real under-allocated overlay buffers in `cdexec`/`ovltest` (37-paragraph overlay loaded into 0x20-paragraph buffers — harmless only because last-fit placed them against the arena tail).
  3. The VESA mode-flag mystery en route: SeaBIOS builds the VBE mode list in its volatile scratch RAM (`VideoModePtr=9E70:01CD`), not in ROM as era BIOSes did, so Blue Byte SETUP's deferred parse of the list reads garbage and writes `MODE_*=N` for everything — an emulator/BIOS-class behavior, not a kernel gap.
- With first fit restored the game runs: 640x480 VESA splash, then the full main menu (Römische Kampagne … Programm verlassen, 157 colors). INT 33h gained the functions S2 probes — AX=0024h driver info (8.20/PS2/IRQ0 — before this the game logged "Unknown mouse type"), AX=0015h/16h/17h state size/save/restore — and callbacks now pass the cumulative mickey counters in SI/DI like real drivers (`mousetest`/`mousecb` extended).
- Remaining blocker: the menu consumes no input. Evidence: mouse events verifiably reach the game's DOS/4GW real-mode callback (mask 0x1F, advancing coordinates logged at the call site), keystrokes are eaten instantly off the BIOS buffer by the game's INT 9 hook, the splash advances by timeout — yet nothing reacts. The event pump that should process the queued input appears timer-driven and never runs. This smells like the Civilization QEMU PIT interaction class; next step is an 86Box cross-check, which needs a mouse-injection endpoint added to the RPC patch.
- Resolved by the 86Box cross-check, no RPC needed: under an interactive 86Box VM (Pentium 75 `p54tp4xe`, 16 MiB, S3 Trio32 PCI, PS/2 mouse, SB16, the C: image on IDE 0:0 with 63/16/325 geometry and the ISO as ATAPI 1:0) the game is fully playable — menus, campaign start, in-game — so the stalled pump is the QEMU emulator-interaction class, not a kernel gap. The same interaction swallows the installer's post-install Setup menu under QEMU: no key moves the button focus (DOWN/TAB/ESC leave the screen pixel-identical) and button clicks never fire, while cursor *motion* and ENTER/SPACE get through — under 86Box the keyboard works normally, so the focus/click dispatch evidently rides the same stalled pump while ENTER/SPACE are read more directly. The vendor-gated smoke therefore exits that menu the period way — reboot after install, with QEMU's `-snapshot` overlay carrying the installed C: across `system_reset` — and `make test-settlers2-smoke` drives install → reboot → launch to the 640x480 menu with the BIOS tick advancing. A steering postscript for future smokes: closing the loop on consecutive screendump diffs reads the median of the old-plus-new cursor clusters as the position and converges on the wrong button; the WC smoke survives this only because its target box is far larger than the cluster separation. The reliable recipe (now proven at the game menu): park the cursor in a corner with gentle single-packet `mouse_move -100 -100` repeats (one `mouse_move -2000 -2000` burst desyncs the PS/2 stream), grab a baseline, and steer by diff-vs-baseline with the corner region excluded.
- A QEMU timer-option sweep against the installed image rules out the obvious knobs: `-rtc clock=vm`, `-icount shift=auto`, `-icount shift=4`, `-cpu pentium`, and `-machine hpet=off` all behave exactly like the baseline — the menu redraws its cursor on PS/2 motion (so the INT 33h callback path is alive) but never dispatches clicks or keystrokes; a steered click dead-center on the Intro button leaves the screen pixel-identical, with not even a hover highlight. Only `-icount shift=4` changes anything at all (the BIOS tick slows from ~21/s to ~9/s, dispatch still dead). So the stall is insensitive to virtual-clock pacing; whatever the menu's event dispatch waits on never fires under QEMU's emulation specifically.

## 2026-06-12 CD Audio: MSCDEX Device Requests Over ATAPI

- Settlers II Gold reported "no audio tracks" even fully playable under 86Box with the mixed-mode cue/bin mounted — and the gap was ours, not the emulator's: the MSCDEX surface stopped at the detection calls (1500h/150Bh/150Ch/150Dh). Games do CD audio through INT 2Fh AX=1510h, handing the kernel a DOS device request header; without it no title can even enumerate audio tracks.
- The new dispatcher serves the audio path a game needs: IOCTL Input control blocks 10/11 (Audio Disc/Track Info, from ATAPI READ TOC in MSF form), 12/15 (Q-channel position and audio status, from READ SUB-CHANNEL), 6/7/8/9 (device status with the audio-capable bit, sector size, volume size from the lead-out, media changed), and device commands 132/133/136 (Play Audio in both HSG and Red Book addressing via PLAY AUDIO MSF, Stop via STOP PLAY/SCAN as best-effort like era drivers, Resume via PAUSE/RESUME). The READ(10) PIO plumbing generalized into `atapi_packet_in` (variable-length responses, zero-length command packets).
- Two emulator-shaped findings on the way:
  1. The ATAPI probe's power-on signature check (byte counts 0x14/0xEB) fails once the BIOS has talked to the device — those registers hold the last transfer's byte count, not the signature. The probe now falls back to IDENTIFY PACKET DEVICE (0xA1), which only ATAPI devices answer with a data phase. This also makes the lazy bring-up work under QEMU, where the EDD path wins the mount and the ATAPI transport is only touched on the first audio request.
  2. A division-overflow guard in the LBA-to-MSF conversion: a hostile (or buggy-caller) start sector past any disc's capacity used to fault INT 0 inside the INT 2Fh handler — found the hard way when the test program's Play request carried a garbage pointer-shaped LBA.
- `tests/programs/cdaudio.asm` pins the contract under QEMU against the generated single-data-track ISO: bogus command and bad unit produce done+error with the right codes, disc info reports tracks 1..1 with a nonzero lead-out, track info flags the data-track control bit, Play on a data disc fails cleanly (QEMU's ATAPI implements no audio commands), Stop still reports done. The suite is 143/143 with `test_cd_audio` in the default ladder. Actual playback verification needs 86Box with the cue/bin loaded — QEMU cannot mount mixed-mode images at all.

## 2026-06-12 CD Audio: The Busy Bit Games Actually Poll

- First 86Box test with the real cue/bin: the game's CD player listed all eight tracks (the 1510h TOC path works) but its UI ran inverted — selecting a track started the drive (86Box's CD icon flipped to playing) yet went silent immediately, while "Wiedergabe stoppen" *started* playback. Classic symptom: the CD-ROM device driver spec requires the request status word to carry the busy bit (0x0200) while an audio play operation is in progress, and CD player UIs poll it to track state. Without it the game saw "not busy" right after Play, concluded playback had already ended, and stopped the drive; its stop button then toggled from the inverted state.
- Every 1510h request now ORs the busy bit into its returned status while READ SUB-CHANNEL reports play-in-progress (0x11), and Stop Audio (133) maps to ATAPI PAUSE/RESUME hold rather than STOP PLAY/SCAN — the spec gives Stop pause-with-resume-point semantics, which also keeps Resume Audio (136) meaningful.

## 2026-06-12 The "CD Read Hang" That Wasn't: Boot COMs Run In A Cramped Block

- Red Alert's DOS installer black-screened under both QEMU and 86Box, and a new chunk-pattern CD read test (200 KB file, mixed chunk sizes) hung deterministically on a 6144-byte read — everything pointed at the CD path. A long forensic chain (gdb watchpoints crawl under TCG hardware watch overhead; per-iteration serial markers worked far better) kept exonerating each suspect: the EDD DAP was intact through every sector read, stack discipline was perfect, `indos` was sane, and the INT 21h entry was parking SS:SP = 9E90:12F6 — which looked like SeaBIOS's EBDA stack until the program printed its own SS:SP and matched it.
- The truth: the boot launcher (`BOOT_FILE` path) allocates the boot program with `alloc_mem_direct_high` sized to the file, so the test COM lived at segment ~0x9E90 in a block barely bigger than its image, with `exec_com_dyn`'s small-block rule putting SP at the block top, ~0x12FE. The test's in-image 30 KB buffer ran past its own stack: the 6144-byte read overwrote the program's saved INT 21h iret frame, the kernel's (correct) restore handed control to overwritten memory, and the machine wandered into the weeds — the "BIOS-context corruption" in the gdb dumps was downstream wreckage, not cause. Three-iteration reads stopped just short of the stack; four reached it.
- The kernel CD read path is fully exonerated: with the test reading into a separately allocated 32 KiB block (INT 21h AH=48h), the whole adversarial chunk schedule passes on the untouched kernel (`test_cd_chunks`, suite 144/144). Real DOS would have made the original test legal — a COM gets the largest free block — so the boot launcher's cramped high allocation is filed as a faithfulness issue (boot-com-largest-block).
- Debugging notes for the next survivor: QEMU TCG hardware watchpoints slow the whole guest ~100x (gate the workload behind a keypress and arm the watch late); `pmemsave` still silently fails with absolute paths; and a wedged 86Box that deadlocks in `startblit` survives SIGTERM from the sandbox — fresh image copies beat fighting the file lock.

## 2026-06-12 COM Programs Own The Largest Block, Like Real DOS
...
- The installer's black screen (identical under QEMU and 86Box, with constant CD access) came down to one masked byte. The INT 21h trace showed a healthy open/seek/read/close asset loop over `SETUP.MIX` — but every close consumed a fresh handle (5, 6, 7 … 0x13), and after fifteen the table was exhausted: 40,000+ failed reopens in ninety seconds, the installer retrying its asset loads forever. A slot ledger (`+N` on alloc, `-N` on release, route markers in the close path) pinned it: `r1 R F e` — refcount fine, `close_root_handle` entered, the **flush-on-close path taken, flush fails on the read-only CD, slot never freed**.
- Why the flush path? Era programs open read-only files with DOS sharing bits set — Red Alert uses deny-none reads (`AX=3D20h`). `close_root_handle` tested the whole `H_MODE` byte against zero to decide whether the file needs its directory entry flushed back; share bits made a read-only open look writable. The write path already masked with `and al, 7`; the close path forgot. One masked comparison fixes it, and the installer's full sound-setup UI appears where six minutes of black used to be.
- Repro lesson: three minimal repros (open/close loops — boot COM, shell child, subdirectory file) all failed to reproduce because they opened with `AL=0`. The discriminating variable was the *mode byte*, visible only once the slot ledger contradicted the seemingly-clean call trace. `test_cd_share` pins the contract (thirty share-mode open/close cycles must reuse one slot); suite 146/146.

## 2026-06-12 Red Alert: The "Hang" That Was A Dialog Waiting For A Label

- The installed game "hung at launch with constant HD read and occasional write." The INT 21h trace told a stranger story: a healthy startup — 16 MB swap created (`DOS4GVM=SwapMin:12M` comes from `RA.EXE` itself), ~15 MB XMS handed to DOS/4GW Professional in one clean allocation, `LOCAL.MIX`/`LORES.MIX` cached out of the 25 MB `REDALERT.MIX` — and then total DOS silence at the byte-identical trace position, every run. CS:EIP sampling through the QEMU monitor showed the game *alive*, spinning across broad swaths of protected-mode code with IRQ0 ticking at 142 Hz. Not a hang at all: a `screendump` finally showed "PLEASE INSERT A RED ALERT CD INTO THE CD-ROM DRIVE", fully drawn, mouse cursor and all — with the disc in the drive.
- The GPL'd engine source pinned the check: `Get_CD_Index()` calls `_dos_findfirst("D:\", _A_VOLID, &ft)` and compares the returned **volume label** against `"CD1"`/`"CD2"` before it will believe a disc is present. MSCDEX answers an exclusive volume-label search (attribute byte exactly `0x08`) from the ISO9660 PVD volume identifier, ignoring the search pattern entirely — a bare `D:\` works on real DOS. Our FindFirst dutifully searched the root directory for an entry matching an empty pattern, found nothing, and returned error 2; the game concluded "no disc" and put up the dialog. The fix is MSCDEX's behavior: `cd_validate_pvd` now keeps the PVD volume id (bytes 40–50), and an exclusive-`ATTR_VOLUME` FindFirst on the CD drive synthesizes the label entry instead of searching. One run later the Allied disc identifies as `CD1` and the intro movie streams off the CD ("Trinity, New Mexico").
- Two instrumentation lessons. First, `-serial stdio` backpressure can throttle the guest hard enough to fake a 10x slowdown — the same load that "took 150+ seconds" finished in 19 with `-serial file:`; never benchmark through a chatty serial console. Second, a game waiting on a drawn dialog is indistinguishable from a hang in a file-op trace — the screen is evidence too; dump it early. `test_cd_volid` pins the contract (bare-root label query returns the volume id with attribute `0x08`, FindNext ends the search, wildcards are ignored, normal searches stay label-free, unlabeled FAT volumes still miss); suite 147/147.

## 2026-06-13 Red Alert: The DOS/4GW "Hang" Was A 16 MB Swap, Written One Sector At A Time

- With the CD volume-label fixed, the installed game still "hung on the DOS/4GW screen with constant hard-disk access and occasional writes" under 86Box -- but ran fine under QEMU. The screen was a clue the trace alone hid: under QEMU the game switched to graphics; under 86Box it sat in *text mode* at the DOS/4GW banner, so it was stalling earlier than the CD check, during extender startup, on the hard disk (not the CD).
- First instinct -- DOS/4GW thrashing swap because it got little XMS -- was wrong. A throwaway `XMSINFO.COM` booted on a copy of the real image reported the *same* 15296 KB of XMS free under both emulators (the BIOS `INT 15h AH=88h` agrees; only E801's above-16M field differs, which DOS/4GW doesn't use). Same memory, so not memory.
- The INT 21h trace froze, byte-for-byte every run, at `SEEK H=0005 ORG=0000 OFF=00FFFFFF` -- DOS/4GW seeking to 16 MiB-1 in its swap file (`DOS4GVM=SwapMin:12M`, set by `RA.EXE`) and writing one byte to reserve the space. Writes aren't traced, so the trace went dark while the kernel churned. A 5-minute 86Box run eventually cleared it (~250 s) and reached the game -- a very slow crawl, not an infinite hang, which is why QEMU's instant disk never showed it.
- Two stacked costs, both O(file size) in disk sectors. (1) Extending across a gap, the write path read-modify-wrote *every* sector of the 0->16 MiB gap to zero-fill it -- 32K sector RMWs. Real DOS leaves a write-past-EOF gap undefined; it only allocates clusters. Fixed `.wf_gap_start` to allocate the chain through the write position via `wf_get_cluster` and set the size, no data I/O (on a zero-initialised volume the new clusters still read back as zero). That alone cut 86Box from ~250 s to ~95 s. (2) The residual ~95 s was the FAT itself: `fat16_set` was write-through, read-modify-writing *both* FAT copies on every single entry -- ~8K entries x 4 sector I/Os for the swap's clusters. FAT12 had long used a resident in-memory table flushed once; FAT16 (large tables) used a one-sector read cache but committed writes immediately. Gave FAT16 a write-back window: `fat16_window_load` keeps one FAT sector in `FAT_SEG`, flushing it to all copies only when the window advances or at `flush_fat`. `activate_drive` already flushed the FAT before switching volumes, so the discipline FAT12 relied on covers FAT16 unchanged.
- Result on 86Box: the swap reservation that took ~250 s now clears in under 19 s; the intro movie is on screen by ~40 s. The write-back window speeds up *every* large FAT16 write, not just swap files. `test_gap_write` pins the behavior (4 MiB gap extend: right size, marker intact, gap reads zero), and the existing FAT16 suite guards coherence; suite 148/148.
- Instrumentation notes: `-serial stdio` backpressure can throttle the guest ~10x (19 s vs 150 s for the same load) -- benchmark with `-serial file:`. And a file-op trace that goes silent isn't necessarily wedged: untraced writes plus a real-speed disk look identical to a hang. Watch the screen and sample CS:EIP before concluding.

## 2026-06-13 Red Alert: Slow Mission Loads — The CD Had No Read Cache

- Once Red Alert was playable, mission loads still took ~a minute off the CD while gameplay ran fine. The 86Box drive is modeled as 8x (~1.2 MB/s), so that was far too slow. (First I mislabeled the read path: I claimed both emulators used the BIOS route; a method-print proved the opposite — QEMU uses INT 13h/EDD (method 0), 86Box uses ATAPI (method 1). The user remembered correctly.)
- Benchmarks reframed it. A sequential 1 MiB read of D:\MAIN.MIX took ~20 ticks (~1.1 s, ~0.9 MB/s) on 86Box — near the 8x rating, so raw bandwidth wasn't the problem. The tell was chunk size: re-reading the same 64 KiB region in 64-byte chunks took 38 ticks vs 1 tick for 8 KiB chunks — 38x slower for identical data. The CD read path (`cd_read_handle`) had no sector cache, unlike the hard-disk path. Every read re-fetched a full 2 KiB sector over ATAPI, so the game's many small reads of the MIX archive (an index-plus-records container it re-reads constantly) paid an ATAPI round-trip per record.
- Added a one-sector read cache: `cd_read_handle` keeps the last sector in `CD_BUF` keyed by `cd_rd_cache_lba` and skips the fetch on a hit. The subtlety is that `CD_BUF` is shared scratch — directory scans, the PVD, and audio TOC/sub-channel all read into it. Rather than sprinkle invalidations, `cd_read_sector_drive` (the one routine every raw read funnels through) clears the cache; `cd_read_handle` re-validates only after its own read. So any non-file read — including a directory re-resolve after a disc swap — leaves the cache invalid automatically, which is exactly the safe default.
- Result: the 64-byte-chunk benchmark dropped from 38 ticks to 2 on 86Box (~19x); small/repeated MIX reads are now CD_BUF hits instead of ATAPI fetches. `test_cd_cache` pins coherence (a FindFirst between two reads of the same file sector must still return the file's bytes, plus cache-hit and sector-crossing reads); the existing `test_cd_chunks` already exercises sub-sector reads for correctness. Suite 149/149.
- Note on what this does *not* fix: true random seeks across the disc still cost the drive's seek latency; the cache only removes redundant re-fetches of the same sector. For RA's MIX access that is the dominant cost, but a different title that genuinely streams seekily would still be drive-bound.

## 2026-06-13 Hard-Disk Write Benchmark Baseline

- Added an opt-in benchmark counter build (`PERF_IO_COUNTS=1`) and `make bench-disk-write`, which boots a generated FAT16 hard-disk image and runs `PERFWR.COM`. The private benchmark API is `INT 21h AH=F0h`: `AL=0` resets counters and `AL=1` prints them to serial. Normal kernels remain quiet.
- First QEMU baseline: `WRITE512` (128 x 512-byte writes) took about 1 tick with `RD=1 WR=132 FF=1 F16=1 DIR=2 WFC=128 WFP=0`; `WRITE128` (512 x 128-byte writes) took low single-digit ticks with `RD=512 WR=516 FF=1 F16=1 DIR=2 WFC=512 WFP=512`. The discriminator is clean: full-sector writes avoid prereads, while small save-style writes read-modify-write every record.
- Added a conservative same-sector preread reuse cache for consecutive `AH=40h` calls. The cache is keyed by active drive and LBA, refreshed after each successful sector write, and invalidated by any non-write INT 21h entry. The generated benchmark now reports `WRITE128` at `RD=128 WR=516 WFC=512 WFP=128`, so redundant prereads dropped 4x while synchronous sector writes remain the next target.

## 2026-06-13 I/O Hot-Path Benchmark Suite

- Added `make bench-io-hot-paths`, which boots generated FAT16 hard-disk media plus a generated ISO and runs `PERFIO.COM` with the opt-in `PERF_IO_COUNTS=1` API. The suite covers `WRITE512`, `WRITE128`, `DRIVESW`, `FAT16ALLOC`, `METADATA`, and `CDMIX64`; the counter line now includes `FA` for FAT allocation events.
- First QEMU result: `WRITE512 ticks=0 rd=1 wr=132 fa=16`, `WRITE128 ticks=2 rd=128 wr=516 fa=16`, `DRIVESW ticks=3 rd=1089 wr=35 cd=1 dsw=65`, `FAT16ALLOC ticks=1 rd=1 wr=260 fa=32`, `METADATA ticks=0 wr=67 ff=33 f16=1 dir=65`, `CDMIX64 ticks=0 cd=3`.
- Interpretation: the synthetic suite now distinguishes the major suspected hot paths without vendor media. The immediate save-game bottleneck remains one physical hard-disk write per 128-byte record (`WRITE128 WR=516`), with redundant metadata flushes visible separately (`METADATA DIR=65`, `FF=33`). Red Alert confirmation remains vendor-gated and was not run here.

## 2026-06-13 Hard-Disk Write-Back Cache

- Added a dedicated one-sector `WRITE_CACHE_BUF` for hard-disk data writes. `AH=40h` now merges repeated partial writes to the same hard-disk sector and defers the physical write until the cache moves to another sector or an explicit flush point runs. Dirty data is flushed before drive switches, reads, disk reset, close/commit/time metadata writes, and before reusing the cache for another sector. The cache is deliberately write-through for floppy media.
- `make bench-disk-write` now verifies written file contents, includes a read-before-close dirty-cache check, and covers 512-byte, 128-byte, and 64-byte records. Result: `WRITE512 WR=132 WD=128`, `WRITE128 WR=132 WD=128`, `WRITE64 WR=132 WD=128`; before dirty coalescing, the 128-byte pattern was `WR=516`.
- Regression caught and fixed: applying deferred writes to floppy media broke `test_savewrite.py` at the first read-after-write check (`FAIL: COMPARE INITIAL`) even though the final image contents were correct. Limiting deferred write-back to BIOS hard disks keeps floppy writes synchronous and restored the test.
- Follow-up regression caught by the hard-disk benchmark: a same-handle read before close initially returned stale `0x5A` sector contents even though the dirty write cache flushed. The fix is to satisfy same-LBA reads from `WRITE_CACHE_BUF` after flushing it, filling `READ_CACHE_BUF` directly instead of immediately rereading the just-written BIOS sector.
- Verification after the hard-disk-only change: `make bench-disk-write`, `make bench-io-hot-paths`, `python3 scripts/test_savewrite.py`, `python3 scripts/test_commit.py`, `python3 scripts/test_termflush.py`, `python3 scripts/test_readcache.py`, `python3 scripts/test_fat16_seek.py`, and `python3 scripts/run_tests.py -j 4` (`150/150`) all passed.

## 2026-06-13 Drive-Switch Root Buffer Thrash

- The synthetic `DRIVESW` phase was still expensive after the write-back cache: alternating a C: hard-disk write with a D: CD read made 65 drive switches and `RD=1065`. The count matches reloading the 32-sector FAT16 C: root directory on every return from CD.
...
## 2026-06-13 Save Metadata Flush Coalescing

- Added `make bench-metadata`, a generated metadata benchmark that measures repeated timestamp updates plus commits, clean commits, clean closes, same-size overwrites, temp-file commit/rename, and delete-old-save patterns. The runner now enforces sector-write ceilings for those phases.
- Baseline before dirty tracking: focused metadata churn showed `TIMECOMMIT WR=68 DIR=65`, `CLEANCOMMIT WR=33 DIR=33`, `CLEANCLOSE WR=1 DIR=1`, and `OVERWRITE WR=3 DIR=2`. The broader `PERFIO` `METADATA` phase likewise showed redundant directory writes (`wr=67/68`, `dir=65`) around repeated `AH=5701` plus `AH=68h` calls.
- Fix: each real file handle uses the spare byte after `H_DRIVE` as a metadata-dirty flag. Size/cluster/truncate/time changes mark it, successful directory-entry flush clears it, and `AH=68h`, close, and termination still flush dirty data first before deciding whether the directory sector needs rewriting. Clean FAT flush calls remain allowed; they write zero sectors when the FAT window is clean.
- Follow-up coverage added after review: `bench-metadata` now also runs a `SUBDIRTIME` phase and repeats the full phase set on a generated FAT12 hard-disk image; `scripts/test_metafail.py` builds with `TEST_FLUSH_DIR_SLOT_FAIL_AFTER=2` and proves a failed file-time directory flush leaves dirty size/cluster/time metadata retryable by a later commit.
- Result: FAT16 `make bench-metadata` reports `TIMECOMMIT WR=35 DIR=32`, `CLEANCOMMIT WR=0 DIR=0`, `CLEANCLOSE WR=0 DIR=0`, `OVERWRITE WR=1 DIR=0`, `TEMPRENAME WR=6 DIR=3`, `DELETEOLD WR=3 DIR=1`, and `SUBDIRTIME WR=35 DIR=32`. FAT12 reports the same clean zero-write behavior with expected FAT12 FAT-sector writes, e.g. `TIMECOMMIT WR=49 DIR=32`, `TEMPRENAME WR=20 DIR=3`, and `DELETEOLD WR=17 DIR=1`. `make bench-io-hot-paths` now reports `METADATA wr=35 dir=32` while the write-cache and drive-switch phases stay in their previous range.
- Verification: `make`, `make check-docs-sync`, `git diff --check`, `make bench-metadata`, `make bench-disk-write`, `make bench-io-hot-paths`, focused commit/termination/save/truncate/directory/time/rename-delete tests, `python3 scripts/test_metafail.py`, and full `make test` (`151/151`) all passed.

## 2026-06-13 CD-ROM Media-Swap Invalidation

- Added `scripts/test_cd_media_swap.py` and `tests/programs/cdswap.asm`. The generated QEMU monitor regression boots with an `OLDCD` ISO, verifies the old volume/file, waits at `READY: CDSWAP`, ejects and changes `ide0-cd0` to a `NEWCD` ISO, then requires the first post-swap volume-label FindFirst, normal root FindFirst, and `D:\NEWFILE.TXT` open/read to see the new disc while `D:\OLD.TXT` is rejected.
- Baseline failure was stale state: the first post-swap volume-label query returned `OLDCD` (`FAIL: CDSWAP STALE LABEL`). A PVD refresh through the existing BIOS EDD path changed that into a first-lookup failure (`FAIL: CDSWAP NEW LABEL`) because SeaBIOS may fail or serve stale sectors after QEMU media changes.
- Fix: CD path/directory opens now refresh PVD/root/volume metadata through direct ATAPI when that transport is available. If an ATAPI read hits post-swap unit attention, the kernel pulses SRST on the ATAPI control port and retries. A valid ATAPI PVD commits `CD_READ_METHOD_ATAPI`, so subsequent directory scans and file reads use the same authoritative transport instead of falling back to stale BIOS EDD sectors. The one-sector file cache still stays out of `cd_read_handle` refresh work and is invalidated at open/resolve boundaries.
- Verification: `make`, five consecutive `python3 scripts/test_cd_media_swap.py` runs, focused generated CD ladder (`test_cd_bios`, `test_cd_file`, `test_cd_subdir`, `test_cd_find`, `test_cd_mscdex`, `test_cd_audio`, `test_cd_chunks`, `test_cd_share`, `test_cd_exec`, `test_cd_media_swap`, plus `test_cd_volid`, `test_cd_cache`, `test_cdmut`, and `test_cddots`), `make check-docs-sync`, and full `make test` (`152/152`) all passed.
- Follow-up probe: `make test-cd-86box` and `python3 scripts/test_cd_86box.py --boot-only` both timed out with empty serial output in the local 86Box harness, so they did not discriminate this CD change. The boot-only failure points at emulator/profile/stdio plumbing rather than the CD media-swap path.

## 2026-06-13 CD-ROM Archive Read Cache

- Added `make bench-cd-cache`, `scripts/bench_cd_cache.py`, and `tests/programs/perfcd.asm`. The generated ISO benchmark reads a 4-sector archive-shaped file with repeated same-sector, sequential 64-byte, two-sector alternating, and four-sector alternating patterns under `PERF_IO_COUNTS=1`.
- Baseline after media-swap correctness but before expanding the cache: `CDSAME64 CD=1`, `CDSEQ64 CD=3`, `CDALT2_64 CD=64`, and `CDALT4_64 CD=64`. Same-sector and sequential patterns were already acceptable, but alternating MIX-style access defeated the one-sector file cache.
- Fix: `cd_read_handle` now looks up sectors in a four-slot cache backed by dedicated 2 KiB buffers at `CD_CACHE_BUF`; raw CD sector users still enter through `cd_read_sector_drive`, which invalidates the file cache before PVD, directory, audio, and media-refresh reads. An intermediate probe showed `CDALT2_64 CD=3` and `CDALT4_64 CD=5`; the cause was a stale invalidation in the low-level fetch helper clearing slot 0 on every cache miss.
