...
PROGRAM = os.path.join(WORKDIR, "cdmut.com")
HELLO = os.path.join(WORKDIR, "hello.txt")
IMG = os.path.join(WORKDIR, "cdmut.img")
ISO = os.path.join(WORKDIR, "cdmut.iso")
TIMEOUT = 10
...
def build_artifacts():
    ...
    with open(HELLO, "wb") as f:
        f.write(b"Hello from LainDOS CD mutation guard test.\r\n")
    run_cmd(["python3", "scripts/mkiso.py", ISO, f"HELLO.TXT={HELLO}"])
    run_cmd(["nasm", "-DFAT12=1", "-f", "bin", "src/boot.asm", "-o", BOOT])
    run_cmd(["nasm", '-DBOOT_FILE="CDMUT   COM"', "-f", "bin", "src/kernel.asm",
             "-o", KERNEL])
    run_cmd(["nasm", "-f", "bin", "tests/programs/cdmut.asm", "-o", PROGRAM])
