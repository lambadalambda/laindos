...
BUILDDIR = build_dir()
IMG = os.path.join(BUILDDIR, "duptest.img")
KERNEL = os.path.join(BUILDDIR, "duptest_kernel.bin")
TIMEOUT = 10
...
def build_image():
    ...
    dup_com = os.path.join(BUILDDIR, "duptest.com")
...
def main():
    ...
    output = run_qemu()
    failed = False
    for marker in ["PASS: DUP", "Program exited, code=00", "HALT"]:
        if marker in output:
            print(f"  PASS: found '{marker}'")
        else:
            print(f"  FAIL: missing '{marker}'")
            failed = True
    for marker in ["FAIL:", "EXC ", "INT 21h AH=45", "INT 21h AH=46"]:
        if marker in output:
