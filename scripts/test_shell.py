...
def send_keys(output_chunks):
    ...
    require_command_output("del missing.dat", del_missing, ["File not found"], ["File error"])
    rd_nonempty = send_command(sock, output_chunks, "rd dironly")
    require_command_output("rd dironly", rd_nonempty, ["Directory not empty or access denied"], ["Path not found"])
    deltree_no_arg = send_command(sock, output_chunks, "deltree")
    require_command_output("deltree", deltree_no_arg, ["Missing argument"])
    deltree_too_many = send_command(sock, output_chunks, "deltree one two")
    require_command_output("deltree one two", deltree_too_many, ["Too many arguments"])
    deltree_wild = send_command(sock, output_chunks, "deltree /y tr*")
    require_command_output("deltree /y tr*", deltree_wild, ["Wildcard not supported"])
    deltree_file = send_command(sock, output_chunks, "deltree /y testfile.dat")
    require_command_output("deltree /y testfile.dat", deltree_file, ["Directory not empty or access denied"])
    deltree_root = send_command(sock, output_chunks, "deltree /y " + "\\")
    require_command_output("deltree /y root", deltree_root, ["Cannot delete root directory"])
    deltree_current = send_command(sock, output_chunks, "deltree /y .")
    require_command_output("deltree /y .", deltree_current, ["Cannot delete current directory"])
    root_guard_type = send_command(sock, output_chunks, "type testfile.dat")
    require_command_output("type testfile.dat after deltree guards", root_guard_type, ["Hello from TESTFILE.DAT!"])
    send_command(sock, output_chunks, "md ask")
    ask_copy = send_command(sock, output_chunks, "copy testfile.dat ask\\ask.dat")
    require_command_output("copy testfile.dat ask\\ask.dat", ask_copy, ["1 File(s) copied."])
    ask_cancel = send_command_answer(sock, output_chunks, "deltree ask", "Delete directory tree", "n")
    require_command_output("deltree ask", ask_cancel, ["Directory tree not deleted."])
    ask_cancel_type = send_command(sock, output_chunks, "type ask\\ask.dat")
    require_command_output("type ask\\ask.dat after deltree cancel", ask_cancel_type, ["Hello from TESTFILE.DAT!"])
    ask_confirm = send_command_answer(sock, output_chunks, "deltree ask", "Delete directory tree", "y")
    require_command_output("deltree ask", ask_confirm, ["Deleted directory tree"])
    ask_confirm_type = send_command(sock, output_chunks, "type ask\\ask.dat")
    require_command_output("type ask\\ask.dat after deltree confirm", ask_confirm_type, ["File not found"])
    send_command(sock, output_chunks, "md tree")
    send_command(sock, output_chunks, "md tree\\sub")
    send_command(sock, output_chunks, "md tree\\sub\\deep")
    tree_copy = send_command(sock, output_chunks, "copy testfile.dat tree\\one.dat")
    require_command_output("copy testfile.dat tree\\one.dat", tree_copy, ["1 File(s) copied."])
    subtree_copy = send_command(sock, output_chunks, "copy testfile.dat tree\\sub\\two.dat")
    require_command_output("copy testfile.dat tree\\sub\\two.dat", subtree_copy, ["1 File(s) copied."])
    deep_copy = send_command(sock, output_chunks, "copy testfile.dat tree\\sub\\deep\\three.dat")
    require_command_output("copy testfile.dat tree\\sub\\deep\\three.dat", deep_copy, ["1 File(s) copied."])
    deltree = send_command(sock, output_chunks, "deltree /y tree")
    require_command_output("deltree /y tree", deltree, ["Deleted directory tree"], ["Path not found"])
    deltree_type = send_command(sock, output_chunks, "type tree\\sub\\deep\\three.dat")
    require_command_output("type tree\\sub\\deep\\three.dat", deltree_type, ["File not found"])
    deltree_cwd = send_command(sock, output_chunks, "cd")
    require_command_output("cd after deltree", deltree_cwd, ["A:\\"])
    send_command(sock, output_chunks, "md selfdel")
    selfdel_copy = send_command(sock, output_chunks, "copy testfile.dat selfdel\\in.dat")
    require_command_output("copy testfile.dat selfdel\\in.dat", selfdel_copy, ["1 File(s) copied."])
    send_command(sock, output_chunks, "cd selfdel")
    selfdel = send_command(sock, output_chunks, "deltree /y \\selfdel")
    require_command_output("deltree /y \\selfdel", selfdel, ["Deleted directory tree"])
    selfdel_cwd = send_command(sock, output_chunks, "cd")
    require_command_output("cd after deleting current dir", selfdel_cwd, ["A:\\"])
    selfdel_type = send_command(sock, output_chunks, "type selfdel\\in.dat")
    require_command_output("type selfdel\\in.dat", selfdel_type, ["File not found"])
    ...
    ren_missing = send_command(sock, output_chunks, "ren missing.dat missing2.dat")
    require_command_output("ren missing.dat missing2.dat", ren_missing, ["File not found"], ["File error"])
