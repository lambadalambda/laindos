...
def build_image():
    ...
    run(["nasm", "-f", "bin", "tests/programs/envmcb.asm", "-o", parent])
    run(["nasm", "-f", "bin", "tests/programs/envchld.asm", "-o", child])
    run(["python3", "scripts/mkimage.py", boot, KERNEL, IMG, parent, child])
...
def main():
    ...
    output = run_qemu()
    failed = False
    for marker in ["PASS: ENVMCB", "Program exited, code=00", "HALT"]:
        if marker in output:
