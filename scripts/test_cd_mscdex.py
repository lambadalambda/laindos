...
BUILDDIR = build_dir()
WORKDIR = os.path.join(BUILDDIR, "cd_mscdex")
BOOT = os.path.join(WORKDIR, "boot.bin")
KERNEL = os.path.join(WORKDIR, "kernel.bin")
PROGRAM = os.path.join(WORKDIR, "cdmscdex.com")
HELLO = os.path.join(WORKDIR, "hello.txt")
IMG = os.path.join(WORKDIR, "cd_mscdex.img")
ISO = os.path.join(WORKDIR, "cd_mscdex.iso")
TIMEOUT = 10
...
def build_artifacts():
    ...
    with open(HELLO, "wb") as f:
        f.write(b"Hello from LainDOS CD-ROM MSCDEX test.\r\n")
    run_cmd(["python3", "scripts/mkiso.py", ISO, f"HELLO.TXT={HELLO}"])
    run_cmd(["nasm", "-DFAT12=1", "-f", "bin", "src/boot.asm", "-o", BOOT])
    run_cmd(["nasm", '-DBOOT_FILE="CDMSCDEXCOM"', "-f", "bin", "src/kernel.asm", "-o", KERNEL])
    run_cmd(["nasm", "-f", "bin", "tests/programs/cdmscdex.asm", "-o", PROGRAM])
...
def main():
    ...
    ok = check_markers(
        output,
        required=("PASS: CDMSCDEX", "Program exited, code=00", "HALT"),
        forbidden=("FAIL:", "EXC ", "INT 21h AH="),
        output_label="CD MSCDEX QEMU serial output",
    )
    if not ok:
        sys.exit(1)
    print("\nCD MSCDEX test passed.")
