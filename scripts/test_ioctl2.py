...
BUILDDIR = build_dir()
FLOPPY_IMG = os.path.join(BUILDDIR, "ioctl2_floppy.img")
HD_IMG = os.path.join(BUILDDIR, "ioctl2_hd.img")
KERNEL = os.path.join(BUILDDIR, "ioctl2_kernel.bin")
TIMEOUT = 12
...
def build_images():
    ...
    program = os.path.join(BUILDDIR, "ioctl2.com")
    aonly = os.path.join(BUILDDIR, "aonly.txt")
    with open(aonly, "wb") as f:
        f.write(b"DRIVEOK")
    run_cmd(["nasm", "-DFAT12=1", "-f", "bin", "src/boot.asm", "-o", boot])
    run_cmd(["nasm", "-DFAT16=1", "-f", "bin", "src/boot.asm", "-o", boot16])
    run_cmd(["nasm", '-DBOOT_FILE="IOCTL2  COM"', "-f", "bin", "src/kernel.asm",
             "-o", KERNEL])
    run_cmd(["nasm", "-f", "bin", "tests/programs/ioctl2.asm", "-o", program])
